...
//                    $('#Nm_pagina').val(""); 
                ...
                valore = componenteTipoHelp.text();
                if (valore == 'HELP_PAGINA') {
                    $('#Nm_pagina').attr("disabled", false);
                    $('#Nm_entry_menu').val("");
                    $('#Nm_entry_menu').attr("disabled", true);
...
//                    $('#Nm_pagina').val(""); 
            ...
            function showHelp(valoreHelp) {
                ...
                $('#helpDialog').dialog({
                    ...
                    modal: true,
                    resizable: true,
                    title: '<%=(String) session.getAttribute("titlePopup")%>',
                    closeOnEscape: true,
                    close: function () {
                        $(this).html('');
...
//                $.base64.utf8encode = true;				
    ...
    <sl:body>
        ...
        <sl:content multipartForm="${sessionScope['###_FORM_CONTAINER']['dettaglioHelpOnLine'].status ne 'view'}">
            ...
            <c:choose>
                <c:when test='<%="INFO_PRIVACY".equals((String) session.getAttribute("tiHelpOnLine"))%>'>
                    <sl:contentTitle title="Info privacy"/>    
                    <c:set var="legend" value="Info privacy" />
                </c:when>
                <c:otherwise>
                    <sl:contentTitle title="Help online"/>                
                    <c:set var="legend" value="Help online" />
                </c:otherwise>
            </c:choose>
            ...
            <sl:newLine />
            <%--<slf:lblField name="<%=GestioneHelpOnLineForm.DettaglioHelpOnLine.INFO_PRIVACY%>" colSpan="4" controlWidth="w40" />
            <sl:newLine />--%>

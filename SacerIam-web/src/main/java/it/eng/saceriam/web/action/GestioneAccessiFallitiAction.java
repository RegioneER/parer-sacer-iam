...
public class GestioneAccessiFallitiAction extends GestioneAccessiFallitiAbstractAction {
	...
	if (getForm().getFiltriAccessiFalliti().validate(getMessageBox())) {
	    ...
	    String descrizioneDataDa = getForm().getFiltriAccessiFalliti().getDt_evento_da()
		    .getHtmlDescription();
	    String descrizioneDataA = getForm().getFiltriAccessiFalliti().getDt_evento_a()
		    .getHtmlDescription();

	    TypeValidator valid = new TypeValidator(getMessageBox());
	    Date[] dateDecorate = valid.validaDate(dtEventoDa, oraRifDa, minRifDa, dtEventoA,
		    oraRifA, minRifA, descrizioneDataDa, descrizioneDataA);
	    ...
	    String inclUtentiAutomi = getForm().getFiltriAccessiFalliti().getIncl_utenti_automi()
		    .parse();
	    ...
	    try {
		loginEventoTableBean = amministrazioneUtentiEjb.getLogVRicAccessiTableBean(
			dateDecorate[0], dateDecorate[1], nmUserid, tipoEvento, inclUtentiAutomi,
			inclLoginOK, nome, cognome, cf, email, idEsterno);
	...
	getForm().getFiltriAccessiFalliti().getTipo_evento()
		.setDecodeMap(ComboGetter.getMappaTipoEvento());
    }

...
public class HelpZipProcessor {
    ...
    /*
     * Attiva il processamento dello zip e la scompattazione di tutto. Torna TRUE se tutto è OK
     * altrimenti FALSE.
     */
	...
	if (zipBlob != null) {
		...
		while ((entry = stream.getNextEntry()) != null) {
			...
			/*
			 * Se trovo una directory che ha piu' di uno slash significa che nello zip
			 * tutto il resto era all'interno di una directory radice quando invece deve
			 * essere tutto sulla root (a parte la directory delle immagini che e'
			 * l'unica ammessa.
			 */
			...
			if ((entryName.endsWith(".docx") || entryName.endsWith(".doc"))
			    ...
			    log.debug(
				    "entryName: " + entry.getName() + ", size: " + entry.getSize());
			    estraiHtm(stream);
			}
		    }
		}
    ...
    /*
     * prende tutto lo zip e naviga solo tra le immagini contenute nella directory delle immagini,
     * codifica le immagini in base64 e le include nei tag <IMG> embeddate nell'HTML dell'help
     */
    ...
    /*
     * Estrae il file HTM dell'Help filtrato da Word e prende solo la parte compresa tra i tag
     * <body></body>
     */
	...
	helpHtml = helpHtml.replaceFirst(pathEncodato,

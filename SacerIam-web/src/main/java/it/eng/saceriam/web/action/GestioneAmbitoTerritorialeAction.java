...
public class GestioneAmbitoTerritorialeAction extends GestioneAmbitoTerritorialeAbstractAction {
    ...
    private static final Logger log = LoggerFactory
	    .getLogger(GestioneAmbitoTerritorialeAction.class);
	...
	OrgAmbitoTerritTableBean table = gestioneAmbitoTerritorialeEjb
		.getOrgAmbitoTerritTableBean(null);
	...
	if (publisher.equals(Application.Publisher.GESTIONE_AMBITO_TERRITORIALE)) {
		...
		// popup per conferma
		try {
		    gestioneAmbitoTerritorialeEjb.deleteOrgAmbitoTerrit(
			    getForm().getAmbitoTerritoriale().getCd_ambito_territ().parse());
	...
	if (publisher.equals(Application.Publisher.GESTIONE_AMBITO_TERRITORIALE)) {
		...
		getSession().setAttribute("idToUpdate",
			getForm().getAmbitoTerritoriale().getId_to_update().parse());
	    }
	    forwardToPublisher(Application.Publisher.GESTIONE_AMBITO_TERRITORIALE);
	}
    }
	...
	if (getForm().getAmbitoTerritoriale().validate(getMessageBox())) {
	    // Invece di usare il copyToBean, creo dinamicamente il rowBean per colpa di
	    // REGIONE/STATO
	    // DANNATO SLASH
	    String cdAmbitoTerrit;
	    String tiAmbitoTerrit;
	    OrgAmbitoTerritRowBean orgAmbitoTerritRowBean = new OrgAmbitoTerritRowBean();
	    if ((cdAmbitoTerrit = getForm().getAmbitoTerritoriale().getCd_ambito_territ()
		    .parse()) == null) {
		getMessageBox().addError("Inserire nome ambito territoriale</br>");
	    } else {
		orgAmbitoTerritRowBean.setCdAmbitoTerrit(cdAmbitoTerrit);
	    }
	    if ((tiAmbitoTerrit = getForm().getAmbitoTerritoriale().getTi_ambito_territ()
		    .parse()) == null) {
		getMessageBox().addError("Inserire tipo ambito territoriale</br>");
	    } else {
		if ((!tiAmbitoTerrit
			.equals(WebConstants.TipoAmbitoTerritoriale.REGIONE_STATO.name()))
			&& (getForm().getAmbitoTerritoriale().getId_ambito_territ_padre()
				.parse() == null)) {
		    getMessageBox().addError("Inserire nodo padre</br>");
		} else if (tiAmbitoTerrit
			.equals(WebConstants.TipoAmbitoTerritoriale.REGIONE_STATO.name())) {
		...
		if (getMessageBox().isEmpty()) {
			...
			gestioneAmbitoTerritorialeEjb
				.insertOrgAmbitoTerritoriale(orgAmbitoTerritRowBean);
		    } else if (getForm().getAmbitoTerritoriale().getStatus()
			    .equals(Status.update)) {
			BigDecimal idToUpdate = (BigDecimal) getSession()
				.getAttribute("idToUpdate");
			gestioneAmbitoTerritorialeEjb.updateOrgAmbitoTerritoriale(idToUpdate,
				orgAmbitoTerritRowBean);
		    }
		    getForm().getAmbitoTerritoriale().setViewMode();
		    getForm().getAmbitoTerritoriale().clear();
		    OrgAmbitoTerritTableBean table = gestioneAmbitoTerritorialeEjb
	...
	if (tipo != null) {
		...
		tableBean = gestioneAmbitoTerritorialeEjb

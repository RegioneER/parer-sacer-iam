...
public class AssociazioneUtenteAction extends ActionBase {
	...
	if (returnurl == null || hmac == null || salt == null || codiceFiscale == null) {
	    ...
	    cf = new String(EncryptionUtil.aesDecrypt(Base64.decodeBase64(codiceFiscale),
		    EncryptionUtil.Aes.BIT_256), StandardCharsets.UTF_8);
	    String calc = ((userid != null && userid.length() > 0) ? (userid + ":") : (""))
		    + returnUrlDecoded + ":" + cf + ":" + salt;
	...
	if (!getMessageBox().hasError()) {
		...
		LogParam param = SpagoliteLogUtil.getLogParam(
			paramHelper.getParamApplicApplicationName(), userid, "/associazioneUtente",
			"button/AssociazioneUtenteForm#Bottoni/associa");
		...
		if (tentativi <= 0) {
		    ...
		    getMessageBox().setViewMode(MessageBox.ViewMode.plain);
		    // Si tratta di un utente disattivato ma presente in archivio, quindi lo segnalo
		    // all'utente
		    // MEV#26484 - Modificare la gestione dell'accesso SPID quando l'utente non è
		    // attivo su SIAM
		    if (e instanceof ParerWarningException) {
			getMessageBox()
				.addInfo(((ParerWarningException) e).getDescription() + "<BR/>");
		    }
		    if (tentativi > 1) {
			getMessageBox().addInfo("Restano ancora " + tentativi
				+ " tentativi di associazione con utenza Parer");
		    } else {
			getMessageBox().addInfo("Resta ancora " + tentativi
				+ " tentativo di associazione con utenza Parer");
	...
	if (returnurl == null || hmac == null || salt == null || f == null) {
	    ...
	    String cfDecoded = new String(
		    EncryptionUtil.aesDecrypt(Base64.decodeBase64(f), EncryptionUtil.Aes.BIT_256),
		    StandardCharsets.UTF_8);
		...
		if (tentativi != null && Integer.parseInt(tentativi.toString()) <= 0) {
		    ...
		    String c = getRequest().getParameter("c");
		    String n = getRequest().getParameter("n");
		    String cognome = new String(EncryptionUtil.aesDecrypt(Base64.decodeBase64(c),
			    EncryptionUtil.Aes.BIT_256), StandardCharsets.UTF_8);
		    String nome = new String(EncryptionUtil.aesDecrypt(Base64.decodeBase64(n),
			    EncryptionUtil.Aes.BIT_256), StandardCharsets.UTF_8);
		    String saluto = MessageUtil.getSalutoPerOrario();

...
public class GestioneNoteRilascioAction extends GestioneNoteRilascioAbstractAction {
    ...
    public void insertDettaglio() throws EMFError {
        ...
        if (lista.equals(getForm().getListaNoteRilascio().getName())) {
            ...
            // e azzero l'attributo in sessione usato per i controlli sulla visualizzazione della lista
        ...
        if (lista != null) {
            ...
            if (lista.equals(getForm().getListaNoteRilascio().getName())
                ...
                BigDecimal idNotaRilascio = ((AplNotaRilascioTableBean) getForm().getListaNoteRilascio().getTable())
                        .getCurrentRow().getIdNotaRilascio();
                AplNotaRilascioRowBean detailRow = gestioneNoteRilascioEjb.getAplNotaRilascioRowBean(idNotaRilascio);
                ...
                getForm().getNoteRilascioPrecedentiList().getTable()
                        .addSortingRule(AplNotaRilascioTableDescriptor.COL_DT_VERSIONE, SortingRule.DESC);
    ...
    public void reloadAfterGoBack(String publisherName) {
        try {
            if (publisherName.equals(Application.Publisher.RICERCA_NOTE_RILASCIO)) {
                ...
                getForm().getListaNoteRilascio().setTable(
                        gestioneNoteRilascioEjb.getAplNoteRilascioTableBean(getForm().getFiltriNoteRilascio()));
    ...
    private void salvaNoteRilascio() throws EMFError {
        ...
        if (noteRilascioDetail.getDt_ini_val().parse() == null) {
            ...
            // getMessageBox().addError("Errore di compilazione form: data inizio validità precedente alla data
            // odierna</br>");
        }
        if (noteRilascioDetail.getDt_fine_val().parse() == null) {
            // campiErrati.add(noteRilascioDetail.getDt_fine_val().getDescription());
        } else if (noteRilascioDetail.getDt_ini_val().parse() != null
                && noteRilascioDetail.getDt_ini_val().parse().after(noteRilascioDetail.getDt_fine_val().parse())) {
            getMessageBox()
                    .addError("Errore di compilazione form: data fine validità precedente a data inizio validità</br>");
        }
        ...
        if (noteRilascioDetail.validate(getMessageBox()) && getMessageBox().isEmpty()) {
            try {
                ...
                if (getForm().getDettaglioNoteRilascio().getStatus().equals(Status.insert)) {
                    long idNotaRilascio = gestioneNoteRilascioEjb.insertAplNoteRilascio(noteRilascioRowBean);
                    ...
                    getMessageBox().addMessage(new Message(MessageLevel.INF, "Nota di rilascio inserita con successo"));
                } else if (getForm().getDettaglioNoteRilascio().getStatus().equals(Status.update)) {
                    BigDecimal idNotaRilascio = ((AplNotaRilascioRowBean) getForm().getListaNoteRilascio().getTable()
                            .getCurrentRow()).getIdNotaRilascio();
                    gestioneNoteRilascioEjb.updateAplNoteRilascio(idNotaRilascio, noteRilascioRowBean);
                    // getForm().getListaNoteRilascio().getTable().setCurrentRowIndex(getForm().getListaNoteRilascio().getTable().getCurrentRowIndex());
                    getMessageBox()
                            .addMessage(new Message(MessageLevel.INF, "Nota di rilascio modificata con successo"));
                }
                getForm().getDettaglioNoteRilascio().setViewMode();
    ...
    private void redirectToNoteRilascioPage() throws EMFError {
        ...
        redirectToPage(Application.Actions.GESTIONE_NOTE_RILASCIO, form, form.getListaNoteRilascio().getName(),
    ...
    public void deleteListaNoteRilascio() throws EMFError {
        getMessageBox().clear();
        AplNotaRilascioRowBean noteRilascioRowBean = (AplNotaRilascioRowBean) getForm().getListaNoteRilascio()
        ...
        try {
            ...
            getMessageBox().addMessage(
                    new Message(MessageLevel.INF, "Cancellazione nota di rilascio effettuata con successo"));
            getMessageBox().setViewMode(ViewMode.plain);
        } catch (IncoherenceException ex) {
            logger.error("Errore nella cancellazione della nota di rilascio: " + ex.getMessage(), ex);

...
public class AmministrazioneTipiServizioAction extends AmministrazioneTipiServizioAbstractAction {
    ...
    private static final Logger logger = LoggerFactory
	    .getLogger(AmministrazioneTipiServizioAction.class);
	...
	try {
		...
		if (getTableName().equals(getForm().getListaTipiServizio().getName())) {
		    ...
		    OrgTipoServizioRowBean currentRow = (OrgTipoServizioRowBean) getForm()
			    .getListaTipiServizio().getTable().getCurrentRow();
	...
	try {
		...
		String cdTipoServizio = getForm().getFiltriTipiServizio().getCd_tipo_servizio()
			.parse();
		String tiClasseTipoServizio = getForm().getFiltriTipiServizio()
			.getTi_classe_tipo_servizio().parse();
		String tipoFatturazione = getForm().getFiltriTipiServizio().getTipo_fatturazione()
			.parse();
		OrgTipoServizioTableBean table = entiConvenzionatiEjb.getOrgTipoServizioTableBean(
			cdTipoServizio, tiClasseTipoServizio, tipoFatturazione);
	...
	getForm().getFiltriTipiServizio().getTipo_fatturazione()
		.setDecodeMap(ComboGetter.getMappaTipoFatturazione());
	...
	getForm().getTipoServizioDetail().getTi_classe_tipo_servizio()
		.setDecodeMap(ComboGetter.getMappaTiClasseTipoServizio());
	getForm().getTipoServizioDetail().getTipo_fatturazione()
		.setDecodeMap(ComboGetter.getMappaTipoFatturazione());
	getForm().getTipoServizioDetail().getFl_tariffa_accordo()
		.setDecodeMap(ComboGetter.getMappaGenericFlagSiNo());
	getForm().getTipoServizioDetail().getFl_tariffa_annualita()
		.setDecodeMap(ComboGetter.getMappaGenericFlagSiNo());
    }

    private void loadDettaglioTipoServizio(BigDecimal idTipoServizio)
	    throws ParerUserError, EMFError {
	OrgTipoServizioRowBean detailRow = entiConvenzionatiEjb
	...
	getForm().getListaTariffeTipoServizio().setTable(
		entiConvenzionatiEjb.getOrgTariffaByTipoServizioTableBean(idTipoServizio));
	getForm().getListaTariffeTipoServizio().getTable().setPageSize(10);
	//
	getForm().getListaTariffeAccordiTipoServizio().setTable(
		entiConvenzionatiEjb.getOrgTariffaAccordoByTipoServizioTableBean(idTipoServizio));
	getForm().getListaTariffeTipoServizio().getTable().setPageSize(10);
	getForm().getListaTariffeTipoServizio().getTable().first();
	//
	getForm().getListaTariffeAnnualitaTipoServizio().setTable(
		entiConvenzionatiEjb.getOrgTariffaAnnualitaByTipoServizioTableBean(idTipoServizio));
	getForm().getListaTariffeTipoServizio().getTable().setPageSize(10);
	getForm().getListaTariffeTipoServizio().getTable().first();
	...
	if (getLastPublisher().equals(Application.Publisher.DETTAGLIO_TIPO_SERVIZIO)) {
	    if (!idTipoServizio
		    .equals(getForm().getTipoServizioDetail().getId_tipo_servizio().parse())) {
		getMessageBox()
			.addError("Eccezione imprevista nell'eliminazione del tipo servizio");
	    }
	}
	...
	if (!getMessageBox().hasError() && idTipoServizio != null) {
		...
		if (entiConvenzionatiEjb
			.checkEsistenzaServiziErogatiPerTipoServizio(idTipoServizio)) {
		...
		if (!getMessageBox().hasError()) {
		    ...
		    if (param.getNomePagina()
			    .equalsIgnoreCase(Application.Publisher.DETTAGLIO_TIPO_SERVIZIO)) {
		...
		getMessageBox().addError(
			"Il tipo servizio non pu\u00F2 essere eliminato: " + ex.getDescription());
	    }
	}
	if (!getMessageBox().hasError()
		&& getLastPublisher().equals(Application.Publisher.DETTAGLIO_TIPO_SERVIZIO)) {
	    goBack();
	} else {
	    forwardToPublisher(getLastPublisher());
	}
	...
	if (getForm().getTipoServizioDetail().postAndValidate(getRequest(), getMessageBox())) {
	    try {
		String cdTipoServizio = getForm().getTipoServizioDetail().getCd_tipo_servizio()
			.parse();
		String dsTipoServizio = getForm().getTipoServizioDetail().getDs_tipo_servizio()
			.parse();
		String tiClasseTipoServizio = getForm().getTipoServizioDetail()
			.getTi_classe_tipo_servizio().parse();
		String tipoFatturazione = getForm().getTipoServizioDetail().getTipo_fatturazione()
			.parse();
		String flTariffaAccordo = getForm().getTipoServizioDetail().getFl_tariffa_accordo()
			.parse();
		String flTariffaAnnualita = getForm().getTipoServizioDetail()
			.getFl_tariffa_annualita().parse();

		// // Controllo che il campo Tempistiche di fatturazione sia nella forma corretta
		// gg/mm
		...
		// Controllo che il tipo servizio sia imposto o su accordo o su annualità ma non
		// entrambi
		if (flTariffaAccordo != null && flTariffaAccordo.equals("1")
			&& flTariffaAnnualita != null && flTariffaAnnualita.equals("1")) {
		    getMessageBox().addError(
		...
		if (!getMessageBox().hasError()) {
			...
			Long idTipoServizio = entiConvenzionatiEjb.saveTipoServizio(param,
				cdTipoServizio, dsTipoServizio, tiClasseTipoServizio,
				tipoFatturazione, flTariffaAccordo, flTariffaAnnualita);
			if (idTipoServizio != null) {
			    getForm().getTipoServizioDetail().getId_tipo_servizio()
				    .setValue(idTipoServizio.toString());
			}
			...
			BigDecimal idTipoServizio = getForm().getTipoServizioDetail()
				.getId_tipo_servizio().parse();
			entiConvenzionatiEjb.saveTipoAccordo(param, idTipoServizio, cdTipoServizio,
				dsTipoServizio, tiClasseTipoServizio, tipoFatturazione,
				flTariffaAccordo, flTariffaAnnualita);
		    }
		    BigDecimal idTipoServizio = getForm().getTipoServizioDetail()
			    .getId_tipo_servizio().parse();
		    if (idTipoServizio != null) {
	...
	form.getListaTariffe().setFilterValidRecords(
		getForm().getListaTariffeTipoServizio().isFilterValidRecords());
	form.getListaTariffe().setUserOperations(true, false, false, false);
	getSession().setAttribute("isFromTipoServizio", true);
	redirectToPage(Application.Actions.AMMINISTRAZIONE_TARIFFARI, form,
		form.getListaTariffe().getName(),
	...
	form.getOggettoDetail().getNm_tipo_oggetto()
		.setValue(SacerLogConstants.TIPO_OGGETTO_TIPO_SERVIZIO);

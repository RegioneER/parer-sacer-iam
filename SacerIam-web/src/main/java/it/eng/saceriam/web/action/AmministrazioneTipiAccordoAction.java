...
public class AmministrazioneTipiAccordoAction extends AmministrazioneTipiAccordoAbstractAction {
	...
	try {
		...
		if (getTableName().equals(getForm().getListaTipiAccordo().getName())) {
		    ...
		    OrgTipoAccordoRowBean currentRow = (OrgTipoAccordoRowBean) getForm()
			    .getListaTipiAccordo().getTable().getCurrentRow();
	...
	form.getListaTariffario()
		.setFilterValidRecords(getForm().getListaTariffario().isFilterValidRecords());
	redirectToPage(Application.Actions.AMMINISTRAZIONE_TARIFFARI, form,
		form.getListaTariffario().getName(), getForm().getListaTariffario().getTable(),
		getNavigationEvent());
	...
	try {
		...
		String cdTipoAccordo = getForm().getFiltriTipiAccordi().getCd_tipo_accordo()
			.parse();
		String dsTipoAccordo = getForm().getFiltriTipiAccordi().getDs_tipo_accordo()
			.parse();
		...
		OrgTipoAccordoTableBean table = entiConvenzionatiEjb.getOrgTipoAccordoTableBean(
			cdTipoAccordo, dsTipoAccordo, flPagamento, isAttivo);
		...
		BigDecimal idTipoAccordo = getForm().getTipoAccordoDetail().getId_tipo_accordo()
			.parse();
	...
	getForm().getFiltriTipiAccordi().getFl_pagamento()
		.setDecodeMap(ComboGetter.getMappaGenericFlagSiNo());
	getForm().getFiltriTipiAccordi().getIs_attivo()
		.setDecodeMap(ComboGetter.getMappaGenericFlagSiNo());
	...
	getForm().getTipoAccordoDetail().getFl_pagamento()
		.setDecodeMap(ComboGetter.getMappaGenericFlagSiNo());
	getForm().getTipoAccordoDetail().getCd_algo_tariffario()
		.setDecodeMap(ComboGetter.getMappaCdAlgoTariffario());
	getForm().getTipoAccordoDetail().getCd_algo_tariffario().setValue("NO_CLASSE_ENTE");
    }

    private void loadDettaglioTipoAccordo(BigDecimal idTipoAccordo)
	    throws ParerUserError, EMFError {
	OrgTipoAccordoRowBean detailRow = entiConvenzionatiEjb
	...
	getForm().getListaTariffario()
		.setTable(entiConvenzionatiEjb.getOrgTariffarioTableBean(idTipoAccordo));
	...
	if (getLastPublisher().equals(Application.Publisher.DETTAGLIO_TIPO_ACCORDO)) {
	    if (!idTipoAccordo
		    .equals(getForm().getTipoAccordoDetail().getId_tipo_accordo().parse())) {
	...
	if (!getMessageBox().hasError() && idTipoAccordo != null) {
		...
		getMessageBox().addError(
			"Il tipo accordo non pu\u00F2 essere eliminato: " + ex.getDescription());
	    }
	}
	if (!getMessageBox().hasError()
		&& getLastPublisher().equals(Application.Publisher.DETTAGLIO_TIPO_ACCORDO)) {
	    goBack();
	} else {
	    forwardToPublisher(getLastPublisher());
	}
	...
	if (getForm().getTipoAccordoDetail().postAndValidate(getRequest(), getMessageBox())) {
	    try {
		String cdTipoAccordo = getForm().getTipoAccordoDetail().getCd_tipo_accordo()
			.parse();
		String dsTipoAccordo = getForm().getTipoAccordoDetail().getDs_tipo_accordo()
			.parse();
		String flPagamento = getForm().getTipoAccordoDetail().getFl_pagamento().parse();
		String cdAlgoTariffario = getForm().getTipoAccordoDetail().getCd_algo_tariffario()
			.parse();
		...
		if (!getMessageBox().hasError()) {
			...
			Long idTipoAccordo = entiConvenzionatiEjb.saveTipoAccordo(param,
				cdTipoAccordo, dsTipoAccordo, flPagamento, cdAlgoTariffario,
				dtIniVal, dtFineVal);
			if (idTipoAccordo != null) {
			    getForm().getTipoAccordoDetail().getId_tipo_accordo()
				    .setValue(idTipoAccordo.toString());
			}
			...
			BigDecimal idTipoAccordo = getForm().getTipoAccordoDetail()
				.getId_tipo_accordo().parse();
			entiConvenzionatiEjb.saveTipoAccordo(param, idTipoAccordo, cdTipoAccordo,
				dsTipoAccordo, flPagamento, cdAlgoTariffario, dtIniVal, dtFineVal);
		    }
		    BigDecimal idTipoAccordo = getForm().getTipoAccordoDetail().getId_tipo_accordo()
			    .parse();
		    if (idTipoAccordo != null) {
	...
	form.getOggettoDetail().getNm_tipo_oggetto()
		.setValue(SacerLogConstants.TIPO_OGGETTO_TIPO_ACCORDO);

...
public class GestioneNewsAction extends GestioneNewsAbstractAction {
    ...
    public void loadDettaglio() throws EMFError {
        ...
        if (lista != null) {
            ...
            if (lista.equals(getForm().getListaNews().getName()) && !getNavigationEvent().equals(NE_DETTAGLIO_INSERT)) {
                ...
                AplNewsRowBean newsRowBean = (AplNewsRowBean) getForm().getListaNews().getTable().getCurrentRow();
    ...
    public void undoDettaglio() throws EMFError {
        ...
        if (publisher.equals(Application.Publisher.DETTAGLIO_NEWS) && getForm().getDettaglioNews().getStatus() != null
    ...
    /**
     * Esegue la post dei campi: utilizzato quando forzo la post a seguito dell'eliminazione di una riga dalla lista
     * applicazione, operazione che, essendo un link (lavora in get), non effettuerebbe la post http
     *
     * @throws EMFError
     *             errore generico
     */
    ...
    public void reloadAfterGoBack(String publisherName) {
        try {
            ...
            getForm().getListaNews().setTable(gestioneNewsEjb.getAplNewsTableBean(getForm().getFiltriNews()));
    ...
    private void salvaNews() throws EMFError {
        ...
        AplApplicTableBean applicTableBean = (AplApplicTableBean) getForm().getApplicazioniList().getTable();
        ...
        getForm().getDettaglioNews().getFl_pubblic_homepage()
                .setChecked(getRequest().getParameter("Fl_pubblic_homepage") != null ? true : false);
        ...
        if (newsDetail.getDt_ini_pubblic().parse() == null) {
            ...
            getMessageBox()
                    .addError("Errore di compilazione form: data pubblicazione precedente alla data odierna</br>");
        }
        if (newsDetail.getDt_fin_pubblic().parse() == null) {
            campiErrati.add(newsDetail.getDt_fin_pubblic().getDescription());
        } else if (newsDetail.getDt_ini_pubblic().parse() != null
                && newsDetail.getDt_ini_pubblic().parse().after(newsDetail.getDt_fin_pubblic().parse())) {
            getMessageBox()
        ...
        if (getRequest().getParameter("Fl_pubblic_homepage") == null
            ...
            getMessageBox().addError("Errore di compilazione form: selezionare almeno un metodo di pubblicazione</br>");
        }
        ...
        if (getForm().getDettaglioNews().getFl_pubblic_homepage().isChecked() && applicTableBean.isEmpty()) {
            getMessageBox()
                    .addError("Attenzione: flag mostra in home page settato ma nessuna applicazione selezionata!</br>");
        }
        ...
        if (newsDetail.validate(getMessageBox()) && getMessageBox().isEmpty()) {
            try {
                ...
                if (getForm().getDettaglioNews().getStatus().equals(Status.insert)) {
                    ...
                    getMessageBox().addMessage(new Message(MessageLevel.INF, "News inserita con successo"));
                } else if (getForm().getDettaglioNews().getStatus().equals(Status.update)) {
                    BigDecimal idNews = ((AplNewsRowBean) getForm().getListaNews().getTable().getCurrentRow())
                            .getIdNews();
                    gestioneNewsEjb.updateAplNews(idNews, newsRowBean, applicTableBean);
                    // getForm().getListaNews().getTable().setCurrentRowIndex(getForm().getListaNews().getTable().getCurrentRowIndex());
                    getMessageBox().addMessage(new Message(MessageLevel.INF, "News modificata con successo"));
                }
                getForm().getDettaglioNews().setViewMode();
                getForm().getApplicazioniFields().setViewMode();
                ...
                // Ricarico la lista (subito perch̬ se ad esempio scorro i record del dettaglio con le frecce,
    ...
    public void deleteListaNews() throws EMFError {
        ...
        try {
            ...
            getMessageBox().addMessage(new Message(MessageLevel.INF, "Cancellazione news effettuata con successo"));
            getMessageBox().setViewMode(ViewMode.plain);
    ...
    public void deleteApplicazioniList() throws EMFError {
        AplApplicRowBean applicRowBean = (AplApplicRowBean) getForm().getApplicazioniList().getTable().getCurrentRow();

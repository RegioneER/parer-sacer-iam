 ...
 * @author Gilioli_P
 */
public class AmministrazioneSistemiVersantiAction
	extends AmministrazioneSistemiVersantiAbstractAction {

    private static final Logger log = LoggerFactory
	    .getLogger(AmministrazioneSistemiVersantiAction.class);
	...
	try {
	    /*
	     * Se l'evento per il quale passo in loadDettaglio NON è inserimento e cancellazione
	     * (per i quali non ha senso caricare dei dati...)
	     */
		...
		if (getTableName().equals(getForm().getListaSistemiVersanti().getName())) {
		    ...
		    initSistemaVersante();
		    BigDecimal idSistemaVersante = ((AplVRicSistemaVersanteRowBean) getForm()
			    .getListaSistemiVersanti().getTable().getCurrentRow())
	...
	OrgVRicEnteNonConvenzTableBean entiNonConvenzionati = entiConvenzionatiEjb
		.getOrgVRicEnteNonConvenzAbilListTableBean(
			BigDecimal.valueOf(getUser().getIdUtente()), listaTipiEntiAbilitati);
	getForm().getSistemaVersanteDetail().getId_ente_siam().setDecodeMap(DecodeMap.Factory
		.newInstance(entiNonConvenzionati, "id_ente_siam", "nm_ente_siam"));
    ...
    private void loadDettaglioSistemaVersante(BigDecimal idSistemaVersante)
	...
	getForm().getStruttureVersantiList().setTable(
		sistemiVersantiEjb.getAplVLisOrganizUsoSisVersTableBean(idSistemaVersante));
	...
	getForm().getReferentiDittaProduttriceList().setTable(
		sistemiVersantiEjb.getAplSistemaVeranteUserRefTableBean(idSistemaVersante));
	...
	try {
		...
		getForm().getListaSistemiVersanti().getTable()
			.add(new AplVRicSistemaVersanteRowBean());
		...
		final BigDecimal idSistemaVersante = getForm().getSistemaVersanteDetail()
			.getId_sistema_versante().parse();
		BigDecimal idUserIamCor = new BigDecimal(getUser().getIdUtente());
		// Inizializzo la form per andare alla ricerca utenti amministratori inserendo i
		// valori trovati
		AmministrazioneUtentiForm amministrazioneUtentiForm = new AmministrazioneUtentiForm();
		amministrazioneUtentiForm.getUtentiArchivistiPerSistemaVersante()
			.getId_sistema_versante().setValue(idSistemaVersante.toPlainString());
		...
		amministrazioneUtentiForm.getFiltriUtenti().getId_ente_convenz_appart()
			.setDecodeMap(DecodeMap.Factory.newInstance(enteSiamTableBean,
		...
		// Recupero gli ambienti enti di appartenzenza di tipo conservatore e amministratore
		// e i rispettivi
		...
		OrgAmbienteEnteConvenzTableBean ambienteEnteConvenzTableBean = entiConvenzionatiEjb
			.getOrgAmbientiEnteConvenzCollegUserAbilTableBeanByTiEnteConvenz(
				idUserIamCor, ConstOrgEnteSiam.TiEnteConvenz.CONSERVATORE,
				ConstOrgEnteSiam.TiEnteConvenz.AMMINISTRATORE);
		...
		redirectToAction(Application.Actions.AMMINISTRAZIONE_UTENTI,
		    ...
		    .equals(getForm().getReferentiDittaProduttriceList().getName())) {
		final BigDecimal idSistemaVersante = getForm().getSistemaVersanteDetail()
			.getId_sistema_versante().parse();
		// Inizializzo la form per andare alla ricerca utenti inserendo i valori trovati
		AmministrazioneUtentiForm amministrazioneUtentiForm = new AmministrazioneUtentiForm();
		amministrazioneUtentiForm.getReferentiDittaProduttricePerSistemaVersante()
			.getId_sistema_versante().setValue(idSistemaVersante.toPlainString());
		redirectToAction(Application.Actions.AMMINISTRAZIONE_UTENTI,
			"?operation=ricercaUtentiPage", amministrazioneUtentiForm);
	    }
	...
	if (getForm().getSistemaVersanteDetail().postAndValidate(getRequest(), getMessageBox())) {
	    // Recupero i campi
	    String denominazione = getForm().getSistemaVersanteDetail().getNm_sistema_versante()
		    .parse();
	    String descrizione = getForm().getSistemaVersanteDetail().getDs_sistema_versante()
		    .parse();
	    ...
	    String flIntegrazione = getForm().getSistemaVersanteDetail().getFl_integrazione()
		    .parse();
	    String flAssociaPersonaFisica = getForm().getSistemaVersanteDetail()
		    .getFl_associa_persona_fisica().parse();
		...
		try {
		    ...
		    if (getForm().getListaSistemiVersanti().getStatus()
			    .equals(BaseElements.Status.insert)) {
			// Inserisco il record
			Long idSistemaVersante = sistemiVersantiEjb.insertSistemaVersante(
				denominazione, descrizione, versione, idEnteSiam, dsEmail, flPec,
				flIntegrazione, flAssociaPersonaFisica, dtIniVal, dtFineVal,
				dsNote);
		    ...
		    else if (getForm().getListaSistemiVersanti().getStatus()
			    .equals(BaseElements.Status.update)) {
			BigDecimal idSistemaVersante = getForm().getSistemaVersanteDetail()
				.getId_sistema_versante().parse();
			sistemiVersantiEjb.updateSistemaVersante(idSistemaVersante, denominazione,
				descrizione, versione, idEnteSiam, dsEmail, flPec, flIntegrazione,
				flAssociaPersonaFisica, dtIniVal, dtFineVal, dsNote);
	...
	try {
		...
		BaseRowInterface currentRow = getForm().getListaSistemiVersanti().getTable()
			.getCurrentRow();
	...
	try {
	    ...
	    UsrVTreeOrganizIamTableBean tabellaOrganiz = sistemiVersantiEjb
		    .getOrganizUltimoLivelloSacer();
	    DecodeMap mappaOrganizApplic = new DecodeMap();
	    mappaOrganizApplic.populatedMap(tabellaOrganiz, "id_organiz_applic",
		    "dl_composito_organiz");
	    getForm().getFiltriSistemiVersanti().getId_organiz_applic()
		    .setDecodeMap(mappaOrganizApplic);
	    // Carico i nomi degli archivisti di riferimento dei sistemi versanti già presenti sul
	    // sistema
	    getForm().getFiltriSistemiVersanti().getArchivista()
		    .setDecodeMap(DecodeMap.Factory.newInstance(
			    entiConvenzionatiEjb.getUtentiArchivisti(), "id_user_iam",
			    "nm_userid"));
	    //
	...
	BigDecimal idSistemaVersante = ((AplVRicSistemaVersanteRowBean) getForm()
		.getListaSistemiVersanti().getTable().getCurrentRow()).getIdSistemaVersante();
	...
	AplSistemaVersArkRifRowBean currentRow = (AplSistemaVersArkRifRowBean) getForm()
		.getUtentiArchivistiList().getTable().getCurrentRow();
	...
	BigDecimal idSistemaVersArkRif = (BigDecimal) getSession()
		.getAttribute("idSistemaVersArkRif");
	...
	if (!getMessageBox().hasError() && idSistemaVersArkRif != null) {
		...
		if (!getMessageBox().hasError()) {
		    ...
		    getMessageBox().addInfo(
			    "Utente archivista eliminato con successo dal sistema versante");
		    getMessageBox().setViewMode(MessageBox.ViewMode.plain);
		}
	    } catch (ParerUserError ex) {
	...
	BigDecimal idSistemaVersanteUserRef = (BigDecimal) getSession()
		.getAttribute("idSistemaVersanteUserRef");
	int riga = (Integer) getSession().getAttribute("riga");
	if (!getMessageBox().hasError() && idSistemaVersanteUserRef != null) {
		...
		if (!getMessageBox().hasError()) {
		    ...
		    getMessageBox().addInfo(
			    "Referente ditta produttice eliminato con successo dal sistema versante");
		    getMessageBox().setViewMode(MessageBox.ViewMode.plain);
		}
	    } catch (ParerUserError ex) {
		getMessageBox().addError(
			"Il referente ditta produttrice del sistema versante non pu\u00F2 essere eliminato: "
				+ ex.getDescription());
	    }
	} else {
	    getMessageBox().addError(
	...
	try {
		...
		OrgEnteSiamRowBean enteSiam = entiConvenzionatiEjb
			.getOrgEnteConvenzRowBean(idEnteSiam);
		getForm().getSistemaVersanteDetail().getDs_via_sede_legale()
			.setValue(enteSiam.getDsViaSedeLegale());
		getForm().getSistemaVersanteDetail().getDs_citta_sede_legale()
			.setValue(enteSiam.getDsCittaSedeLegale());
		getForm().getSistemaVersanteDetail().getCd_cap_sede_legale()
			.setValue(enteSiam.getCdCapSedeLegale());
	    } else {

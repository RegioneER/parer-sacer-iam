...
public class GestioneHelpOnLineAction extends GestioneHelpOnLineAbstractAction {
    ...
    public void ricercaHelpOnLine() throws EMFError {
        ...
        if (getForm().getFiltriHelpOnLine().postAndValidate(getRequest(), getMessageBox())) {
            ...
            if (getForm().getFiltriHelpOnLine().getDt_riferimento() != null) {
                String dtRiferimento = getForm().getFiltriHelpOnLine().getDt_riferimento().getValue();
                try {
                    dataRiferimento = DateUtils.parseDate(dtRiferimento, new String[] { DateUtil.DATE_FORMAT_SLASH });
            ...
            AplVVisHelpOnLineTableBean tb = recuperoHelpEjb.getAplVVisHelpOnLineTableBean(
                    ...
                    (idPaginaWeb == null || idPaginaWeb.trim().equals("")) ? null : new BigDecimal(idPaginaWeb),
                    (idEntryMenu == null || idEntryMenu.trim().equals("")) ? null : new BigDecimal(idEntryMenu));
        ...
        if (isMultipart) {
            if (getLastPublisher().equals(Application.Publisher.DETTAGLIO_HELP_ON_LINE)) {
                try {
                     ...
                     */
                    if (paramReturn != null && paramReturn.length > 0 && paramReturn[0].equals("preview")) {

                        // Altrimenti lo prende dalla multipart
                        byte[] fileBlob = getForm().getDettaglioHelpOnLine().getDs_file_zip_help_online()
                                .getFileBytes();
                        // Nel caso si chiede la preview dell'help in modificama non si fornisce lo zip
                        // l'help viene preso da db
                        if (fileBlob == null && getForm().getDettaglioHelpOnLine().getStatus().equals(Status.update)) {
                            preview();
                            ripristinaValoriOriginariInUpdateOPreview();
                        } else {
                            ripristinaValoriOriginariInUpdateOPreview();
                            ...
                            if (proc.processa()) {
                                if (proc.getHelpHtml() == null) {
                                    // caso in cui l'utente clicca su preview senza fornire il file da caricare
                                    getMessageBox().addError(MSG_ERROR_DOC_ZIP);
                                } else {
                                    ObjectMapper om = new ObjectMapper();
                                    String arr = om.convertValue(proc.getHelpHtml().getBytes(StandardCharsets.UTF_8),
                                            String.class);
                        ...
                        forwardToPublisher(Application.Publisher.DETTAGLIO_HELP_ON_LINE);
                    } else if (paramReturn != null && paramReturn.length > 0 && paramReturn[0].equals("downloadFile")) {
                        ...
                        /*
                         * Se ero in inserimento cancello la riga che avevo inserito nella lista, altrimenti quando
                         * torno indietro la trovo vuota!
                         */
                        if (getForm().getListaHelpOnLine().getStatus().equals(Status.insert)) {
                            getForm().getListaHelpOnLine()
                                    .remove(getForm().getListaHelpOnLine().getTable().getCurrentRowIndex());
                        }
                        getForm().getDettaglioHelpOnLine().reset();
                        goBack();
                        ...
                        if (getForm().getDettaglioHelpOnLine().getStatus().equals(Status.update)) {
                            String tmpIdApplic = "" + getForm().getListaHelpOnLine().getTable().getCurrentRow()
                                    .getBigDecimal("id_applic").longValueExact();
                        ...
                        if (validaDati()) {
                            ...
                            if (proc.processa()) {
                                ...
                                // se si sta salvando la modifica (tasto "save") si va in modifica del record...
                                if (paramReturn.length > 2 && paramReturn[2].equals(ListAction.NE_DETTAGLIO_SAVE)) {
                                    if (getForm().getDettaglioHelpOnLine().getStatus().equals(Status.update)) {
                                        // MODIFICA DETTAGLIO
                                        ...
                                        rigaProcessata = recuperoHelpEjb.modificaHelp(idHelp, proc.getDocFileName(),
                                                dataIni, dataFine, proc.getHelpHtml(), proc.getDocSorgenteHelp());

                                    } else {
                                        // INSERIMENTO DETTAGLIO
                                        rigaProcessata = recuperoHelpEjb.inserisciHelp(longApplic, idTipoHelp,
                                                proc.getDocFileName(), longPagina, longMenu, dataIni, dataFine,
                                ...
                                if (paramReturn.length > 2 && !paramReturn[2].equals(ListAction.NE_DETTAGLIO_DELETE)) {
                                    AplVVisHelpOnLineRowBean bean = (AplVVisHelpOnLineRowBean) getForm()
                                            .getListaHelpOnLine().getTable().getCurrentRow();
                                    // bean.entityToRowBean(rigaInserita);
                                    bean.setIdApplic(new BigDecimal(rigaProcessata.getAplApplic().getIdApplic()));
                                    bean.setDtIniVal(new Timestamp(rigaProcessata.getDtIniVal().getTime()));
                                    bean.setDtFineVal(new Timestamp(rigaProcessata.getDtFineVal().getTime()));
                                    bean.setIdPaginaWeb(
                                            new BigDecimal(rigaProcessata.getAplPaginaWeb().getIdPaginaWeb()));
                                    bean.setDsPaginaWeb(rigaProcessata.getAplPaginaWeb().getDsPaginaWeb());
                                    bean.setDlCompositoEntryMenu(
                                            idMenu == null ? "" : rigaProcessata.getAplEntryMenu().getDsEntryMenu());
                                    bean.setIdEntryMenu(rigaProcessata.getAplEntryMenu() == null ? null
                                            : new BigDecimal(rigaProcessata.getAplEntryMenu().getIdEntryMenu()));
                                    bean.setTiHelpOnLine(rigaProcessata.getTiHelpOnLine());
                                    bean.setIdHelpOnLine(new BigDecimal(rigaProcessata.getIdHelpOnLine()));
                                    bean.setDsFileHelpOnLine(rigaProcessata.getDsFileHelpOnLine());
                                    /*
                                     * Nel caso di modifica ripopola il dettaglio che altrimenti sarebbe vuoto perchè
                                     * alcuni campi sono in readonly.
                                     */
                                    if (getForm().getDettaglioHelpOnLine().getStatus().equals(Status.update)) {
                                        getForm().getDettaglioHelpOnLine().getNm_applic()
                                                .setValue("" + rigaProcessata.getAplApplic().getIdApplic());
                                        getForm().getDettaglioHelpOnLine().getNm_pagina()
                                                .setValue(bean.getIdPaginaWeb().toString());
                                        if (bean.getIdEntryMenu() != null
                                                && !bean.getIdEntryMenu().toString().trim().equals("")) {
                                            getForm().getDettaglioHelpOnLine().getNm_entry_menu()
    ...
    /*
     * Nel caso di messaggi a video che comportano il rimanere nella stessa form in modifica riprende i valori originari
     * dalla lista e filtro della pagina precedente e li mette a video
     */
    ...
    private void processaCampiData() throws EMFError {
        ...
        Date recentDate = recuperoHelpEjb.findMostRecentDtFin(idApplic.longValueExact(), idTipoHelp,
                idPagina == null ? 0 : idPagina.longValueExact(), idMenu == null ? 0 : idMenu.longValueExact());
        /*
         * Se trova un help con data piu' recente (diversa da infinito) mette data di inizio alla data fine che ha
         * trovato + 1 altrimenti mette la data attuale. La data fine e' sempre infinita.
         */
        if (recentDate == null) {
            recentDate = new Date();
        } else {
        ...
        getForm().getDettaglioHelpOnLine().getDt_ini_val().setValue(DateUtil.formatDateWithSlash(recentDate));
        getForm().getDettaglioHelpOnLine().getDt_fine_val().setValue(DateUtil.MAX_DATE_FORMAT_SLASH);
    }
    ...
    /*
     * Viene richiamato sia dal framework quando il dettaglio Ã¨ in modalitÃ  view oppure dal metodo process quando si
     * chiede la preview dell'Help ma si Ã¨ in modalitÃ  modifica
     */
    @Override
    public void preview() throws EMFError {
        ...
        if (getForm().getDettaglioHelpOnLine().getStatus().equals(Status.view)
            ...
            ObjectMapper om = new ObjectMapper();
            String arr = om.convertValue(bean.getBlHelpOnLine().getBytes(StandardCharsets.UTF_8), String.class);
            getRequest().setAttribute("Preview_hidden_field", arr);
        }
        forwardToPublisher(Application.Publisher.DETTAGLIO_HELP_ON_LINE);
    }
    ...
    private boolean validaDati() throws EMFError {
        ...
        if (getForm().getDettaglioHelpOnLine().getStatus().equals(Status.insert)) {
            ...
            if (idTipoHelp.equals(ConstAplVVisHelpOnLine.TiHelpOnLine.HELP_RICERCA_DIPS.name())) {
                if (idMenu == null) {
                    getMessageBox().addError("Per l'help di SacerDips e' necessario inserire un menu'");
                    retCode = false;
                }
            }
        }
    ...
    public void downloadFile() throws EMFError {
        ...
        if (help != null) {
            if (!getMessageBox().hasError()) {
                File tmpFile = new File(System.getProperty("java.io.tmpdir"), help.getDsFileHelpOnLine());
    ...
    public void download() throws EMFError {
        ...
        if (path != null && filename != null) {
            ...
            if (fileToDownload.exists()) {
                /*
                 * Definiamo l'output previsto che sarÃƒÂ  un file in formato zip di cui si occuperÃƒÂ  la servlet per
                 * fare il download
                 */
                OutputStream outUD = getServletOutputStream();
                getResponse().setContentType(
                        StringUtils.isBlank(contentType) ? WebConstants.MIME_TYPE_MICROSOFT_WORD : contentType);

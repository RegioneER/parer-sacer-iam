...
public class GestioneJobAction extends GestioneJobAbstractAction {
    ...
    public void gestioneJobRicercaPage() throws EMFError {
        ...
        // Rimuovo il parametro che mi serve per visualizzare il tasto Indietro (quando provengo da Lista Schedulazioni
        // Job)
    ...
    public void startMassivoGestioneJob() throws EMFError {
        ...
        if (tabella != null) {
            ...
            if (almenoUnoSel) {// listaSelezionati
                ...
                getMessageBox()
                        .addInfo(message + "<br>L'operazione richiesta diventerà effettiva entro il prossimo minuto.");
            } else {
    ...
    private void redirectToSchedulazioniJob(String nmJob) throws EMFError {
        ...
        redirectToPage(Application.Actions.AMMINISTRAZIONE_UTENTI, form, form.getJobSchedulatiList().getName(),
                getForm().getGestioneJobRicercaList().getTable(), getNavigationEvent());
    }

    private AmministrazioneUtentiForm prepareRedirectToSchedulazioniJob(String nmJob) throws EMFError {
    ...
    /**
     * Metodo che fornisce uno stack utilizzato per mantenere le pagine visualizzate durante la "navigazione"
     *

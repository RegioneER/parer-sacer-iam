...
public class GestioneJobAction extends GestioneJobAbstractAction {
	...
	Timestamp dataAttivazioneJob = getActivationDateJob(
		Constants.NomiJob.REPLICA_UTENTI.name());
	StatoJob replicaUtenti = new StatoJob(Constants.NomiJob.REPLICA_UTENTI.name(),
		...
		getForm().getReplicaUtenti().getDt_prossima_attivazione(),
		getForm().getReplicaUtenti().getAttivo(),
	...
	StatoJob scadenzaFatture = new StatoJob(Constants.NomiJob.SCADENZA_FATTURE.name(),
		...
		getForm().getScadenzaFatture().getDt_prossima_attivazione(),
		getForm().getScadenzaFatture().getAttivo(),
		getForm().getScadenzaFatture().getDt_reg_log_job_ini(), dataAttivazioneJob);

	gestisciStatoJob(scadenzaFatture);
	// </editor-fold>

	// <editor-fold defaultstate="collapsed" desc="UI Gestione job per l'inizializzazione dei
	// LOG">
	...
	esegui(Constants.NomiJob.REPLICA_UTENTI.name(), "Replica utenti", null,
	...
	esegui(Constants.NomiJob.SCADENZA_FATTURE.name(), "Scadenza fatture", null,
		OPERAZIONE.ESECUZIONE_SINGOLA);
    }

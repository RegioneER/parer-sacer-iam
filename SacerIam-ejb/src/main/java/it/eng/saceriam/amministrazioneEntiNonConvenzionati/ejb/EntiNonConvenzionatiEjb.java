...
public class EntiNonConvenzionatiEjb {
     ...
     *
     * @param tiAmbitoTerrit tipo ambiente
     *
     * @return il tableBean
     *
     ...
     * dato come parametro
     *
     * @param idAmbitoTerrit id ambito terrotoriale
     *
     * @return il rowBean
     *
     * @throws ParerUserError errore generico
    ...
    public OrgVRicEnteNonConvenzTableBean getOrgVRicEnteNonConvenzTableBean(String nmEnteSiam,
	...
	List<OrgVRicEnteNonConvenz> enteNonConvenzs = helper.retrieveOrgEnteNonConvenzList(
		nmEnteSiam, idUserIamCor, /*
					   * idAmbienteEnteConvenz,
					   */
		flEnteCessato, idArchivista, noArchivista, dataOdierna, tiEnteNonConvenz);
	if (!enteNonConvenzs.isEmpty()) {
	    try {
		orgEnteNonConvenzTableBean = customEntities2EnteNonConvenzTableBean(
			enteNonConvenzs);
	    } catch (ClassNotFoundException | NoSuchMethodException | InstantiationException
		    | IllegalAccessException | IllegalArgumentException
		    | InvocationTargetException e) {
		LOGGER.error("Errore durante il recupero degli enti non convenzionati "
			+ ExceptionUtils.getRootCauseMessage(e), e);
		throw new ParerUserError(
			"Errore durante il recupero della lista di enti non convenzionati");
	    }
	}
     ...
     * @param creazioneBean bean {@link EnteNonConvenzionatoBean}
	...
	try {
	    ...
	    OrgEnteSiam enteConvenzCreazione = helper.getOrgEnteConvenzByTiEnteConvenz(
	    ...
	    sacerLogEjb.log(param.getTransactionLogContext(),
		    ...
		    param.getNomeAzione(), nmTipoOggetto, new BigDecimal(idEnteSiam),
		    param.getNomePagina());
	} catch (Exception ex) {
	    LOGGER.error("Errore imprevisto durante il salvataggio dell'ente non convenzionato : "
		    + ExceptionUtils.getRootCauseMessage(ex), ex);
	    throw new ParerUserError(
		    "Eccezione imprevista durante il salvataggio dell'ente non convenzionato");
	}
	return idEnteSiam;
    }
     ...
     * @param param                  parametri per il logging
     * @param idUserIamCor           id user IAM
     * @param idEnteNonConvenzionato id ente non convenzionato
     * @param creazioneBean          bean {@link EnteNonConvenzionatoBean}
     *
     * @throws ParerUserError errore generico
     */
    @TransactionAttribute(TransactionAttributeType.REQUIRES_NEW)
    public void saveEnteNonConvenzionato(LogParam param, BigDecimal idUserIamCor,
	    BigDecimal idEnteNonConvenzionato, EnteNonConvenzionatoBean creazioneBean)
	    throws ParerUserError {
	...
	try {
	    ...
	    List<UsrUser> utenti = utentiHelper.retrieveUsrUserEnteSiamAppart(
		    idEnteNonConvenzionato, null,
		    Collections.singletonMap("NOT IN",
			    Arrays.asList(ConstUsrStatoUser.TiStatotUser.CESSATO.name())),
		    Collections.singletonMap("IN",
			    Arrays.asList(ApplEnum.TipoUser.PERSONA_FISICA.name())));

	    if (idEnteSiamProdutCorrispSaved != null && !idEnteSiamProdutCorrispSaved
		    .equals(creazioneBean.getId_ente_siam_produt_corrisp())) {
		for (UsrUser utente : utenti) {
		    // 1. RIMUOVE ABIL
		    userHelper.eliminaAbilEntiConv(utente.getIdUserIam(),
			    idEnteSiamProdutCorrispSaved.longValue());
		    // 2. AGGIUNGE ABIL
		    userHelper.aggiungiAbilEnteCorrisp(idUserIamCor.longValue(),
			    utente.getIdUserIam());
		}
	    }
	    ...
	    sacerLogEjb.log(param.getTransactionLogContext(),
		    paramHelper.getParamApplicApplicationName(), param.getNomeUtente(),
		    param.getNomeAzione(), nmTipoOggetto, idEnteNonConvenzionato,
		    param.getNomePagina());
    ...
    private void setDatiEnteNonConvenzionato(OrgEnteSiam ente,
	    EnteNonConvenzionatoBean creazioneBean) {
	...
	ente.setTiEnteNonConvenz(
		ConstOrgEnteSiam.TiEnteNonConvenz.valueOf(creazioneBean.getTi_ente_non_convenz()));
	...
	try {
		...
		row.setBigDecimal("id_ambiente_ente_convenz",
			BigDecimal.valueOf(ente.getOrgEnteSiamByIdEnteProdutCorrisp()
		...
		row.setBigDecimal("id_ente_siam_produt_corrisp", BigDecimal
			.valueOf(ente.getOrgEnteSiamByIdEnteProdutCorrisp().getIdEnteSiam()));
     ...
     *
     * @param idEnteNonConvezDaEscludere id ente non convenzionato da escludere
     *
     * @return il tableBean
     *
     * @throws ParerUserError errore generico
     */
    public OrgEnteSiamTableBean getOrgEnteSiamTableBean(BigDecimal idEnteNonConvezDaEscludere)
	    throws ParerUserError {
	OrgEnteSiamTableBean enteNonConvenzTableBean = new OrgEnteSiamTableBean();
	List<OrgEnteSiam> enteNonConvenzList = helper
		.getOrgEnteNonConvenzList(idEnteNonConvezDaEscludere);
	if (!enteNonConvenzList.isEmpty()) {
	    try {
		enteNonConvenzTableBean = (OrgEnteSiamTableBean) Transform
			.entities2TableBean(enteNonConvenzList);
	    } catch (ClassNotFoundException | NoSuchMethodException | InstantiationException
		    | IllegalAccessException | IllegalArgumentException
		    | InvocationTargetException e) {
		LOGGER.error("Errore durante il recupero degli enti non convenzionati "
			+ ExceptionUtils.getRootCauseMessage(e), e);
		throw new ParerUserError(
			"Errore durante il recupero della lista degli enti non convenzionati");
	    }
	}
     ...
     * @throws ParerUserError errore generico
     */
    public OrgSuptEsternoEnteConvenzTableBean getOrgEntiSupportatiTableBean(
	    BigDecimal idEnteFornitEst) throws ParerUserError {
	...
	if (!suptExtEnteConvenzList.isEmpty()) {
		...
		for (OrgSuptEsternoEnteConvenz suptExtEnteConvenz : suptExtEnteConvenzList) {
		    ...
		    if (suptExtEnteConvenz.getOrgEnteSiamByIdEnteProdut()
			    .getOrgAmbienteEnteConvenz() != null) {
			suptExtEnteConvenzRowBean.setString("nm_ambiente_ente_convenz",
				suptExtEnteConvenz.getOrgEnteSiamByIdEnteProdut()
					.getOrgAmbienteEnteConvenz().getNmAmbienteEnteConvenz());
		    }
		    ...
		    | IllegalAccessException | IllegalArgumentException
		    | InvocationTargetException e) {
		LOGGER.error("Errore durante il recupero degli enti produttori "
			+ ExceptionUtils.getRootCauseMessage(e), e);
		throw new ParerUserError(
			"Errore durante il recupero della lista degli enti produttori");
	    }
	}
    ...
    /**
     * Ritorna il tableBean contenente per l'organo di vigilanza dato in input la lista di enti
     * conservatori
     *
     * @param idEnteOrganoVigil id ente organo vigilanza
     *
     * @return il tableBean della lista
     *
     * @throws ParerUserError errore generico
     */
    public OrgAccordoVigilTableBean getOrgAccordoVigilTableBean(BigDecimal idEnteOrganoVigil)
	    throws ParerUserError {
	...
	if (list != null && !list.isEmpty()) {
		...
		for (OrgAccordoVigil accordoVigil : list) {
		    ...
		    accordoVigilRowBean = (OrgAccordoVigilRowBean) Transform
			    .entity2RowBean(accordoVigil);
     ...
     * che hanno ente non convenzionato di appartenenza = ente corrente
     *
     * @param idEnteNonConvenz id ente non convenzionato
     ...
     * @throws ParerUserError errore generico
     */
    public UsrUserTableBean getUsrUserEnteNoconvenzTableBean(BigDecimal idEnteNonConvenz,
	    List<String> tiStatoUser, List<String> tipoUser) throws ParerUserError {
	UsrUserTableBean table = new UsrUserTableBean();
	List<UsrUser> list = utentiHelper.retrieveUsrUserEnteSiamAppart(idEnteNonConvenz,
		tiStatoUser, tipoUser);
	if (list != null && !list.isEmpty()) {
	    try {
	...
	List<BigDecimal> organizIamList = utentiHelper.retrieveIdOrganizIamUserEnteSiamAppartList(
		idEnteNonConvenz, Arrays.asList(ConstUsrStatoUser.TiStatotUser.CESSATO.name()),
		Arrays.asList(ApplEnum.TipoUser.PERSONA_FISICA.name()));
    ...
    /**
     * Ritorna il tableBean contenente per l'ente non convenzionato dato in input la lista di utenti
     * archivisti
     *
     * @param idEnteNonConvenz id ente non convenzionato
     *
     ...
     * @throws ParerUserError errore generico
     */
    public OrgEnteArkRifTableBean getOrgEnteArkRifTableBean(BigDecimal idEnteNonConvenz)
	    throws ParerUserError {
     ...
     *
     * @param idEnteUserRif id ente/user
     *
     ...
     * @throws ParerUserError errore generico
     */
    public OrgEnteUserRifTableBean getOrgEnteUserRifTableBean(BigDecimal idEnteNonConvenz)
	    throws ParerUserError {
     ...
     * @throws ParerUserError errore generico
     */
    public OrgEnteConvenzOrgTableBean getOrgEnteConvenzOrgTableBean(BigDecimal idEnteFornitEst)
	    throws ParerUserError {
	OrgEnteConvenzOrgTableBean enteConvenzOrgTableBean = new OrgEnteConvenzOrgTableBean();
	List<OrgEnteConvenzOrg> enteConvenzOrgList = helper
		.getOrgEnteConvenzOrgList(idEnteFornitEst);
	if (!enteConvenzOrgList.isEmpty()) {
		...
		for (OrgEnteConvenzOrg enteConvenzOrg : enteConvenzOrgList) {
		    ...
		    enteConvenzOrgRowBean = (OrgEnteConvenzOrgRowBean) Transform
			    .entity2RowBean(enteConvenzOrg);
		    enteConvenzOrgRowBean.setString("nm_vers_ping",
			    enteConvenzOrg.getOrgEnteSiam().getNmEnteSiam());
		    enteConvenzOrgRowBean.setString("nm_organiz",
			    enteConvenzOrg.getUsrOrganizIam().getNmOrganiz());
		    enteConvenzOrgTableBean.add(enteConvenzOrgRowBean);
		}
	    } catch (ClassNotFoundException | NoSuchMethodException | InstantiationException
		    | IllegalAccessException | IllegalArgumentException
		    | InvocationTargetException e) {
		LOGGER.error("Errore durante il recupero dei versatori associati "
			+ ExceptionUtils.getRootCauseMessage(e), e);
		throw new ParerUserError(
			"Errore durante il recupero della lista dei versatori associati");
	    }
	}
    ...
    /**
     * Ritorna il tableBean con tutti i sistemi versanti dipendendi dal dal fornitore esterno
     * passato in input
     *
     * @param idEnteFornitEst id ente fornitore esterno
     *
	...
	List<AplSistemaVersante> sistemaVersanteList = helper
		.getAplSistemaVersanteById(idEnteFornitEst);
	try {
		...
		List<AplSistemaVersArkRif> archivisti = svHelper.getAplSistemaVersArkRifList(
			BigDecimal.valueOf(sistemaVersante.getIdSistemaVersante()));
     ...
     *
     * @param idSuptEstEnteConvenz l'id dell'ente supportato da recuperare su DB
     *
     * @return il rowbean contenente il dettaglio ente supportato
     *
     * @throws ParerUserError errore generico
     */
    public OrgSuptEsternoEnteConvenzRowBean getOrgSuptEsternoEnteConvenzRowBean(
	    BigDecimal idSuptEstEnteConvenz) throws ParerUserError {
	OrgSuptEsternoEnteConvenz enteSupt = helper.findById(OrgSuptEsternoEnteConvenz.class,
	...
	try {
		...
		row.setBigDecimal("id_ambiente_ente_convenz",
			BigDecimal.valueOf(enteSupt.getOrgEnteSiamByIdEnteProdut()
				.getOrgAmbienteEnteConvenz().getIdAmbienteEnteConvenz()));
		row.setString("nm_ambiente_ente_convenz", enteSupt.getOrgEnteSiamByIdEnteProdut()
			.getOrgAmbienteEnteConvenz().getNmAmbienteEnteConvenz());
	    }
	    ...
	    row.setString("nm_ente_convenz",
		    enteSupt.getOrgEnteSiamByIdEnteProdut().getNmEnteSiam());
	} catch (ClassNotFoundException | NoSuchMethodException | InstantiationException
		| IllegalAccessException | IllegalArgumentException
		| InvocationTargetException ex) {
	    String msg = "Errore durante il recupero dell'ente supportato "
		    + ExceptionUtils.getRootCauseMessage(ex);
	    LOGGER.error(msg, ex);
	    throw new ParerUserError(msg);
	}
     ...
     *
     * @param param                parametri per il logging
     * @param idSuptEstEnteConvenz id ente
     * @param tipoEnte             tipo ente
     *
     * @throws ParerUserError errore generico
     */
    @TransactionAttribute(TransactionAttributeType.REQUIRES_NEW)
    public void deleteOrgSuptEsternoEnteConvenz(LogParam param, BigDecimal idSuptEstEnteConvenz,
	    String tipoEnte) throws ParerUserError {
	LOGGER.debug("Eseguo l'eliminazione dell'ente supportato");
	try {
	    OrgSuptEsternoEnteConvenz suptEstEnteConvenz = helper
		    .findById(OrgSuptEsternoEnteConvenz.class, idSuptEstEnteConvenz);
		...
		// Determino gli utenti appartenenzi all'ENTE CONVENZIONATO rimosso dal supporto,
		// perchè poi su di essi
		// dovrò rimuove l'abilitazione all'ente NON convenzionato di tipo fornitore esterno
		// (visto che ora non
		// mi supporta più...)
		...
		for (UsrUser utente : utenti) {
		    userHelper.eliminaAbilEntiConv(utente.getIdUserIam(),
			    idEnteFornitEst.longValue());
		}

		// determino utenti PERSONA_FISICA con stato diverso da cessato
		// e flag “Allinea abilitazioni alle organizzazioni e agli enti supportati in
		// automatico”
		...
		for (UsrUser utente : utentiSup) {
		    /*
		     * Il sistema elimina l’abilitazione all'utente all’ente siam che non è più
		     * supportato dal fornitore
		     */
		    userHelper.eliminaAbilEnteSupest(utente.getIdUserIam(),
			    idEnteConvenzSupt.longValue());
		    /*
	...
	// Determino gli utenti appartenenzi all'ENTE CONVENZIONATO rimosso dal supporto, perchè poi
	// su di essi
	// dovrò rimuove l'abilitazione all'ente NON convenzionato di tipo fornitore esterno (visto
	// che ora non mi
	// supporta più...)
	List<UsrUser> utenti = utentiHelper.retrieveUsrUserEnteSiamAppart(idEnteConvenzSupt,
		Collections.singletonMap("NOT IN",
	...
	if (!utentiHelper
		.retrieveUsrUserEnteSiamAppart(idEnteConvenzSupt,
			Collections.singletonMap("NOT IN",
				Arrays.asList(ConstOrgEnteSiam.TiEnteConvenz.AMMINISTRATORE)),
			Collections.singletonMap("NOT IN",
				Arrays.asList(ConstUsrStatoUser.TiStatotUser.CESSATO.name())),
	...
	if (!utentiHelper
		.retrieveUsrUserEnteSup(idEnteFornitEst,
			Collections.singletonMap("IN",
				Arrays.asList(ConstOrgEnteSiam.TiEnteNonConvenz.FORNITORE_ESTERNO)),
			Collections.singletonMap("NOT IN",
				Arrays.asList(ConstUsrStatoUser.TiStatotUser.CESSATO.name())),
     ...
     *
     * @param idAccordoVigil l'id dell'accordo di vigilanza da recuperare su DB @return, il rowbean
     *                       contenente il dettaglio accordo di vigilanza
     *
     * @return row bean {@link OrgAccordoVigilRowBean}
     *
     * @throws ParerUserError errore generico
     */
    public OrgAccordoVigilRowBean getOrgAccordoVigilRowBean(BigDecimal idAccordoVigil)
	    throws ParerUserError {
	...
	try {
		...
		row.setString("ti_ente_non_convenz", accordoVigil
			.getOrgEnteSiamByIdEnteOrganoVigil().getTiEnteNonConvenz().name());
	    }
	    row.setString("nm_ente_siam",
		    accordoVigil.getOrgEnteSiamByIdEnteOrganoVigil().getNmEnteSiam());
	    row.setTimestamp("dt_ini_val_ente_siam", new Timestamp(
		    accordoVigil.getOrgEnteSiamByIdEnteOrganoVigil().getDtIniVal().getTime()));
	    if (accordoVigil.getOrgEnteSiamByIdEnteOrganoVigil().getDtCessazione() != null) {
		row.setTimestamp("dt_cessazione_ente_siam", new Timestamp(accordoVigil
			.getOrgEnteSiamByIdEnteOrganoVigil().getDtCessazione().getTime()));
	    }
	    if (accordoVigil.getOrgEnteSiamByIdEnteOrganoVigil()
		    .getOrgEnteSiamByIdEnteProdutCorrisp() != null) {
		row.setString("nm_ente_siam_produt_corrisp",
			accordoVigil.getOrgEnteSiamByIdEnteOrganoVigil()
				.getOrgEnteSiamByIdEnteProdutCorrisp().getNmEnteSiam());
		if (accordoVigil.getOrgEnteSiamByIdEnteOrganoVigil()
			.getOrgEnteSiamByIdEnteProdutCorrisp()
			.getOrgAmbienteEnteConvenz() != null) {
     ...
     *
     * @param idAccordoVigil id accordo di vigilanza
     *
     * @return il tablebean
     *
     * @throws ParerUserError errore generico
     */
    public OrgVigilEnteProdutTableBean getOrgVigilEnteProdutTableBean(BigDecimal idAccordoVigil)
	    throws ParerUserError {
	OrgVigilEnteProdutTableBean vigilEnteProdutTableBean = new OrgVigilEnteProdutTableBean();
	List<OrgVigilEnteProdut> vigilEnteProdutList = helper
		.retrieveOrgVigilEnteProdut(idAccordoVigil);
	try {
		...
		for (OrgVigilEnteProdut vigilEnteProdut : vigilEnteProdutList) {
			...
			vigilEnteProdutRowBean.setString("nm_ambiente_ente_convenz",
				vigilEnteProdut.getOrgEnteSiam().getOrgAmbienteEnteConvenz()
					.getNmAmbienteEnteConvenz());
		    }
		    vigilEnteProdutRowBean.setString("nm_ente_siam",
			    vigilEnteProdut.getOrgEnteSiam().getNmEnteSiam());
		    vigilEnteProdutTableBean.add(vigilEnteProdutRowBean);
		}
	    }
	} catch (Exception e) {
	    LOGGER.error("Errore durante il recupero degli enti vigilati per l'accordo avente id "
		    + idAccordoVigil + " " + ExceptionUtils.getRootCauseMessage(e), e);
	    throw new ParerUserError(
		    "Errore durante il recupero degli enti vigilati per l'accordo avente id "
			    + idAccordoVigil);
	}
    ...
    public boolean checkUserAbilEnteConvenz(BigDecimal idUserIam, BigDecimal idEnteSiam)
	    throws ParerUserError {
	...
	try {
	    ...
	    sacerLogEjb.log(param.getTransactionLogContext(),
		    paramHelper.getParamApplicApplicationName(), param.getNomeUtente(),
		    param.getNomeAzione(), nmTipoOggetto,
    ...
    /**
     * Recupera gli ambienti ente a cui l'utente passato in ingresso è abilitato (anche quelli che
     * non sono ancora padri di alcun ente)
     *
	...
	UsrVAbilAmbEnteXenteTableBean abilAmbEnteXEnteTableBean = new UsrVAbilAmbEnteXenteTableBean();
	List<UsrVAbilAmbEnteXente> abilAmbEnteXEnteList = helper
		.retrieveAbilAmbEnteXEnteValidAmbienti(idUserIam);
	if (!abilAmbEnteXEnteList.isEmpty()) {
	    try {
		abilAmbEnteXEnteTableBean = (UsrVAbilAmbEnteXenteTableBean) Transform
			.entities2TableBean(abilAmbEnteXEnteList);
    ...
    @Deprecated
    public OrgVRicEnteNonConvenzTableBean getOrgVRicEnteNonConvenzTableBean(
	...
	if (!ricEnteNonConvenzList.isEmpty()) {
	    try {
		ricEnteNonConvenzTableBean = customEntities2EnteNonConvenzTableBean(
			ricEnteNonConvenzList);
	    } catch (ClassNotFoundException | NoSuchMethodException | InstantiationException
		    | IllegalAccessException | IllegalArgumentException
		    | InvocationTargetException e) {
		LOGGER.error("Errore durante il recupero degli enti non convenzionati "
	...
	OrgVRicEnteNonConvenzTableBean ricEnteNonConvenzTableBean = new OrgVRicEnteNonConvenzTableBean();
	List<OrgVRicEnteNonConvenz> ricEnteNonConvenzList = helper
		.retrieveEntiNonConvenzAbilitati(idUserIam);
	if (!ricEnteNonConvenzList.isEmpty()) {
	    try {
    ...
    private OrgVRicEnteNonConvenzTableBean customEntities2EnteNonConvenzTableBean(
	    List<OrgVRicEnteNonConvenz> ricEnteNonConvenzList)
	    throws ClassNotFoundException, NoSuchMethodException, InstantiationException,
	    IllegalAccessException, IllegalArgumentException, InvocationTargetException {

	OrgVRicEnteNonConvenzTableBean ricEnteNonConvenzTableBean = new OrgVRicEnteNonConvenzTableBean();
	for (OrgVRicEnteNonConvenz ente : ricEnteNonConvenzList) {
	    OrgVRicEnteNonConvenzRowBean row = (OrgVRicEnteNonConvenzRowBean) Transform
		    .entity2RowBean(ente);
     ...
     *
     * @param param         parametri per il logging
     * @param idEnteUserRif id ente/utente
     *
     * @throws ParerUserError errore generico
     */
    @TransactionAttribute(TransactionAttributeType.REQUIRES_NEW)
    ...
    /**
     * Metodo centralizzato per verificare che l'intervallo di validità di un'entità figlia sia
     * coerente con l'intervallo di validità dell'entità padre
     *
     * @param dtIniValPadre  data inizio validata padre
     * @param dtFinValPadre  data fine validata padre
     * @param dtIniValFiglio data inizio validata figlio
     * @param dtFinValFiglio data fine validata padre
    ...
    public void checkInclusioneFiglioVigil(Date dtIniValPadre, Date dtFinValPadre,
	    Date dtIniValFiglio, Date dtFinValFiglio, String nmPadre, String nmFiglio)
	    throws ParerUserError {
	// Se l'intevallo del figlio non è incluso totalmente nell'intervallo del padre
	if (dtIniValPadre.after(dtIniValFiglio) || dtFinValPadre.before(dtFinValFiglio)) {
	    throw new ParerUserError("La validità della vigilanza su " + nmFiglio
    ...
    public void checkEnteSuptPerEnteFornit(BigDecimal idEnteNonConvenz, BigDecimal idEnteConvenz,
	    Date dtIniValSupt, Date dtFinValSupt, BigDecimal idSuptEstEnteConvenzDaEscludere)
	    throws ParerUserError {
	if (helper.existsEnteSuptPerEnteFornit(idEnteNonConvenz, idEnteConvenz, dtIniValSupt,
     ...
     * @param idUserIamCor    id user IAM
     * @param idEnteFornitEst id ente fornitore esterno
     * @param idEnteProdut    id ente produttore
     * @param dtIniVal        data inizio validita
     * @param dtFinVal        data fine validita
     * @param tipoEnte        tipo ente
     *
     * @return pk
     *
     ...
     */
    @TransactionAttribute(TransactionAttributeType.REQUIRES_NEW)
    public Long insertOrgSuptEsternoEnteConvenz(LogParam param, long idUserIamCor,
	    BigDecimal idEnteFornitEst, BigDecimal idEnteProdut, Date dtIniVal, Date dtFinVal,
	    String tipoEnte) throws ParerUserError {
	...
	for (UsrUser utente : utentiSup) {
		...
		userHelper.aggiungiAbilEntiSuptFornit(idUserIamCor, utente.getIdUserIam(),
			idEnteFornitEst.longValue(), idEnteProdut.longValue());
	    }
		...
		OrgVEnteConvenzByOrganizTableBean tb = entiComnvenzionatiEjb
			.getOrgVEnteConvenzByOrganizTableBean(
				BigDecimal.valueOf(enteProdut.getIdEnteSiam()), true);
		// Controllo se la struttura/versatore per l'utente è già abilitata
		for (int i = 0; i < tb.size(); i++) {
		    BigDecimal idOrganizIam = tb.getRow(i).getIdOrganizIam();
		    // Controllo se la struttura/versatore (idOrganizIam) per l'utente è già
		    // abilitata
		    if (!userHelper.isAbilEntiConvOrg(usoUserApplic.getIdUsoUserApplic(),
			    idOrganizIam.longValue())) {
     ...
     * @param param                parametri per il logging
     * @param idEnteFornitEst      id ente fornitore esterno
     * @param idSuptEstEnteConvenz id ente convenzionato
     * @param dtIniVal             data inizio validita
     * @param dtFinVal             data fine validita
     * @param tipoEnte             tipo ente
     *
    ...
    public Long updateOrgSuptEsternoEnteConvenz(LogParam param, BigDecimal idSuptEstEnteConvenz,
	    BigDecimal idEnteFornitEst, Date dtIniVal, Date dtFinVal, String tipoEnte)
	    throws ParerUserError {

	OrgSuptEsternoEnteConvenz suptEsternoEnteConvenz = helper
		.findById(OrgSuptEsternoEnteConvenz.class, idSuptEstEnteConvenz);
	suptEsternoEnteConvenz.setDtIniVal(dtIniVal);
	suptEsternoEnteConvenz.setDtFinVal(dtFinVal);
	...
	return helper.checkEsistenzaAccordiDtIniDtFineVal(idEnteOrganoVigil, dtIniVal, dtFinVal,
     ...
     * @param param                parametri per il logging
     * @param idUserIamCor         id user IAM
     * @param idEnteOrganoVigil    l'ente organo di vigilanza di cui l'accordo fa parte
     * @param idEnteConserv        l'ente conservatore che stipula l'accordo con l'organo di
     *                             vigilanza
     * @param dsNoteEnteAccordo    note sull'accordo
     * @param dtIniVal             data inizio validita accordo
     * @param dtFinVal             data fine validita accordo
     * @param cdRegistroRepertorio registro dell'accordo
     * @param aaRepertorio         anno dell'accordo
     * @param cdKeyRepertorio      numero dell'accordo
     * @param dsFirmatarioEnte     firmatario dell'accordo
     * @param dtRegAccordo         data di registrazione dell'accordo
     * @param nmEnte               ente SACER a cui appartiene la struttura in cui e' versata
     *                             l'unità documentaria corrispondente all'accordo sottoscritto con
     *                             un ente
     * @param nmStrut              struttura in cui e' versata l'unità documentaria corrispondente
     ...
     */
    @TransactionAttribute(TransactionAttributeType.REQUIRES_NEW)
    public Long insertOrgAccordoVigil(LogParam param, long idUserIamCor,
	    BigDecimal idEnteOrganoVigil, BigDecimal idEnteConserv, Date dtIniVal, Date dtFinVal,
	    String dsNoteEnteAccordo, String cdRegistroRepertorio, BigDecimal aaRepertorio,
	    String cdKeyRepertorio, String dsFirmatarioEnte, Date dtRegAccordo, String nmEnte,
	    String nmStrut) throws ParerUserError {
	...
	// Determino la lista degli oggetti PRIMA della modifica
	// List<ObjectsToLogBefore> listaOggettiDaLoggarePrima =
	...
	// param.getNomePagina());
	// Determina gli utenti PERSONA_FISICA che appartengono all'ente conservatore dell'accordo
	// di vigilanza
	...
	// Per ciascuno degli utenti trovati al punto precedente il sistema inserisce l’abilitazione
	// all’organo di
	...
	// Determino la lista degli oggetti DOPO la modifica
	// List<ObjectsToLogBefore> listaOggettiDaLoggareDopo =
	// sacerLogEjb.logBefore(param.getTransactionLogContext(),
	// param.getNomeApplicazione(), param.getNomeUtente(), param.getNomeAzione(),
	// SacerLogConstants.TIPO_OGGETTO_ORGANO_VIGILANZA,
	// new BigDecimal(accordoVigil.getOrgEnteSiamByIdEnteOrganoVigil().getIdEnteSiam()),
	...
	// if (!listaOggettiDaLoggarePrima.isEmpty() && !listaOggettiDaLoggareDopo.isEmpty()) {
	// // Calcolo la differenza tra listaOggettiDaLoggarePrima e listaOggettiDaLoggareDopo:
	// List<BigDecimal> listaIdOggetti =
	// listaOggettiDaLoggareDopo.get(0).getIdOggetto().stream().filter(
	// u2 -> listaOggettiDaLoggarePrima.get(0).getIdOggetto().stream().noneMatch(u1 ->
	// u1.equals(u2)))
	// .collect(Collectors.toList());
	// // Aggiorno la lista degli oggetti nella listaOggettiDaLoggareDopo da passare alla
	// logAfter
	...
	sacerLogEjb.log(param.getTransactionLogContext(),
		paramHelper.getParamApplicApplicationName(), param.getNomeUtente(),
		param.getNomeAzione(), SacerLogConstants.TIPO_OGGETTO_ORGANO_VIGILANZA,
     ...
     *
     * @param param                parametri per il logging
     * @param idAccordoVigil       id dell'accordo di vigilanza che sto modificando
     * @param idEnteOrganoVigil    l'ente organo di vigilanza di cui l'accordo fa parte
     * @param dsNoteEnteAccordo    note sull'accordo
     * @param dtIniVal             data inizio validita accordo
     * @param dtFinVal             data fine validita accordo
     ...
     */
    @TransactionAttribute(TransactionAttributeType.REQUIRES_NEW)
    public Long updateOrgAccordoVigil(LogParam param, BigDecimal idAccordoVigil,
	    BigDecimal idEnteOrganoVigil, Date dtIniVal, Date dtFinVal, String dsNoteEnteAccordo,
	    String cdRegistroRepertorio, BigDecimal aaRepertorio, String cdKeyRepertorio,
	    String dsFirmatarioEnte, Date dtRegAccordo) throws ParerUserError {
    ...
    public void checkEnteVigilatoPerAccordoVigil(BigDecimal idAccordoVigil, BigDecimal idEnteProdut,
	    String nmEnteProdut, Date dtIniValVigil, Date dtFinValVigil,
	    BigDecimal idVigilEnteProdutDaEscludere) throws ParerUserError {
	if (helper.existsEnteVigilatoPerAccordoVigil(idAccordoVigil, idEnteProdut, dtIniValVigil,
		dtFinValVigil, idVigilEnteProdutDaEscludere)) {
     ...
     * @param param          parametri per il logging
     * @param idUserIamCor   id user IAM
     * @param idAccordoVigil id dell'accordo di vigilanza
     * @param idEnteProdut   id ente produttore
     * @param dtIniVal       data inizio validita
     * @param dtFinVal       data fine validita
     *
     ...
     */
    @TransactionAttribute(TransactionAttributeType.REQUIRES_NEW)
    public Long insertOrgVigilEnteProdut(LogParam param, long idUserIamCor,
	    BigDecimal idAccordoVigil, BigDecimal idEnteProdut, Date dtIniVal, Date dtFinVal)
	    throws ParerUserError {

	OrgVigilEnteProdut vigilEnteProdut = new OrgVigilEnteProdut();

	OrgAccordoVigil accordoVigil = helper.findByIdWithLock(OrgAccordoVigil.class,
		idAccordoVigil);
	...
	List<UsrUser> utentiAppartenentiOrganoVigilanza = utentiHelper.retrieveUsrUserEnteVigil(
		new BigDecimal(vigilEnteProdut.getOrgAccordoVigil()
			.getOrgEnteSiamByIdEnteOrganoVigil().getIdEnteSiam()),
		Collections.singletonMap("IN",
			Arrays.asList(ConstOrgEnteSiam.TiEnteNonConvenz.ORGANO_VIGILANZA)),
		Collections.singletonMap("NOT IN",
			Arrays.asList(ConstUsrStatoUser.TiStatotUser.CESSATO.name())),
		Collections.singletonMap("IN",
			Arrays.asList(ApplEnum.TipoUser.PERSONA_FISICA.name())));
	...
	sacerLogEjb.log(param.getTransactionLogContext(),
		...
		param.getNomeAzione(), SacerLogConstants.TIPO_OGGETTO_ORGANO_VIGILANZA,
		BigDecimal
			.valueOf(accordoVigil.getOrgEnteSiamByIdEnteOrganoVigil().getIdEnteSiam()),
		param.getNomePagina());
     ...
     *
     * @param param             parametri per il logging
     * @param idVigilEnteProdut id della vigilanza
     ...
     */
    @TransactionAttribute(TransactionAttributeType.REQUIRES_NEW)
    public Long updateOrgVigilEnteProdut(LogParam param, BigDecimal idVigilEnteProdut,
	    BigDecimal idEnteOrganoVigil, Date dtIniVal, Date dtFinVal) throws ParerUserError {

	OrgVigilEnteProdut vigilEnteProdut = helper.findById(OrgVigilEnteProdut.class,
		idVigilEnteProdut);
	vigilEnteProdut.setDtIniVal(dtIniVal);
	vigilEnteProdut.setDtFinVal(dtFinVal);
	...
	BigDecimal idEnteOrganoVigil = BigDecimal.valueOf(vigilEnteProdut.getOrgAccordoVigil()
		.getOrgEnteSiamByIdEnteOrganoVigil().getIdEnteSiam());
	BigDecimal idEnteConvenzVigil = BigDecimal
		.valueOf(vigilEnteProdut.getOrgEnteSiam().getIdEnteSiam());

	SortedSet<String> listaTotaleUtenti = new TreeSet<>();

	// // Determino gli utenti appartenenzi all'ENTE CONVENZIONATO rimosso dalla vigilanza,
	// perchè poi su di essi
	// // dovrò rimuove l'abilitazione all'ente NON convenzionato di tipo organo di vigilanza
	// (visto che ora non mi
	// // vigila più...)
     ...
     */
    @TransactionAttribute(TransactionAttributeType.REQUIRES_NEW)
    public void deleteOrgVigilEnteProdut(LogParam param, BigDecimal idVigilEnteProdut)
	    throws ParerUserError {
	...
	try {
	    ...
	    sacerLogEjb.log(param.getTransactionLogContext(), param.getNomeApplicazione(),
		    param.getNomeUtente(), param.getNomeAzione(),
		    SacerLogConstants.TIPO_OGGETTO_ORGANO_VIGILANZA, idEnteOrganoVigil,
		    param.getNomePagina());
	    ...
	    for (UsrUser utente : utentiAppartenentiOrganoVigilanza) {
		/*
		 * Il sistema elimina le abilitazioni alle organizzazioni (a cui l’utente corrente
		 * e’ abilitato) appartenenti all'ente vigilato rimosso
		 */
		userHelper.eliminaAbilOrganizVigil(utente.getIdUserIam(),
			vigilEnteProdut.getIdVigilEnteProdut());
		/*
		 * Il sistema elimina le abilitazioni ai tipi dato (a cui l’utente corrente e’
		 * abilitato) delle organizzazioni appartenenti all'ente vigilato rimosso
		 */
	    ...
	    // // Calcolo la differenza tra listaOggettiDaLoggarePrima e listaOggettiDaLoggareDopo:
	    // List<BigDecimal> listaIdOggetti =
	    // listaOggettiDaLoggarePrima.get(0).getIdOggetto().stream()
    ...
    // // e flag “Allinea abilitazioni alle organizzazioni vigilate in automatico”
    // // che appartengono all’ente NON convenzionato di tipo ORGANO DI VIGILANZA
    // List<UsrUser> utentiAppartenentiOrganoVigilanza =
    // utentiHelper.retrieveUsrUserEnteVigil(idEnteOrganoVigil,
    // Collections.singletonMap("IN",
    // Arrays.asList(ConstOrgEnteSiam.TiEnteNonConvenz.ORGANO_VIGILANZA)),
    // Collections.singletonMap("NOT IN",
    // Arrays.asList(ConstUsrStatoUser.TiStatotUser.CESSATO.name())),
     ...
     *
     * @param idVigilEnteProdut l'id dell'ente vigilato da recuperare su DB
     *
     * @return il rowbean contenente il dettaglio ente vigilato
     *
     * @throws ParerUserError errore generico
     */
    public OrgVigilEnteProdutRowBean getOrgVigilEnteProdutRowBean(BigDecimal idVigilEnteProdut)
	    throws ParerUserError {
	OrgVigilEnteProdut vigilEnteProdut = helper.findById(OrgVigilEnteProdut.class,
		idVigilEnteProdut);
	...
	try {
	    ...
	    row.setTimestamp("dt_ini_val_vigil",
		    new Timestamp(vigilEnteProdut.getDtIniVal().getTime()));
	    row.setTimestamp("dt_fin_val_vigil",
		    new Timestamp(vigilEnteProdut.getDtFinVal().getTime()));
	    if (vigilEnteProdut.getOrgEnteSiam().getOrgAmbienteEnteConvenz() != null) {
		row.setBigDecimal("id_ambiente_ente_convenz", BigDecimal.valueOf(vigilEnteProdut
			.getOrgEnteSiam().getOrgAmbienteEnteConvenz().getIdAmbienteEnteConvenz()));
		row.setString("nm_ambiente_ente_convenz", vigilEnteProdut.getOrgEnteSiam()
			.getOrgAmbienteEnteConvenz().getNmAmbienteEnteConvenz());
	    }
	    row.setBigDecimal("id_ente_convenz",
		    BigDecimal.valueOf(vigilEnteProdut.getOrgEnteSiam().getIdEnteSiam()));
	    row.setString("nm_ente_convenz", vigilEnteProdut.getOrgEnteSiam().getNmEnteSiam());
	} catch (ClassNotFoundException | NoSuchMethodException | InstantiationException
		| IllegalAccessException | IllegalArgumentException
		| InvocationTargetException ex) {
	    String msg = "Errore durante il recupero dell'ente vigilato "
		    + ExceptionUtils.getRootCauseMessage(ex);
	    LOGGER.error(msg, ex);
	    throw new ParerUserError(msg);
	}

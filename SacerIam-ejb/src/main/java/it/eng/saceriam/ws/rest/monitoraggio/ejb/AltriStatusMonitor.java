...
public class AltriStatusMonitor {
	...
	STATO_CONNESSIONE_ORCL, STATO_ERRORI_REPLICHE_UTENTI, STATO_TIMEOUT_REPLICHE_UTENTI,
	PRESENZA_REPLICHE_BLOCCATE
    }
	...
	if (controlliMonitor.controllaStatoDbOracle()) {
	    ...
	    tmpAltro.setStato(
		    MonitorSondeGenEsiti.ERROR.name() + "|Il database Oracle non è raggiungibile ");
	    log.error("Errore: Il database Oracle non è raggiungibile");
	}
	listaMon.add(tmpAltro);
    }

    public void calcolaErroriReplicheUtenti(RispostaWSStatusMonitor rispostaWs,
	    List<MonitorAltro> listaMon) {
    ...
    public void calcolaTimeoutReplicheUtenti(RispostaWSStatusMonitor rispostaWs,
	    List<MonitorAltro> listaMon) {
	RispostaControlli rc;
	MonitorAltro tmpAltro = new MonitorAltro();
    ...
    public void calcolaReplicheBloccate(RispostaWSStatusMonitor rispostaWs,
	    List<MonitorAltro> listaMon) {
	RispostaControlli rc;
	MonitorAltro tmpAltro = new MonitorAltro();

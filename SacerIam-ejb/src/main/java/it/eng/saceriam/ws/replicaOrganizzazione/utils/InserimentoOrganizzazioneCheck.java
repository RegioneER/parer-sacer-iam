...
public class InserimentoOrganizzazioneCheck {
	...
	if (rispostaWs.getSeverity() != SeverityEnum.ERROR) {
		...
		if (rispostaControlli.getCodErr() == null) {
		    ...
		    rispostaControlli.setDsErr(MessaggiWSBundle.getString(
			    MessaggiWSBundle.SERVIZI_ORG_002, inserimentoOrganizzazioneExt
				    .getInserimentoOrganizzazioneInput().getIdOrganizApplic()));
		    setRispostaWsError(SeverityEnum.ERROR, Constants.EsitoServizio.KO);
		} else {
		    // Errore 666
		    setRispostaWsError(SeverityEnum.ERROR, Constants.EsitoServizio.KO);
	...
	if (rispostaWs.getSeverity() != SeverityEnum.ERROR) {
	    // Se l'idOrganizApplicPadre (non obbligatorio) è != null, allora significa che deve
	    // avere il papi
	    // (perchè se ad esempio fosse un AMBIENTE non servirebbe), dunque verifico
	    if (inserimentoOrganizzazioneExt.getInserimentoOrganizzazioneInput()
		    .getIdOrganizApplicPadre() != null) {
		rispostaControlli.reset();
		rispostaControlli = controlliWS.verificaEsistenzaOrganizzazione(
			inserimentoOrganizzazioneExt.getInserimentoOrganizzazioneInput()
				.getNmApplic(),
			...
			inserimentoOrganizzazioneExt.getInserimentoOrganizzazioneInput()
				.getNmTipoOrganizPadre());
		if (!rispostaControlli.isrBoolean()) {
		    if (rispostaControlli.getCodErr() == null) {
			rispostaControlli.setCodErr(MessaggiWSBundle.SERVIZI_ORG_003);
			rispostaControlli.setDsErr(
				MessaggiWSBundle.getString(MessaggiWSBundle.SERVIZI_ORG_003,
					inserimentoOrganizzazioneExt
						.getInserimentoOrganizzazioneInput()
						.getIdOrganizApplicPadre()));
			setRispostaWsError(SeverityEnum.ERROR, Constants.EsitoServizio.KO);
		    } else {
			// Errore 666
	...
	if (rispostaWs.getSeverity() != SeverityEnum.ERROR) {
		...
		if (rispostaControlli.getCodErr() == null) {
		    ...
		    rispostaControlli
			    .setDsErr(MessaggiWSBundle.getString(MessaggiWSBundle.SERVIZI_ORG_004));
		    setRispostaWsError(SeverityEnum.ERROR, Constants.EsitoServizio.KO);
		} else {
		    // Errore 666
		    setRispostaWsError(SeverityEnum.ERROR, Constants.EsitoServizio.KO);
	...
	if (rispostaWs.getSeverity() != SeverityEnum.ERROR) {
		...
		if (rispostaControlli.getCodErr() == null) {
		    ...
		    rispostaControlli
			    .setDsErr(MessaggiWSBundle.getString(MessaggiWSBundle.SERVIZI_ORG_006));
		    setRispostaWsError(SeverityEnum.ERROR, Constants.EsitoServizio.KO);
		} else {
		    // Errore 666
		    setRispostaWsError(SeverityEnum.ERROR, Constants.EsitoServizio.KO);

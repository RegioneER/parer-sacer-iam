...
public class AllineaRuoloEjb {
	...
	try {
	    ...
	    Long idRuolo = context.getBusinessObject(AllineaRuoloEjb.class).updateRuolo(param,
		    input.getNmRuolo(), input.getDsRuolo(), input.getTiRuolo(),
		    input.getListaCategRuolo());
		...
		if (cdErr.equals(MessaggiWSBundle.SERVIZI_RUOLI_002)) {
		    resp.setDlErr(
			    MessaggiWSBundle.getString(cdErr, ruolo.getDsMsgAllineamentoParz()));
		} else {
	...
	sacerLogEjb.log(param.getTransactionLogContext(), param.getNomeApplicazione(),
		...
		new BigDecimal(ruolo.getIdRuolo()), param.getNomeComponenteSoftware(),
     ...
     * @param applics lista applicazioni di tipo {@link ListaApplic}
     *
	...
	for (Applic applic : applics) {
	    ...
	    RispostaControlli verificaApplicazione = controlliWs
		    .verificaApplicazione(applic.getNmApplic());
		...
		if (applicDb.getCdVersioneComp().equalsIgnoreCase(applic.getCdVersioneComp())) {
		    ...
		    // Si appoggia sulla tabella PrfAllineaRuolo per ottenere le dichiarazioni da
		    // eliminare e creare
		    ...
		    // Il flag potrebbe essere di nuovo a true se ci sono incongruenze tra i csv di
		    // profilazione delle
		    // due applicazioni
		    PrfVChkAllineaRuolo prfVChkAllineaRuolo = ammRuoliHelper
			    .getPrfVChkAllineaRuolo(idRuolo, applic.getNmApplic());
		    flAllineamentoParziale = prfVChkAllineaRuolo.getFlAllineamentoParziale()
			    .equals("1");
			...
			ruolo.setDsMsgAllineamentoParz(StringUtils.defaultString(oldMsg)
				+ "L'applicazione " + applic.getNmApplic()
		    ...
		    ruolo.setDsMsgAllineamentoParz(StringUtils.defaultString(oldMsg)
			    + "L'applicazione " + applic.getNmApplic()
			    + " nell'ambiente source ha versione " + applic.getCdVersioneComp()
	...
	if (flAllineamentoParziale && countAllineaRuolo == 0L) {
	    ...
	    List<PrfVLisUserDaReplic> lisUserDaReplic = ammRuoliHelper
		    .retrievePrfVLisUserDaReplic(idRuolo);
	    ...
	    List<PrfVLisDichAllineaToadd> dichToAdd = ammRuoliHelper
		    .retrievePrfVLisDichAllineaToadd(idRuolo);
	    for (PrfVLisDichAllineaToadd dich : dichToAdd) {
		PrfDichAutor dichAutor = new PrfDichAutor();
		dichAutor.setPrfUsoRuoloApplic(ammRuoliHelper.findById(PrfUsoRuoloApplic.class,
			dich.getIdUsoRuoloApplic()));
		...
		dichAutor.setAplAzionePagina((dich.getIdAzionePagina() != null
			? ammRuoliHelper.findById(AplAzionePagina.class, dich.getIdAzionePagina())
			: null));
		dichAutor.setAplPaginaWeb((dich.getIdPaginaWeb() != null
			? ammRuoliHelper.findById(AplPaginaWeb.class, dich.getIdPaginaWeb())
			: null));
		dichAutor.setAplServizioWeb((dich.getIdServizioWeb() != null
			? ammRuoliHelper.findById(AplServizioWeb.class, dich.getIdServizioWeb())
			: null));
		dichAutor.setAplEntryMenuPadre((dich.getIdEntryMenuPadre() != null
			? ammRuoliHelper.findById(AplEntryMenu.class, dich.getIdEntryMenuPadre())
			: null));
		dichAutor.setAplEntryMenuFoglia((dich.getIdEntryMenuFoglia() != null
			? ammRuoliHelper.findById(AplEntryMenu.class, dich.getIdEntryMenuFoglia())
			: null));
		ammRuoliHelper.insertEntity(dichAutor, true);
	    }
	    // Chiamo la procedura di allineamento autorizzazioni utilizzata anche in modifica di un
	    // ruolo
		...
		userHelper.registraLogUserDaReplic(
			userDaReplic.getPrfVLisUserDaReplicId().getIdUserIam(),
			userDaReplic.getPrfVLisUserDaReplicId().getIdApplic(),
    ...
    // for (Applic applic : applics) {
    // // Verifica la presenza dell'applicazione
    // RispostaControlli verificaApplicazione =
    // controlliWs.verificaApplicazione(applic.getNmApplic());
    // if (verificaApplicazione.isrBoolean()) {
    // AplApplic applicDb = (AplApplic) verificaApplicazione.getrObject();
    // if (applicDb.getCdVersioneComp().equalsIgnoreCase(applic.getCdVersioneComp())) {
    // // Verifica la presenza della relazione Applicazione-Ruolo (PrfUsoRuoloApplic)
    ...
    // ammRuoliHelper.insertEntity(usoRuolo, true);
    // }
    // // Si appoggia sulla tabella PrfAllineaRuolo per ottenere le dichiarazioni da eliminare e
    // creare
    // for (DichAutor dichAutor : applic.getListaDichAutor()) {
    // PrfAllineaRuolo allineaRuolo = new PrfAllineaRuolo();
    // allineaRuolo.setPrfRuolo(ruolo);

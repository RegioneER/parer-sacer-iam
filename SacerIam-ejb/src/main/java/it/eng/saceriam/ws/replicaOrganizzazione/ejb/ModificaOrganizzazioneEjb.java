...
public class ModificaOrganizzazioneEjb {
    ...
    public ModificaOrganizzazioneRisposta modificaOrganizzazione(String nmApplic,
            Integer idOrganizApplic, String nmTipoOrganiz, Integer idEnteConserv,
            Integer idEnteGestore, Integer idOrganizApplicPadre, String nmTipoOrganizPadre,
            String nmOrganiz, String dsOrganiz, Integer idEnteConvenz, Date dtIniVal,
        ...
        // 1° LOG DEBUG
        log.debug("Modifica Organizzazione - Ricezione dei seguenti parametri da {} per "
                ...
                + ", dtIniVal = {}, dtFineVal = {}", nmApplic, nmOrganiz, idOrganizApplic,
                nmTipoOrganiz, idEnteConserv, idEnteGestore, idOrganizApplicPadre,
        ...
        ModificaOrganizzazioneInput moInput = new ModificaOrganizzazioneInput(nmApplic,
                idOrganizApplic, nmTipoOrganiz, idEnteConserv, idEnteGestore, nmOrganiz, dsOrganiz,
                idOrganizApplicPadre, nmTipoOrganizPadre, listaTipiDato);
        ...
        if (rispostaWs.getSeverity() != IRispostaWS.SeverityEnum.ERROR) {
            ModificaOrganizzazioneCheck checker = new ModificaOrganizzazioneCheck(moExt,
                    rispostaWs);
            ...
            try {
                ...
                if (moExt.getModificaOrganizzazioneInput().getIdOrganizApplicPadre() != null) {
                    if (goh.isDadModified(moExt.getIdApplic(),
                        ...
                        usersToReply = goh.getUsersWithOrganizDadDich(moExt.getIdOrganizIamPadre());
                    }
                }

                // 2° LOG DEBUG
                log.debug(
                        "Modifica Organizzazione - Se ho modificato il padre dell'organizzazione (es. la struttura"
                                + "è ora associata ad un altro ente) ricavo la lista degli utenti da replicare. In questo "
                                + "caso ho ricavato i seguenti utenti: {}",
                ...
                if (numTipiDatoInseriti != 0 || numTipiDatoEliminati != 0) {
                    usersToReply.addAll(
                            goh.getUsersOnDich(moExt.getIdOrganizIam(), ApplEnum.TiOperReplic.MOD));
                }
                ...
                // Se è un caso di importa/duplica standard (sovrascrittura di un template) devo
                // inserire l'associazione
                // struttura-ente
                if (moExt.isTemplate() && idEnteConvenz != null && dtIniVal != null
                        && dtFineVal != null) {
                    /*
                    ...
                    OrgEnteSiam enteSiam = goh.findById(OrgEnteSiam.class,
                            idEnteConvenz.longValue());
                    ...
                    if (enteSiam.getTiEnte().equals(ConstOrgEnteSiam.TiEnteSiam.CONVENZIONATO)) {
                        ...
                        } else if (enteSiam.getTiEnteNonConvenz().name()
                                .equals("SOGGETTO_ATTUATORE")) {
                ...
                UsrOrganizIam organiz = goh.getUsrOrganizIam(moExt.getIdApplic(),
                ...
                if (nmApplic.equals("SACER") && numTipiDatoInseriti > 0) {
                    ...
                    for (BigDecimal idUserIam : usersSacer) {
                        // 9° LOG DEBUG
                        log.debug("Modifica Organizzazione - Procedo ad elaborare l'utente {}",
                                idUserIam);
                        goh.registraUtenteDaReplicare(idUserIam, moExt.getIdApplic(), rispostaWs);
                        log.debug(
                        ...
                        // 10° LOG DEBUG
                        formattaOutputOrganizVigil(user.get(0).getIdUserIam(),
                                idUserIam.longValue());
                        uh.aggiungiAbilOrganizVigil(user.get(0).getIdUserIam(),
                                idUserIam.longValue());
                        log.debug(
                        ...
                        sacerLogEjb.log(param.getTransactionLogContext(),
                                ...
                                param.getNomeAzione(), param.getNomeTipoOggetto(),
                ...
                rispostaWs.getModificaOrganizzazioneRisposta()
                        .setIdOrganizApplic(moInput.getIdOrganizApplic());
                rispostaWs.getModificaOrganizzazioneRisposta()
                        .setNmTipoOrganiz(moInput.getNmTipoOrganiz());
                rispostaWs.getModificaOrganizzazioneRisposta()
                        .setIdOrganizApplicPadre(moInput.getIdOrganizApplicPadre());
                rispostaWs.getModificaOrganizzazioneRisposta()
                        .setNmTipoOrganizPadre(moInput.getNmTipoOrganizPadre());
                ...
                rispostaWs.getModificaOrganizzazioneRisposta()
                        .setIdEnteConvenz(moInput.getIdEnteConvenz());
                ...
                rispostaWs.getModificaOrganizzazioneRisposta()
                        .setListaTipiDato(moInput.getListaTipiDato());
    ...
    private void formattaOutputOrganiz(long idUserIam, long idApplic) {
        ...
        if (!orgToAddList.isEmpty()) {
            for (UsrVAbilOrganizToAdd orgToAdd : orgToAddList) {
                log.debug(
                        "idDichAbilOrganiz = {}, idUsoUserApplic = {}, idOrganizIam = {}, nmApplic = {} ",
                        orgToAdd.getIdDichAbilOrganiz(), orgToAdd.getIdUsoUserApplic(),
                        orgToAdd.getUsrVAbilOrganizToAddId().getIdOrganizIam(),
        ...
        if (!datiToAddList.isEmpty()) {
            for (UsrVAbilDatiToAdd datiToAdd : datiToAddList) {
                log.info(
                        ...
                        datiToAdd.getUsrVAbilDatiToAddId().getIdTipoDatoIam(),
                        datiToAdd.getIdSuptEstEnteConvenz(), datiToAdd.getIdVigilEnteProdut(),
                        datiToAdd.getNmApplic());
            }
        } else {
    ...
    private void formattaListaTipiDato(ListaTipiDato listaTipiDato) {
        ...
        if (listaTipiDato != null && listaTipiDato.getTipoDato() != null
                && !listaTipiDato.getTipoDato().isEmpty()) {
            for (TipoDato tipoDato : listaTipiDato) {
                log.debug("idTipoDatoApplic = {}, nmClasseTipoDato = {}, nmTipoDato = {}",
                        tipoDato.getIdTipoDatoApplic(), tipoDato.getNmClasseTipoDato(),
                        tipoDato.getNmTipoDato());
            }
        }
    }

    private void formattaOutputOrganizVigil(long idUserIamCor, long idUserIamGestito) {
        List<UsrVAbilOrgVigilToAdd> orgVigilToAddList = uh.getAbilOrganizToVigil(idUserIamCor,
                idUserIamGestito);
    ...
    private void formattaOutputDatiVigil(long idUserIamCor, long idUserIamGestito) {
        List<UsrVAbilDatiVigilToAdd> datiVigilToAddList = uh.getAbilDatiVigil(idUserIamCor,
                idUserIamGestito);

...
public class ModificaOrganizzazioneCheck {
	...
	if (rispostaWs.getSeverity() != SeverityEnum.ERROR) {
		...
		if (rispostaControlli.getCodErr() == null) {
		    ...
		    rispostaControlli.setDsErr(MessaggiWSBundle
			    .getString(MessaggiWSBundle.SERVIZI_ORG_005, modificaOrganizzazioneExt
				    .getModificaOrganizzazioneInput().getIdOrganizApplic()));
		    setRispostaWsError(SeverityEnum.ERROR, Constants.EsitoServizio.KO);
		} else {
		    // Errore 666
		    setRispostaWsError(SeverityEnum.ERROR, Constants.EsitoServizio.KO);
	...
	if (rispostaWs.getSeverity() != SeverityEnum.ERROR) {
	    ...
	    if (modificaOrganizzazioneExt.getModificaOrganizzazioneInput()
		    .getIdOrganizApplicPadre() != null) {
		rispostaControlli.reset();
		rispostaControlli = controlliWS.verificaEsistenzaOrganizzazione(
			modificaOrganizzazioneExt.getModificaOrganizzazioneInput().getNmApplic(),
			modificaOrganizzazioneExt.getModificaOrganizzazioneInput()

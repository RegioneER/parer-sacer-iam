...
public class ControlliMonitor {
	...
	try {
	    String queryStr = "select count(u) from LogUserDaReplic u "
		    + "where u.tiStatoReplic = 'REPLICA_IN_ERRORE'";
	    javax.persistence.Query query = entityManager.createQuery(queryStr);
	...
	try {
	    ...
	    rispostaControlli.setCodErr(MessaggiWSBundle.ERR_666);
	    rispostaControlli.setDsErr(MessaggiWSBundle.getString(MessaggiWSBundle.ERR_666,
		    "Eccezione ControlliMonitor.controllaPresenzaTimeoutReplica "
			    + e.getMessage()));
	    log.error("Eccezione nella lettura della tabella LogUserDaReplic ", e);
	}
	...
	try {
	    ...
	    String queryStr = "select s from UsrVLisSched s "
		    + "where s.nmJob like 'REPLICA_UTENTI_%' " + "AND s.tiBlocco IS NOT NULL "
		    + "AND s.dtRegLogJobIni >= :datada " + "AND (s.dlMsgErr IS NULL "

...
public class GestioneOrganizzazioneHelper extends GenericHelper {
    ...
    /**
     * Metodo che restituisce l'entità UsrOrganizIam dati in ingresso i parametri che la identificano univocamente
     *
     * @param idApplic
     *            id applicazione
     ...
     *
     */
    public UsrOrganizIam getUsrOrganizIam(Long idApplic, Integer idOrganizApplic, String nmTipoOrganiz) {
        String queryStr = "SELECT u FROM UsrOrganizIam u " + "WHERE u.aplApplic.idApplic = :idApplic "
                + "AND u.idOrganizApplic = :idOrganizApplic " + "AND u.aplTipoOrganiz.nmTipoOrganiz = :nmTipoOrganiz";
    ...
    public List<OrgEnteConvenzOrg> retrieveOrgEnteConvenzOrg(long idOrganizIam) {
        Query query = getEntityManager()
                .createQuery("SELECT s FROM OrgEnteConvenzOrg s WHERE s.usrOrganizIam.idOrganizIam = :idOrganizIam");
    ...
    public boolean isLastLevel(BigDecimal idApplic, String nmTipoOrganiz) {
        String queryStr = "SELECT u.flLastLivello FROM AplTipoOrganiz u " + "WHERE u.aplApplic.idApplic = :idApplic "
    ...
    /**
     * Metodo che ritorna l'insieme degli utenti per i quali è definita una dichiarazione relativa ad una organizzazione
     * "padre" di quella passata come parametro di ingresso, oppure è definita una dichiarazione relativa a tutte le
     * organizzazioni, oppure è definita una dichiarazione relativa all'organizzazione passata come parametro di
     * ingresso.
     *
     * @param idOrganizIam
     *            id organizzazione
    ...
    public Set<BigDecimal> getUsersOnDich(Long idOrganizIam, ApplEnum.TiOperReplic tiOperReplic) {
        ...
        queryStr.append(", UsrUser user, UsrStatoUser statoUser " + "WHERE u.id.idOrganizIam = :idOrganizIam "
                + "AND u.id.idUserIam = user.idUserIam " + "AND user.idStatoUserCor = statoUser.idStatoUser "
    ...
    /**
     * Metodo che restituisce l'insieme degli utenti per i quali è definita una dichiarazione relativaad una
     * organizzazione "padre" di quella passata come parametro di ingresso
     *
     * @param idOrganizIam
     *            id organizzazione
     *
    ...
    public Set<BigDecimal> getUsersWithOrganizDadDich(Long idOrganizIam) {
        String queryStr = "SELECT DISTINCT u.id.idUserIam "
                ...
                + "WHERE u.dlPathIdOrganizIam LIKE :idOrganizIam " + "AND u.idDichAbilOrganiz = v.idDichAbilOrganiz "
                + "AND v.tiScopoDichAbilOrganiz = 'ALL_ORG_CHILD' " + "AND u.id.idUserIam = user.idUserIam "
    ...
    public UsrOrganizIam insertOrganizIam(InserimentoOrganizzazioneExt ioExt,
        ...
        try {
            ...
            organiz.setAplTipoOrganiz(getAplTipoOrganiz(aplApplic.getIdApplic(), parametriInput.getNmTipoOrganiz()));
            // Inserisco il padre
            UsrOrganizIam papi = getUsrOrganizIam(aplApplic.getIdApplic(), parametriInput.getIdOrganizApplicPadre(),
            ...
            if (organiz.getAplTipoOrganiz().getNmTipoOrganiz().equals("AMBIENTE")
                    && parametriInput.getIdEnteConserv() != null && parametriInput.getIdEnteGestore() != null) {
                organiz.setOrgEnteSiamConserv(
                        getEntityManager().find(OrgEnteSiam.class, parametriInput.getIdEnteConserv().longValue()));
                organiz.setOrgEnteSiamGestore(
                        getEntityManager().find(OrgEnteSiam.class, parametriInput.getIdEnteGestore().longValue()));
            }
            ...
            throw new TransactionException("Errore nel salvataggio dell'organizzazione " + ex.getMessage(), ex);
        }
        return organiz;
    }

    /**
     * Restituisce l'entity della classe tipo dato attraverso i parametri che la identificano univocamente
     *
     * @param idApplic
     *            id applicazione
    ...
    public AplClasseTipoDato getAplClasseTipoDato(Long idApplic, String nmClasseTipoDato) {
        Query ctdQuery = entityManager.createQuery("SELECT u FROM AplClasseTipoDato u "
                + "WHERE u.nmClasseTipoDato = :nmClasseTipoDato " + "AND u.aplApplic.idApplic = :idApplic ");
    ...
    /**
     * Restituisce l'entity della classe tipo organizzazione attraverso i parametri che la identificano univocamente
     *
     * @param idApplic
     *            id applicazione
     * @param nmTipoOrganiz
    ...
    public AplTipoOrganiz getAplTipoOrganiz(Long idApplic, String nmTipoOrganiz) {
        Query atoQuery = entityManager.createQuery("SELECT u FROM AplTipoOrganiz u "
                + "WHERE u.nmTipoOrganiz = :nmTipoOrganiz " + "AND u.aplApplic.idApplic = :idApplic ");
     ...
     */
    @TransactionAttribute(TransactionAttributeType.REQUIRED)
    public void registraUtenteDaReplicare(BigDecimal idUserIam, Long idApplic, IRispostaWS rispostaWs)
            throws TransactionException {
        try {
            ...
            rispostaWs.setErrorMessage(
                    "Errore nella scrittura nel log utenti da replicare " + ExceptionUtils.getRootCauseMessage(ex));
            throw new TransactionException("Errore nella scrittura nel log utenti da replicare " + ex.getMessage(), ex);
        }
    }
    ...
    public void cancellaOrganizzazione(Long idApplic, CancellaOrganizzazioneInput parametriInput,
        ...
        try {
            ...
            rispostaWs.setErrorCode(MessaggiWSBundle.SERVIZI_ORG_001);
            rispostaWs.setErrorMessage(
                    "Errore nella cancellazione dell'organizzazione " + ExceptionUtils.getRootCauseMessage(ex));
            throw new TransactionException("Errore nella cancellazione dell'organizzazione " + ex.getMessage(), ex);
        }
    }
     ...
     */
    @TransactionAttribute(TransactionAttributeType.REQUIRED)
    public int[] updateOrganizIam(ModificaOrganizzazioneExt moExt, RispostaWSModificaOrganizzazione rispostaWs)
            throws TransactionException {
        ModificaOrganizzazioneInput parametriInput = moExt.getModificaOrganizzazioneInput();
        try {
            // Ricavo l'organizzazione da modificare
            UsrOrganizIam organiz = getUsrOrganizIam(moExt.getIdApplic(), parametriInput.getIdOrganizApplic(),
                    parametriInput.getNmTipoOrganiz());
            // Ricavo l'informazione se si tratta di una struttura, e di tipo template, verificando la presenza di
            ...
            organiz.setNmOrganiz(parametriInput.getNmOrganiz());
            // Inserisco il padre
            UsrOrganizIam papi = getUsrOrganizIam(moExt.getIdApplic(), parametriInput.getIdOrganizApplicPadre(),
                    parametriInput.getNmTipoOrganizPadre());
            organiz.setUsrOrganizIam(papi);
            ...
            for (TipoDato tipoDato : listaTipiDatoInput) {
                ...
                if (!presente) {
                    ...
                    td.setIdTipoDatoApplic(new BigDecimal(tipoDato.getIdTipoDatoApplic()));
                    // Recupero la classe tipo dato
                    AplClasseTipoDato actd = getAplClasseTipoDato(moExt.getIdApplic(), tipoDato.getNmClasseTipoDato());
                    td.setAplClasseTipoDato(actd);
            ...
            for (UsrTipoDatoIam tipoDatoIam : listaTipiDatoIam) {
                ...
                for (TipoDato tipoDato : listaTipiDatoInput) {
                    if (tipoDato.getNmTipoDato().equals(tipoDatoIam.getNmTipoDato()) && tipoDato.getNmClasseTipoDato()
                            .equals(tipoDatoIam.getAplClasseTipoDato().getNmClasseTipoDato())) {
                        trovato = true;
                        break;
                    }
                }
    ...
    /**
     * Metodo che restituisce il record di UsrTipoDatoIam, dati in ingresso idOrganizIam e nmTipoDato
     *
     * @param idOrganizIam
     *            id organizzazione
     * @param nmTipoDato
    ...
    public UsrTipoDatoIam getUsrTipoDatoIam(Long idOrganizIam, String nmTipoDato) {
        String queryStr = "SELECT u FROM UsrTipoDatoIam u " + "WHERE u.usrOrganizIam.idOrganizIam = :idOrganizIam "
    ...
    public boolean isDadModified(Long idApplic, Integer idOrganizApplic, String nmTipoOrganiz,
        ...
        Integer idOrganizApplicPadre = getUsrOrganizIam(idApplic, idOrganizApplic, nmTipoOrganiz).getUsrOrganizIam()
                .getIdOrganizApplic().intValue();

...
public class RecuperoAutorizzazioniEjb {
    ...
    public RecuperoAutorizzazioniRisposta recuperoAutorizzazioniPerId(Integer idUserIam,
	    String nmApplic, Integer idOrganiz) {
	return recuperoAutorizzazioni(idUserIam, null, nmApplic, idOrganiz, null);
    }

    public RecuperoAutorizzazioniRisposta recuperoAutorizzazioniPerNome(String nmUserIAM,
	    String nmApplic, Integer idOrganiz) {
	return recuperoAutorizzazioni(null, nmUserIAM, nmApplic, idOrganiz, null);
    }

    private RecuperoAutorizzazioniRisposta recuperoAutorizzazioni(Integer idUserIam,
	    String nmUserIAM, String nmApplic, Integer idOrganiz, String nmOrganiz) {
	if (StringUtils.isEmpty(nmUserIAM) && idUserIam == null) {
	    throw new IllegalArgumentException(
		    "Il parametro idUserIam o nmUserIAM deve essere definito");
	}
	if (StringUtils.isEmpty(nmApplic)) {
	    throw new IllegalArgumentException("Il parametro nmApplic non è definito");
	}
	List<UsrVAllAutor> pagineAzioniList = this.retrievePagineAzioni(idUserIam, nmUserIAM,
		nmApplic, idOrganiz, nmOrganiz);
	List<AplEntryMenu> entryMenuList = this.retrieveMenu(idUserIam, nmUserIAM, nmApplic,
		idOrganiz, nmOrganiz);

	RecuperoAutorizzazioniRisposta resp = fillAutorizzazioni(pagineAzioniList, entryMenuList,
		nmApplic);
	...
	if (userAuths != null && !userAuths.isEmpty() && entryMenuList != null
		&& !entryMenuList.isEmpty()) {
		...
		if (auth.getUsrVAllAutorId().getTiDichAutor().equals("PAGINA")) {
		    ...
		    if (auth.getFlHelpPresente() != null
			    && auth.getFlHelpPresente().trim().equals("1")) {
			...
			List<HelpDips> listaHelp = retrieveMenuPerPage(nmApplic,
				pagina.getNmPaginaWeb());
    ...
    private List<UsrVAllAutor> retrievePagineAzioni(Integer idUserIam, String nmUserIAM,
	    String nmApplic, Integer idOrganiz, String nmOrganiz) {
	...
	queryStr.append("SELECT DISTINCT u FROM UsrVAllAutor u WHERE ")
		...
		.append(" AND u.nmApplic = :nmApplic ")
		.append(" AND u.id.tiDichAutor != 'ENTRY_MENU' ");
	// Se sono vuoti il nome dell'organizzazione e anche l'ID organiz
	if (StringUtils.isEmpty(nmOrganiz) && idOrganiz == null) {
	    queryStr.append(" AND u.id.tiUsoRuo = 'DEF' ")
		    .append(" AND u.nmTipoOrganizApplic IS NULL ")
		    .append(" AND u.idOrganizApplic IS NULL ");
	} else {
	    queryStr.append(" AND u.id.tiUsoRuo = 'DICH' ")
		    .append(" AND u.nmTipoOrganizApplic = :tipoOrganiz ")
    ...
    private List<AplEntryMenu> retrieveMenu(Integer idUserIam, String nmUserIAM, String nmApplic,
	...
	queryStr.append("SELECT * FROM APL_ENTRY_MENU t0 WHERE EXISTS (")
		...
		.append(!StringUtils.isEmpty(nmUserIAM) ? " u.nm_userid = ?  " : "")
		.append(" AND u.nm_applic = ? ").append(" AND u.id_applic = t0.id_applic ")
		.append(" AND u.ti_dich_autor = 'ENTRY_MENU' ")
		.append(" AND u.id_autor = t0.id_entry_menu ");
	// Se sono vuoti il nome dell'organizzazione e anche l'ID organiz
	if (StringUtils.isEmpty(nmOrganiz) && idOrganiz == null) {
	    queryStr.append(" AND u.ti_uso_ruo = 'DEF' ")
		    .append(" AND u.nm_tipo_organiz_applic IS NULL ")
		    .append(" AND u.id_organiz_applic IS NULL ");
	} else {
	    queryStr.append(" AND u.ti_uso_ruo = 'DICH' ")
		    .append(" AND u.nm_tipo_organiz_applic = ? ")
	...
	if (l.size() == 0) {
	    ...
	    queryStr.append("SELECT * FROM APL_ENTRY_MENU t0 WHERE EXISTS (")
		    ...
		    .append(" AND u.ti_dich_autor = 'ENTRY_MENU' ")
		    .append(" AND u.id_autor = t0.id_entry_menu ")
		    .append(" AND u.ti_uso_ruo = 'DEF' ")
		    .append(" AND u.nm_tipo_organiz_applic IS NULL ")
		    .append(" AND u.id_organiz_applic IS NULL ").append(" )")
    ...
    /*
     * Estrae tutti i menu associati alla pagina dell'applicazione ad oggi che hanno un Help
     * Associato in sacerDips.
     */
	...
	query.setParameter("nmApplic", nmApplic);
	query.setParameter("tiHelpOnLine",
		ConstAplVVisHelpOnLine.TiHelpOnLine.HELP_RICERCA_DIPS.name());

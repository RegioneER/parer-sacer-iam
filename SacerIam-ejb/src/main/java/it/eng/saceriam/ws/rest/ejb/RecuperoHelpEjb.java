...
public class RecuperoHelpEjb {
	...
	return recuperoHelpHelper.recuperoHelp(nmApplic, tiHelpOnLine, nmPaginaWeb, nmEntryMenu,
		new Date());
    }

    public AplVVisHelpOnLineTableBean getAplVVisHelpOnLineTableBean(BigDecimal idApplic,
	    String tiHelpOnLine, Date dtRiferimento, BigDecimal idPaginaWeb, BigDecimal idEntryMenu)
	    throws EMFError {
	AplVVisHelpOnLineTableBean aplVVisHelpOnLineTableBean = new AplVVisHelpOnLineTableBean();
	List<AplVVisHelpOnLine> l = null;
	l = recuperoHelpHelper.getHelpBetweenDate(idApplic, tiHelpOnLine, dtRiferimento,
		idPaginaWeb, idEntryMenu);
	try {
	    if (l != null && !l.isEmpty()) {
		aplVVisHelpOnLineTableBean = (AplVVisHelpOnLineTableBean) Transform
			.entities2TableBean(l);
	    }
	} catch (Exception e) {
	...
	return recuperoHelpHelper.inserisciHelp(idApplic, idTipoHelp, fileName, idPaginaWeb,
		idEntryMenu, dataIni, dataFine, blHelpOnLine, blSorgenteHelpOnLine);
    }
	...
	return recuperoHelpHelper.modificaHelp(idHelp, fileName, dataIni, dataFine, blHelpOnLine,
		blSorgenteHelpOnLine);
    }

    public Date findMostRecentDtFin(long idApplic, String idTipoHelp, long idPaginaWeb,
	    long idEntryMenu) {
	Date data = recuperoHelpHelper.findMostRecentDtFin(idApplic, idTipoHelp, idPaginaWeb,
		idEntryMenu);
	...
	if (apl != null) {
	    ...
	    retCode = recuperoHelpHelper.intersectsExistingHelp(idHelp.longValueExact(),
		    apl.getAplApplic().getIdApplic(), apl.getTiHelpOnLine(),
		    apl.getAplPaginaWeb().getIdPaginaWeb(), idMenu, dataInizio, dataFine);
	}
	return retCode;
    }
     ...
     */
    public boolean intersectsExistingHelp(long idApplic, String tiHelpOnLine, long idPaginaWeb,
	    long idEntryMenu, Date dataInizio, Date dataFine) {
	return recuperoHelpHelper.intersectsExistingHelp(0, idApplic, tiHelpOnLine, idPaginaWeb,

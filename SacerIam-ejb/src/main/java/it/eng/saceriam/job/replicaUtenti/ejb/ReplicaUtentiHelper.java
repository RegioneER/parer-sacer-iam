...
public class ReplicaUtentiHelper extends GenericHelper {
    ...
    /**
     * Determina l'insieme degli utenti da replicare sulla base delle repliche relative
     * all'applicazione e agli utenti passati in input
     *
	...
	String queryStr = "SELECT DISTINCT log.idUserIam,log.dtLogUserDaReplic FROM LogUserDaReplic log "
		...
		+ "AND logInCorso.tiStatoReplic = 'REPLICA_IN_CORSO'"
		+ "AND logInCorso.idUserIam = log.idUserIam" + ")"
		+ "ORDER BY log.dtLogUserDaReplic, log.idUserIam ";
	javax.persistence.Query query = entityManager.createQuery(queryStr);
	userDaReplicList = query.getResultList();
	return userDaReplicList.stream().map(o -> BigDecimal.class.cast(o[0]))
     ...
     *
     * @param asyncJob    nome job
     * @param flJobAttivo flag 1/0 (true/false)
     *
    ...
    /**
     * Aggiorna l'insieme delle registrazioni nel log degli utenti da replicare relative agli utenti
     * passati in input settando lo stato 'REPLICA_IN_CORSO' e settando logJob
     *
     * @param users    lista di utenti
     * @param idLogJob id log job
     *
    ...
    /**
     * Determina l'insieme delle registrazioni nel log degli utenti da replicare relative
     * all'applicazione e agli utenti passati in input
     *
     * @param users    lista di utenti
     * @param idLogJob id log job
     ...
     * @return la lista dei record della tabella LogUserDaReplic
     */
    public List<LogUserDaReplic> getLogUserDaReplicList(Collection<BigDecimal> users,
	    Long idLogJob) {
	String queryStr = "SELECT log FROM LogUserDaReplic log "
		+ "WHERE log.tiStatoReplic = 'REPLICA_IN_CORSO' " + "AND log.idUserIam IN (:users) "
		+ "AND log.logJob.idLogJob = :idLogJob "
		+ "ORDER BY log.dtLogUserDaReplic, log.idUserIam ";
	javax.persistence.Query query = entityManager.createQuery(queryStr);
	query.setParameter("users", users);
     ...
     *
     * @param idLogUserDaReplic id log user
     * @param esitoServizio     esito
     * @param cdErr             codice di errore
     * @param dsErr             descrizione errore
     * @param applicErroreSet   lista distinta errori
     */
    @TransactionAttribute(TransactionAttributeType.REQUIRES_NEW)
    public void writeAtomicLogUserDaReplic(Long idLogUserDaReplic,
	    Constants.EsitoServizio esitoServizio, String cdErr, String dsErr,
	    Set<String> applicErroreSet) {
	LogUserDaReplic userDaReplic = entityManager.find(LogUserDaReplic.class, idLogUserDaReplic);
	if (userDaReplic == null) {
	    throw new NoResultException(
		    "Nessun LogUserDaReplic trovato con idLogUserDaReplic " + idLogUserDaReplic);
	}
	Date now = new Date();
	...
	if (esitoServizio == Constants.EsitoServizio.OK) {
		...
		if (userDaReplic.getTiOperReplic().equals(Constants.TiOperReplic.CANC.name())) {
		    ...
		    userDaReplic.setDtChiusuraReplica(now);
		    userDaReplic.setTiStatoReplic(Constants.TiStatoReplic.REPLICA_OK.name());
		} else if (userDaReplic.getTiOperReplic()
	    ...
	    userDaReplic
		    .setDsMsgErr("Per l'applicazione " + userDaReplic.getAplApplic().getNmApplic()
    ...
    /**
     * Determina l’insieme dei servizi web dell’applicazione corrente a cui l’utente e’ abilitato in
     * funzione dei suoi ruoli di default e l’insieme dei servizi web dell’applicazione corrente a
     * cui l’utente e’ abilitato in funzione dei suoi ruoli definiti a fronte delle dichiarazioni di
     * abilitazione alle organizzazioni
     *
	...
	String queryStr = "SELECT DISTINCT new it.eng.saceriam.viewEntity.UsrVAllAutor(u.id.tiDichAutor, "
		...
		+ "FROM UsrVAllAutor u " + "WHERE u.id.idUserIam = :idUserIam "
		+ "AND u.id.idApplic = :idApplic " + "AND u.id.tiDichAutor = 'SERVIZIO_WEB' ";
	...
	String queryStr = "SELECT DISTINCT new it.eng.saceriam.viewEntity.UsrVAbilOrganiz(u.id.idOrganizIam, u.idOrganizApplic) FROM UsrVAbilOrganiz u "
		+ "WHERE u.id.idUserIam = :idUserIam "
		+ "AND u.idApplic = :idApplic ORDER BY u.idOrganizApplic";
	javax.persistence.Query query = entityManager.createQuery(queryStr);
	query.setParameter("idUserIam", bigDecimalFrom(idUserIam));
	query.setParameter("idApplic", bigDecimalFrom(idApplic));
	return query.getResultList();
    ...
    /**
     * Determina l'insieme dei tipi di dato a cui l'utente dato in input è abilitato per quella
	...
	String queryStr = "SELECT u.cdIndIpUser FROM UsrIndIpUser u "
		+ "WHERE u.usrUser.idUserIam = :idUserIam ";
    ...
    public List<String> getTiScopoDichAbilOrganiz(BigDecimal idUsoUserApplic,
	...
	if (resultList.isEmpty()) {
	    throw new NoResultException("Nessun UsrUsoUserApplic trovato per idUserIam " + idUserIam
		    + " e idApplic " + idApplic);
	}
    ...
    /**
     * Determina l'insieme degli utenti da disattivare in quanto la password \u00E8 scaduta da
     * pi\u00F9 di 3 mesi
     *
     * @return la lista dei record della tabella LogUserDaReplic
     */
     ...
     *
     * @param uso       entity {@link UsrUsoUserApplic}
     * @param idUserIam id user IAM
     */
    @TransactionAttribute(TransactionAttributeType.REQUIRED)

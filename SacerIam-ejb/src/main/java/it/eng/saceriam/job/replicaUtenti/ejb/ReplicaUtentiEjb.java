...
public class ReplicaUtentiEjb {
    ...
    /**
     * Prepara gli utenti da replicare presenti in IAM e chiama i servizi di replica dei diversi
     * applicativi
     *
	...
	log.info(
		"Eseguo la disattivazione degli utenti attivi la cui password \u00E8 scaduta da pi\u00F9 di 3 mesi");
	...
	// MEV #26627: Modifica alla replica utenti per non disattivare gli utenti che accedono
	// utilizzando SPID
	...
	for (Long idUserIam : users) {
	    // MEV #26627: Modifica alla replica utenti per non disattivare gli utenti che accedono
	    // utilizzando SPID
	    context.getBusinessObject(ReplicaUtentiEjb.class).resetPasswordAutomaticoUtente(param,
		    idUserIam);
	}
	...
	Integer maxAsyncJobs = Integer.parseInt(paramHelper.getValoreParamApplic(
		ConstIamParamApplic.NmParamApplic.MAX_ASYNC_JOBS.name(), null, null,
		Constants.TipoIamVGetValAppart.APPLIC));
	Integer minUsersPerList = Integer.parseInt(paramHelper.getValoreParamApplic(
		ConstIamParamApplic.NmParamApplic.MIN_USERS_PER_LIST.name(), null, null,
		Constants.TipoIamVGetValAppart.APPLIC));
	Integer maxUsersPerListOnDB = Integer.parseInt(paramHelper.getValoreParamApplic(
		ConstIamParamApplic.NmParamApplic.MAX_USERS_PER_LIST_ON_DB.name(), null, null,
		Constants.TipoIamVGetValAppart.APPLIC));

	// A priori attivo i metodi asincroni fino al numero massimo, poi vado a togliere quelli
	// bloccati (se ce ne
	// sono)
	...
	List<String> asyncJobNames = repHelper
		.getFreeAsyncJobs(Constants.NomiJob.REPLICA_UTENTI.name() + "_", "1");
	...
	if (!asyncIndexes.isEmpty()) {
	    ...
	    // Prima di iniziare, sblocco automaticamente eventuali utenti "bloccati" in fase di
	    // repliche precedenti
	    int replicheAggiornate = amministrazioneUtentiEjb
		    .sbloccaReplicheBloccate(ApplEnum.NmJob.REPLICA_UTENTI, true);
	    ...
	    log.info("Replica Utenti - ottenuti " + utentiDaReplicare.size()
		    + " utenti da replicare");
	...
	if (!utentiDaReplicare.isEmpty()) {
		...
		if (utentiDaReplicare.size() > minUsersPerList) {
		    int utentiPerAsync = (utentiDaReplicare.size() + asyncIndexes.size() - 1)
			    / asyncIndexes.size();
		    if (utentiPerAsync > maxUsersPerListOnDB) {
			utentiPerAsync = maxUsersPerListOnDB;
			log.info("Replica Utenti - Dato che gli utenti sono pi\u00F9 di "
				+ MAX_USERS_PER_LIST_ON_DB
			...
			if (from != to && from < utentiDaReplicare.size()) {
			    ...
			    if (idLogJob != null) {
				context.getBusinessObject(ReplicaUtentiEjb.class)
					.replicaUtentiAsync(asyncIndexes.get(index) + 1,
						utentiDaReplicare.subList(from, to),
		    ...
		    Long idLogJob = context.getBusinessObject(ReplicaUtentiEjb.class)
			    .updateLogUserDaReplicFKeys(utentiDaReplicare, maxUsersPerListOnDB,
				    asyncIndexes.get(0) + 1);
		    if (idLogJob != null) {
			context.getBusinessObject(ReplicaUtentiEjb.class).replicaUtentiAsync(
				asyncIndexes.get(0) + 1, utentiDaReplicare, maxUsersPerListOnDB,
				idLogJob);
		    }
		}
	    } else {
	...
	jobLoggerEjb.writeAtomicLog(Constants.NomiJob.REPLICA_UTENTI,
		Constants.TipiRegLogJob.FINE_SCHEDULAZIONE, replicaFinalMsg);
    }
	...
	if (idLogJob == null) {
	    log.error(
		    "REPLICA UTENTI --- ERRORE INASPETTATO - Impossibile scrivere la riga di log in LogJob");
	} else {
	    int replicheAggiornate = updateLogUserDaReplic(users, maxSize, idLogJob);
	    log.info(Constants.NomiJob.REPLICA_UTENTI + "_" + asyncId + " - Aggiornati "
		    + replicheAggiornate + " record di replica con l'idLogJob "
		    + idLogJob.toString());
	}
	return idLogJob;
    }
	...
	if (users.size() > maxSize) {
	    updatedLogs += recursiveLogUserDaReplicUpdate(users.subList(0, maxSize), maxSize,
		    idLogJob);
	    updatedLogs += recursiveLogUserDaReplicUpdate(users.subList(maxSize, users.size()),
		    maxSize, idLogJob);
    ...
    @Asynchronous
    public void replicaUtentiAsync(int asyncId, List<BigDecimal> users, int maxUsersPerListOnDB,
	    Long idLogJob) {
	if (users != null && !users.isEmpty()) {
	    try {
		log.info(ReplicaUtentiEjb.class.getSimpleName()
			+ " --- INIZIO chiamata asincrona per replica utenti N. " + asyncId);
		Set<String> applicErroreSet = context.getBusinessObject(ReplicaUtentiEjb.class)
			.gestioneRepliche(users, idLogJob);
		...
		log.info(ReplicaUtentiEjb.class.getSimpleName()
			+ " --- FINE chiamata asincrona per replica utenti N. " + asyncId);
	    } catch (Exception e) {
	...
	log.info("Replica Utenti - ottenute " + userDaReplicList.size() + " repliche per "
		+ users.size() + " utenti da replicare");
	...
	for (LogUserDaReplic userDaReplic : userDaReplicList) {
		...
		if (client != null) {
		    ...
		    if (userDaReplic.getTiOperReplic().equals(ApplEnum.TiOperReplic.INS.name())
			    || userDaReplic.getTiOperReplic()
				    .equals(ApplEnum.TiOperReplic.MOD.name())) {
			/*
			 * Se l'utente c'è e non è presente nell'insieme degli utenti già replicati
			 * allora posso chiamare il WS per inserirlo/modificarlo
			 */
			if (user != null && !userGiaReplic
				.contains(new UserApplic(userDaReplic.getAplApplic().getIdApplic(),
					userDaReplic.getIdUserIam()))) {
			    /*
			     * Ricavo l'insieme dei servizi a cui l'utente è abilitato sia in
			     * funzione dei ruoli di default, sia in funzione dei ruoli
			     * dell'organizzazione specificati per la dichiarazione
			     */
			    log.info("Replica Utenti - Inizio recupero servizi abilitati");
			    List<UsrVAllAutor> allServiziWebAutListATutteLeOrg = repHelper
				    .getUsrVAllAutorServiziWeb(user.getIdUserIam(),
					    userDaReplic.getAplApplic().getIdApplic());
			    log.info("Replica Utenti - Fine recupero servizi abilitati");
			    log.debug("Replica Utenti - DUMP MEMORY STATUS - UUID : "
				    + uuid.toString() + "\r\n" + dumpMemoryStatus());
			    /*
			     * Ricavo l'insieme delle organizzazioni abilitate con il flag su quale
			     * sia l'organizzazione di default per l'utente
			     */
			    log.info("Replica Utenti - Inizio recupero organizzazioni abilitate");
			    List<UsrVAbilOrganiz> abilOrganizList = repHelper
				    .getUsrVAbilOrganizList(user.getIdUserIam(),
					    userDaReplic.getAplApplic().getIdApplic());
			    log.info(
				    "Replica Utenti - Fine recupero organizzazioni abilitate - ABILITATE "
				...
				case "DEF":
				    ...
				    break;
				case "DICH":
				    Set<UsrVAllAutor> autorSet = autorDichMap
					    .get(autor.getIdOrganizApplic());
				    if (autorSet == null) {
				...
				BigDecimal idUsoUserApplic = repHelper.getIdUsoUserApplic(
					user.getIdUserIam(),
					userDaReplic.getAplApplic().getIdApplic());
				BigDecimal idOrganizIam = abilOrganiz.getUsrVAbilOrganizId()
					.getIdOrganizIam();
				BigDecimal idOrganizApplic = abilOrganiz.getIdOrganizApplic();
				String tiScopo = null;
				if (idOrganizIam != null) {
				    List<String> scopi = repHelper.getTiScopoDichAbilOrganiz(
					    idUsoUserApplic, idOrganizIam);
				...
				/*
				 * Aggiungo l’organizzazione alla lista delle organizzazioni
				 * abilitate per l’utente, riportando l’identificatore
				 * dell’organizzazione per l’applicazione
				 */
				...
				/*
				 * Inserisco l'insieme dei servizi a cui l'utente è abilitato in
				 * base ai ruoli DEF e DICH
				 */
				ListaServiziAutor listaServiziAutor = new ListaServiziAutor();
				for (UsrVAllAutor allServiziWebAut : autorDef) {
				    listaServiziAutor.getNmServizioAutor()
					    .add(allServiziWebAut.getNmAutor());
				}
			    ...
			    List<String> indIpList = repHelper
				    .getUsrIndIpUserList(user.getIdUserIam());
			    ...
			    ////////////////////////////////////////////////////////
			    if (userDaReplic.getTiOperReplic()
				    .equals(ApplEnum.TiOperReplic.INS.name())) {
			    ...
			    userGiaReplic
				    .add(new UserApplic(userDaReplic.getAplApplic().getIdApplic(),
					    userDaReplic.getIdUserIam()));
			} else {
			...
			log.info("Replica Utenti - Risposta WS positiva per l'utente "
				+ userDaReplic.getNmUserid());
		    } /* Se la risposta ha dato esito negativo */ else {
			/* Mi annoto l'applicazione che ha dato problemi */
			repHelper.writeAtomicLogUserDaReplic(userDaReplic.getIdLogUserDaReplic(),
				Constants.EsitoServizio.KO, resp.getCdErr(), resp.getDsErr(),
				applicErroreSet);
			log.error("Replica Utenti - Risposta WS negativa per l'utente "
				+ userDaReplic.getNmUserid() + " " + resp.getCdErr() + " - "
				+ resp.getDsErr());
		    }
		    ...
		    repHelper.writeAtomicLogUserDaReplic(userDaReplic.getIdLogUserDaReplic(),
			    ...
			    applicErroreSet);
		    log.error("Replica Utenti - Risposta WS negativa per l'utente "
			    + userDaReplic.getNmUserid() + " " + Costanti.SERVIZI_USR_001);
		    break;
		}
		 ...
		 */
		repHelper.writeAtomicLogUserDaReplic(userDaReplic.getIdLogUserDaReplic(),
			Constants.EsitoServizio.KO, Costanti.SERVIZI_USR_005,
			e.getFault().getFaultCode() + ": " + e.getFault().getFaultString(),
			applicErroreSet);
		log.error("Replica Utenti - Risposta WS negativa per l'utente "
			+ userDaReplic.getNmUserid() + " " + Costanti.SERVIZI_USR_005
			+ " - Utente che attiva il servizio non riconosciuto o non abilitato", e);
		break;
	    } catch (WebServiceException e) {
		/*
		 * Se non risponde, mi annoto l'applicazione che ha dato problemi e scrivo nel log
		 * dell'utente
		 */
		repHelper.writeAtomicLogUserDaReplic(userDaReplic.getIdLogUserDaReplic(),
			Constants.EsitoServizio.NO_RISPOSTA, null, null, applicErroreSet);
		log.error("Replica Utenti - Risposta WS negativa per l'utente "
			+ userDaReplic.getNmUserid() + " " + Costanti.REPLICA_UTENTE_001
			+ " - Per l'applicazione " + userDaReplic.getAplApplic().getNmApplic()
			+ " il servizio di replica non risponde", e);
		/*
		 * ...esco dal ciclo di userDaReplic e devo passare a considerare l'applicazione
		 * successiva
		 */
		break;
	    } catch (Exception e) {
		repHelper.writeAtomicLogUserDaReplic(userDaReplic.getIdLogUserDaReplic(),
			Constants.EsitoServizio.KO, Costanti.SERVIZI_USR_001,
			ExceptionUtils.getRootCauseMessage(e), applicErroreSet);
		log.error("Replica Utenti - Risposta WS negativa per l'utente "
			+ userDaReplic.getNmUserid() + " " + Costanti.SERVIZI_USR_001, e);
		break;
	    }
	} // End userDaReplic
	...
	// inizio: MEV #26627: Modifica alla replica utenti per non disattivare gli utenti che
	// accedono utilizzando SPID
	String randomPwd = RandomStringUtils.randomAlphabetic(8);
	byte[] binarySalt = PwdUtil.generateSalt();
	userHelper.resetPwd(idUserIam.longValue(),
	...
	// fine: MEV #26627: Modifica alla replica utenti per non disattivare gli utenti che
	// accedono utilizzando SPID

	/*
	 * Per ogni applicazione usata dall'utente per cui l'applicazione richiede la replica,
	 * registro una riga in logUserDaReplic
	 */
	...
	sacerLogEjb.log(param.getTransactionLogContext(), param.getNomeApplicazione(),
		...
		new BigDecimal(idUserIam), param.getNomeComponenteSoftware());
    }

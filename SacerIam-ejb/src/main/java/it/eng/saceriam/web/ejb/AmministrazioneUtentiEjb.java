...
public class AmministrazioneUtentiEjb {
    ...
    public PrfRuoloTableBean getPrfRuoliUtenteTableBean(BigDecimal idApplic, String tipoEnte,
	    String tipoUtente) {
	PrfRuoloTableBean ruoliTB = new PrfRuoloTableBean();
	List<PrfRuolo> ruoliList = amministrazioneUtentiHelper.getPrfRuoliUtente(idApplic, tipoEnte,
		tipoUtente);
    ...
    public PrfRuoloTableBean getPrfRuoliTableBean(String nmApplic,
	    Set<BigDecimal> idRuoliExcluded) {
	PrfRuoloTableBean ruoliTB = new PrfRuoloTableBean();
	...
	PrfRuoloTableBean ruoliTB = new PrfRuoloTableBean();
	List<PrfRuolo> ruoliList = amministrazioneUtentiHelper.getPrfRuoli(nmApplic);
	List<PrfRuolo> ruoliDefaultList = amministrazioneUtentiHelper
		.getRuoliDefaultUtenteApplic(nmApplic, idUserIam);
	try {
	    if (ruoliList != null && !ruoliList.isEmpty()) {
    ...
    /**
     *
     * @param idUsoRuoloSet lista id ruolo distinti
     * @param tiDichAutor   autore
	...
	List<PrfVLisDichAutor> dichAutorList = amministrazioneUtentiHelper
		.getPrfDichAutorList(idUsoRuoloSet, tiDichAutor, idApplic);
	...
	try {
		...
		organizAbilitateTB = (UsrVAbilOrganizTableBean) Transform
			.entities2TableBean(organizAbilitateList);
	    }
	} catch (Exception e) {
	    log.error(e.getMessage(), e);
	}
    ...
    public UsrVTreeOrganizIamTableBean getUsrVTreeOrganizIamNoLastLevelTableBean(long idUserIam,
	    BigDecimal idApplic) {
	...
	UsrVTreeOrganizIam treeOrganizIam = amministrazioneUtentiHelper
		.getUsrVTreeOrganizIam(idUserIam);
	UsrVTreeOrganizIamRowBean treeOrganizIamRowBean = new UsrVTreeOrganizIamRowBean();
	try {
	    treeOrganizIamRowBean = (UsrVTreeOrganizIamRowBean) Transform
		    .entity2RowBean(treeOrganizIam);
	} catch (Exception e) {
	    log.error(e.getMessage(), e);
	}
    ...
    public BaseTable getUsrVTreeOrganizIamAllOrgChildTableBean(long idUserIamCor,
	    BigDecimal idApplic, String tipoEnte, BigDecimal idEnte, String flAbilOrganizAutom,
	    String flAbilOrganizEntiAutom, BigDecimal idRichGestUser) {
	List<Object[]> dichAutorList = amministrazioneUtentiHelper
		.getUsrVTreeOrganizIamAllOrgChildList(idUserIamCor, idApplic, tipoEnte, idEnte,
			flAbilOrganizAutom, flAbilOrganizEntiAutom, idRichGestUser);
    ...
    public BaseTable getUsrVTreeOrganizIamUnaOrgTableBean(long idUserIamCor, BigDecimal idApplic,
	    String tipoEnte, BigDecimal idEnte, String flAbilOrganizAutom,
	    String flAbilOrganizEntiAutom, BigDecimal idRichGestUser) {
	List<Object[]> dichAutorList = amministrazioneUtentiHelper.getUsrVTreeOrganizIamUnaOrgList(
		idUserIamCor, idApplic, tipoEnte, idEnte, flAbilOrganizAutom,
    ...
    public UsrVAbilOrganizNolastLivTableBean getUsrVAbilOrganizNolastLivTableBean(long idUserIam,
	    BigDecimal idApplic, String dlPathIdOrganizIam) {
    ...
    public UsrVAbilOrganizTableBean getUsrVAbilOrganizLastLevelTableBean(long idUserIam,
	    BigDecimal idApplic) {
	List<UsrVAbilOrganiz> dichAutorList = amministrazioneUtentiHelper
		.getUsrVAbilOrganizLastLevelList(idUserIam, idApplic);
    ...
    public UsrVAbilOrganizTableBean getOrganizPerRuoliSpecificiUnaOrg(long idUserIam,
	    BigDecimal idApplic, String dlPathIdOrganizIam) {
	List<UsrVAbilOrganiz> dichAutorList = amministrazioneUtentiHelper
		.getUsrVAbilOrganizLastLevelList(idUserIam, idApplic, dlPathIdOrganizIam);
	UsrVAbilOrganizTableBean dichsTB = new UsrVAbilOrganizTableBean();
	try {
	    if (dichAutorList != null && !dichAutorList.isEmpty()) {
		dichsTB = (UsrVAbilOrganizTableBean) Transform.entities2TableBean(dichAutorList);
	...
	List<UsrVTreeOrganizIam> dichAutorList = amministrazioneUtentiHelper
		.getOrganizIamChilds(idOrganizIam);
	UsrVTreeOrganizIamTableBean dichsTB = new UsrVTreeOrganizIamTableBean();
	try {
	    if (dichAutorList != null && !dichAutorList.isEmpty()) {
		dichsTB = (UsrVTreeOrganizIamTableBean) Transform.entities2TableBean(dichAutorList);
     ...
     *
     *
     * @param idEntryMenu id entry menu
     *
	...
	List<UsrOrganizIam> organizs = amministrazioneUtentiHelper
		.getUsrOrganizIamChilds(idEntryMenu);
	try {
	...
	List<AplEntryMenu> entries = amministrazioneUtentiHelper
	...
	try {
		...
		usoRuoloApplicRowBean = (PrfUsoRuoloApplicRowBean) Transform
	...
	List<PrfDichAutor> dichs = amministrazioneUtentiHelper.getPrfDichAutor(tiScopoDichAutor,
     ...
     *
     * @param idPaginaWeb l'id della pagina web
     *
     ...
     *
     * @param idApplicazione id applicazione
     * @param idPaginaWeb    id pagina
     *
	...
	AplAzionePaginaTableBean actionsTb = new AplAzionePaginaTableBean();
	List<AplAzionePagina> actions = amministrazioneUtentiHelper.getActionsList(idApplicazione,
		idPaginaWeb);
	try {
	    if (actions != null && !actions.isEmpty()) {
		actionsTb = (AplAzionePaginaTableBean) Transform.entities2TableBean(actions);
    ...
    @Deprecated
    public AplEntryMenuTableBean getEntryMenuList(BigDecimal idApplicazione,
	    boolean ricercaFoglie) {
	AplEntryMenuTableBean entriesTB = new AplEntryMenuTableBean();
	List<AplEntryMenu> entries = amministrazioneUtentiHelper.getEntryMenuList(idApplicazione,
		ricercaFoglie);
	try {
	    if (entries != null && !entries.isEmpty()) {
		entriesTB = (AplEntryMenuTableBean) Transform.entities2TableBean(entries);
    ...
    public UsrVLisUserTableBean getUsrVLisUserTableBean(FiltriUtenti filtri, Date[] dateValidate,
	    long idUserIamCorr, List<BigDecimal> idEntiConvenzionatiAmministratori) {
	UsrVLisUserTableBean userTB = new UsrVLisUserTableBean();
	try {
	    // MAC #20499
	    // Devo ricavare i filtri "corretti" per ambiente ed ente di appartenenza
	    List<BigDecimal> idAmbienteEnteAppartList = filtri.getId_amb_ente_convenz_appart()
		    .parse();
		...
		for (BigDecimal idAmbienteEnteConvenzAppart : idAmbienteEnteAppartList) {
		    ...
		    // altrimenti se non ne ho selezionati devo utilizzare tutti e soli gli enti ai
		    // quali l'utente
		    ...
		    // Se ho selezionato almeno un ente sono a posto, altrimenti aggiungo tutti e
		    // soli gli enti
	    ...
	    List<UsrVLisUser> userList = amministrazioneUtentiHelper.getUsrVLisUserList(filtri,
		    idEnteAppartDaPassareAllaQuery, dateValidate, idUserIamCorr,
	...
	List<UsrUsoUserApplic> uuuaList = amministrazioneUtentiHelper
		.getUsrUsoUserApplicList(idUserIam);
	try {
	    for (UsrUsoUserApplic uuua : uuuaList) {
		UsrUsoUserApplicRowBean uuuaRowBean = new UsrUsoUserApplicRowBean();
		uuuaRowBean.setBigDecimal("id_applic",
			new BigDecimal(uuua.getAplApplic().getIdApplic()));
		...
		uuuaRowBean.setBigDecimal("id_uso_user_applic",
			new BigDecimal(uuua.getIdUsoUserApplic()));
    ...
    public UsrUsoRuoloUserDefaultTableBean getUsrUsoRuoloUserDefaultTableBean(
	    Set<BigDecimal> uuuaSet) {
	UsrUsoRuoloUserDefaultTableBean uurudTableBean = new UsrUsoRuoloUserDefaultTableBean();
	List<UsrUsoRuoloUserDefault> uurudList = amministrazioneUtentiHelper
		.getUsrUsoRuoloUserDefaultList(uuuaSet);
	try {
		...
		uurudRowBean.setString("nm_applic",
			uurud.getUsrUsoUserApplic().getAplApplic().getNmApplic());
	...
	List<UsrDichAbilOrganiz> udaoList = amministrazioneUtentiHelper
		.getUsrDichAbilOrganizList(uuuaSet);
	try {
		...
		udaoRowBean.setString("nm_applic",
			udao.getUsrUsoUserApplic().getAplApplic().getNmApplic());
	...
	List<UsrDichAbilDati> udadList = amministrazioneUtentiHelper
		.getUsrDichAbilDatiList(uuuaSet);
	try {
		...
		udadRowBean.setString("nm_applic",
			udad.getUsrUsoUserApplic().getAplApplic().getNmApplic());
		udadRowBean.setString("nm_classe_tipo_dato",
			udad.getAplClasseTipoDato().getNmClasseTipoDato());
		if (udad.getUsrOrganizIam() != null) {
		    UsrVTreeOrganizIam tree = amministrazioneUtentiHelper.getUsrVTreeOrganizIam(
			    new BigDecimal(udad.getUsrOrganizIam().getIdOrganizIam()));
		    udadRowBean.setIdOrganizIam(
			    new BigDecimal(udad.getUsrOrganizIam().getIdOrganizIam()));
    ...
    public UsrDichAbilEnteConvenzTableBean getUsrDichAbilEnteConvenzTableBean(
	...
	List<UsrDichAbilEnteConvenz> daecList = amministrazioneUtentiHelper
		.getUsrDichAbilEnteConvenz(idUserIam);
	try {
		...
		daecRowBean.setString("nm_ambiente_ente_convenz",
			daec.getOrgAmbienteEnteConvenz() != null
				? daec.getOrgAmbienteEnteConvenz().getNmAmbienteEnteConvenz()
				: null);
		daecRowBean.setString("nm_ente_convenz",
			daec.getOrgEnteSiam() != null ? daec.getOrgEnteSiam().getNmEnteSiam()
				: null);
    ...
    public UsrDichAbilEnteConvenzTableBean getUsrDichAbilEnteConvenzUnEnteAttiviTableBean(
	...
	List<UsrVLisStatoUser> statoUserList = amministrazioneUtentiHelper
		.getUsrVLisStatoUserList(idUserIam);
	try {
	    if (statoUserList != null && !statoUserList.isEmpty()) {
		statoUserTableBean = (UsrVLisStatoUserTableBean) Transform
			.entities2TableBean(statoUserList);
	    }
	} catch (Exception e) {
	    log.error(e.getMessage(), e);
	}
	...
	List<AplClasseTipoDato> listaClasse = amministrazioneUtentiHelper
		.getAplClasseTipoDatoList(idApplic);
	try {
	    if (listaClasse != null && !listaClasse.isEmpty()) {
		classeTipoDatoTableBean = (AplClasseTipoDatoTableBean) Transform
			.entities2TableBean(listaClasse);
	    }
	} catch (Exception e) {
	    log.error(e.getMessage(), e);
	}
    ...
    public UsrVAbilDatiTableBean getUsrVAbilDatiTableBean(long idUserIam, String nmClasseTipoDato,
	    BigDecimal idApplic, BigDecimal idOrganizIam) {
	UsrVAbilDatiTableBean usrTipoDatoIamTableBean = new UsrVAbilDatiTableBean();
	List<UsrVAbilDati> listaTipoDato = amministrazioneUtentiHelper
		.getUsrVAbilDatiList(idUserIam, nmClasseTipoDato, idApplic, idOrganizIam);
	try {
	    if (listaTipoDato != null && !listaTipoDato.isEmpty()) {
		usrTipoDatoIamTableBean = (UsrVAbilDatiTableBean) Transform
			.entities2TableBean(listaTipoDato);
	    }
	} catch (Exception e) {
	    log.error(e.getMessage(), e);
	}
    ...
    public UsrTipoDatoIamTableBean getUsrTipoDatoIamTableBean(BigDecimal idApplic,
	    String nmClasseTipoDato, BigDecimal idOrganizIam) {
	UsrTipoDatoIamTableBean usrTipoDatoIamTableBean = new UsrTipoDatoIamTableBean();
	List<UsrTipoDatoIam> listaTipoDato = amministrazioneUtentiHelper
		.getUsrTipoDatoIamList(idApplic, nmClasseTipoDato, idOrganizIam);
	try {
	    if (listaTipoDato != null && !listaTipoDato.isEmpty()) {
		usrTipoDatoIamTableBean = (UsrTipoDatoIamTableBean) Transform
			.entities2TableBean(listaTipoDato);
	    }
	} catch (Exception e) {
	    log.error(e.getMessage(), e);
    ...
    public boolean checkUserExists(String nmUserId, String nmCognomeUser, String nmNomeUser,
	    String cdFisc, String dsEmail, String flContrIp) {
	return amministrazioneUtentiHelper.checkUserExists(nmUserId, nmCognomeUser, nmNomeUser,
		cdFisc, dsEmail, flContrIp);
    }
    ...
    public boolean checkModificheCampiUtente(String nmUserid, String nmCognomeUser,
	    String nmNomeUser, String flAttivo, String cdFisc, String dsEmail,
	    String dsEmailSecondaria, String flContrIp, String tipoUser) {
	...
	return !StringUtils.equals(userDB.getNmCognomeUser(), nmCognomeUser)
		...
		|| !StringUtils.equals(userDB.getCdFisc(), cdFisc)
		|| !StringUtils.equals(userDB.getDsEmail(), dsEmail)
	...
	UsrOrganizIam organizIam = amministrazioneUtentiHelper.findById(UsrOrganizIam.class,
		idOrganizIam);
     ...
     *
     * @param idDichAbilOrganiz l'id della dichiarazione di abilitazione in organizzazione
     *
	...
	List<Object[]> ruoliList = amministrazioneUtentiHelper
		.getRuoliAbilOrganizList(idDichAbilOrganiz);
	PrfRuoloTableBean ruoliTB = new PrfRuoloTableBean();
	try {
	    if (ruoliList != null && !ruoliList.isEmpty()) {
		PrfRuoloRowBean ruoloRB = new PrfRuoloRowBean();
    ...
    /**
     * Ritorna la lista di dichiarazione dei ruoli specifici relativi alla dichiarazione di
     * abilitazioni alle organizzazioni passata in ingresso
     *
     * @param idDichAbilOrganiz l'id della dichiarazione di abilitazione in organizzazione
     *
     * @return la lista dei ruoli per quella dichiarazione
     */
    public UsrVLisUsoRuoloDichTableBean getUsrVLisUsoRuoloDichTableBean(
	    BigDecimal idDichAbilOrganiz) {
	List<UsrVLisUsoRuoloDich> ruoliList = amministrazioneUtentiHelper
		.getUsrVLisUsoRuoloDichList(idDichAbilOrganiz);
    ...
    public int sbloccaReplicheBloccate(ApplEnum.NmJob nmJob, boolean soloSbloccoRepliche)
	...
	dateValidate[1] = fine;
	List<UsrVLisSched> listaSched = amministrazioneUtentiHelper.getUsrVLisSchedList(nmJob,
		dateValidate, soloSbloccoRepliche);
	...
	for (int i = 0; i < schedTableBean.size(); i++) {
	    ...
	    if (row.getTiBlocco() != null && (row.getDlMsgErr() == null
		    || !row.getDlMsgErr().equals(SBLOCCO_REPLICHE_ERR))) {
    ...
    public UsrVLisUserReplicTableBean getUsrVLisUserReplicTableBean(FiltriReplica filtri,
	    Date dataValidata, boolean isFromDettaglioUtente) throws EMFError {
	UsrVLisUserReplicTableBean replicTableBean = new UsrVLisUserReplicTableBean();
	List<UsrVLisUserReplic> listaSched = amministrazioneUtentiHelper
		.getUsrVLisUserReplicList(filtri, dataValidata, isFromDettaglioUtente);

	try {
	    if (listaSched != null && !listaSched.isEmpty()) {
		replicTableBean = (UsrVLisUserReplicTableBean) Transform
    ...
    public void attivaDisattivaUtente(LogParam param, BigDecimal idUserIam, boolean attiva)
	...
	try {
	    ...
	    /*
	     * Se sto riattivando l'utente e la password e' scaduta da piu di tre mesi da oggi
	     * l'aggiorno per evitare che venga disattivato nuovamente alla prossima replica
	     */
	    ...
	    // Se Ã¨ stato indicato il sistema versante, gestisco l'aggiornamento dei servizi
	    // erogati
    ...
    @TransactionAttribute(TransactionAttributeType.REQUIRES_NEW)
    public KeycloakMessageSent attivaUtente(LogParam param, BigDecimal idUserIam)
	    throws ParerUserError {
	// Controllo che all'utente sia associata una richiesta in stato NON_EVASA in cui sia
	// referenziata una richiesta
	// di riattivazione
	KeycloakMessageSent mex = null;
	UsrAppartUserRich richiestaDaEvadere = amministrazioneUtentiHelper.getRichiestaDaEvadere(
		idUserIam,
	...
	try {
		...
		if (!amministrazioneUtentiHelper.existAzioni(
			richiestaDaEvadere.getUsrRichGestUser().getIdRichGestUser(), "0")) {
		    richiestaDaEvadere.getUsrRichGestUser().setTiStatoRichGestUser(
		...
		sacerLogEjb.log(param.getTransactionLogContext(),
			paramHelper.getParamApplicApplicationName(), param.getNomeUtente(),
			param.getNomeAzione(), SacerLogConstants.TIPO_OGGETTO_RICHIESTA,
			BigDecimal.valueOf(
				richiestaDaEvadere.getUsrRichGestUser().getIdRichGestUser()),
			param.getNomePagina());
	    }
	...
	if (writeLog) {
	    ...
	    jobLogger.writeLog(logJob.getNmJob(), Constants.TipiRegLogJob.ERRORE,
		    SBLOCCO_REPLICHE_ERR);
	}
	return replicheAggiornate;
    }

    /**
     * Verifica se nella lista di abilitazioni organizzazioni sono giÃ  presenti dichiarazioni
     * gerarchicamente 'figlie' dell'organizzazione non di ultimo livello passata come parametro
     *
     *
     * @param idOrganizIam id organizzazione
     * @param applicazione lista elementi di tipo {@link PairAuth}
     *
     * @return true se la lista contiene giÃ  delle dichiarazioni
     */
    public boolean checkDichAutorSetOrganizForChilds(BigDecimal idOrganizIam,
    ...
    public boolean checkDichAutorSetOrganizUnaOrgForChilds(BigDecimal idOrganizIam,
	    Set<PairAuth> applicazione) {
	boolean result = false;
	UsrOrganizIamTableBean figli = getUsrOrganizIamChilds(idOrganizIam);
	if (!figli.isEmpty()) {
     ...
     *
     * @param idOrganizIam     id organizzazione
     * @param idClasseTipoDato id classe tipo dato
     * @param applicazione     lista elementi di tipo {@link PairAuth}
     *
     * @return true se la lista contiene giÃ  delle dichiarazioni
     */
    public boolean checkDichAutorTipiDatoSetOrganizForChilds(BigDecimal idOrganizIam,
	    BigDecimal idClasseTipoDato, Set<PairAuth> applicazione) {
	boolean result = false;
	UsrOrganizIamTableBean figli = getUsrOrganizIamChilds(idOrganizIam);
	if (!figli.isEmpty()) {
	    for (UsrOrganizIamRowBean row : figli) {
    ...
    public boolean checkDichAutorTipiDatoSetOrganizUnaOrgForChilds(BigDecimal idOrganizIam,
	    BigDecimal idClasseTipoDato, Set<PairAuth> applicazione) {
	boolean result = false;
	UsrOrganizIamTableBean figli = getUsrOrganizIamChilds(idOrganizIam);
	if (!figli.isEmpty()) {
    ...
    // public LogEventoLoginUserTableBean getLogEventoLoginUserTableBean(Date dtEventoDa, Date
    // dtEventoA, String
    // nmUserid,
    ...
    // throw new ParerUserError(
    // "Attenzione: Ã¨ stato selezionato di visualizzare i login di tipo LOGIN_OK e al tempo stesso
    // di escluderli dalla
    ...
    // }
    // } // Se invece non ne ho selezionato nessuno, prendo tutti i tipi possibili, eventualmente
    // escludendo il
    // // LOGIN_OK
    // else {
    // tipiEventoSet = new HashSet(Arrays.stream(ConstLogEventoLoginUser.TipoEvento.values()).map(t
    // -> t.name())
    ...
    // .retrieveLogEventoLoginUserList(dtEventoDa, dtEventoA, nmUserid, tipiEventoSet,
    // inclUtentiAutomi);
    // try {
    ...
    // }
    public LogVRicAccessiTableBean getLogVRicAccessiTableBean(Date dtEventoDa, Date dtEventoA,
	    String nmUserid, String tipoEvento, String inclUtentiAutomi, String inclLoginOK,
	    String nome, String cognome, String cf, String email, String idEsterno)
	    throws ParerUserError {
	...
	List<LogVRicAccessi> loginEventiList = amministrazioneUtentiHelper
		.retrieveLogVRicAccessiList(dtEventoDa, dtEventoA, nmUserid, tipiEventoSet,
			inclUtentiAutomi, nome, cognome, cf, email, idEsterno);
	try {
	    if (loginEventiList != null && !loginEventiList.isEmpty()) {
		loginEventiTB = (LogVRicAccessiTableBean) Transform
			.entities2TableBean(loginEventiList);
	    }
	} catch (Exception e) {
	    log.error(e.getMessage(), e);
	    throw new ParerUserError("Errore inatteso nel caricamento degli accessi");
    ...
    public UsrVRicRichiesteTableBean getUsrVRicRichiesteTableBean(BigDecimal idUserIamCor,
	    String nmCognomeUserAppRich, String nmNomeUserAppRich, String nmUseridAppRich,
	    BigDecimal idAmbienteEnteConvenz, BigDecimal idEnteSiam, String nmCognomeUserRich,
	    String nmNomeUserRich, String nmUseridRich, BigDecimal idOrganizIam,
	    String cdRegistroRichGestUser, BigDecimal aaRichGestUser, String cdKeyRichGestUser,
	    String cdRichGestUser, String tiStatoRichGestUser) {

	UsrVRicRichiesteTableBean ricRichiesteTB = new UsrVRicRichiesteTableBean();
	List<UsrVRicRichieste> ricRichiesteList = amministrazioneUtentiHelper
		.getUsrVRicRichiesteList(idUserIamCor, nmCognomeUserAppRich, nmNomeUserAppRich,
			...
			aaRichGestUser, cdKeyRichGestUser, cdRichGestUser, tiStatoRichGestUser);
	try {
		...
		String ambienteEnteConvenz = ricRichieste.getNmAmbienteEnteConvenz() != null
			...
			: "";
		ricRichiesteRB.setString("ente_convenzionato",
			ambienteEnteConvenz + ricRichieste.getNmEnteSiam());

		if (ricRichieste.getCdRegistroRichGestUser() != null) {
		    ricRichiesteRB.setString("identificativo_ud_rich",
			    ricRichieste.getDlCompositoOrganiz() + " - "
				    + ricRichieste.getCdRegistroRichGestUser() + " - "
		...
		ricRichiesteRB.setString("richiedente",
			ricRichieste.getNmUserRich() != null ? ricRichieste.getNmUserRich()
				: ricRichieste.getNmCognomeUserRich() + " "
					+ ricRichieste.getNmNomeUserRich());
	...
	UsrRichGestUser richGestUser = amministrazioneUtentiHelper.findById(UsrRichGestUser.class,
		idRichGestUser);
	try {
	    if (richGestUser != null) {
		richGestUserRowBean = (UsrRichGestUserRowBean) Transform
			.entity2RowBean(richGestUser);
		...
		if (richGestUserRowBean.getTiUserRich()
			.equals(ConstUsrRichGestUser.TiUserRich.UTENTE_CODIF.name())) {
		    richGestUserRowBean.setString("nm_userid_codificato",
			    richGestUser.getUsrUser().getNmCognomeUser() + " "
		    ...
		    richGestUserRowBean.setString("nm_userid_manuale",
			    richGestUser.getNmUserRich());
		}
		...
		if (richGestUser.getUsrOrganizIam() != null) {
		    ...
		    UsrVTreeOrganizIam organiz = amministrazioneUtentiHelper
			    .findViewById(UsrVTreeOrganizIam.class, idOrganizIam);
		    dlCompositoOrganiz += "(" + organiz.getDlCompositoOrganiz() + ")";
		}
		richGestUserRowBean.setString("numero_protocollo",
			StringUtils.defaultString(
				richGestUserRowBean.getString("identificativo_ud_rich"))
				+ dlCompositoOrganiz);
	    }
	} catch (Exception e) {
	    log.error(e.getMessage(), e);
	...
	try {
		...
		// Cognome, nome, userid
		if (appartUserRich.getTiAppartUserRich().equals(
			ConstUsrAppartUserRich.TiAppartUserRich.UTENTE_NON_CODIFICATO.name())) {
		    appartUserRichRowBean.setString("nm_cognome_user",
			    appartUserRich.getNmCognomeUser());
		    ...
		    appartUserRichRowBean.setString("nm_nome_user",
			    appartUserRich.getUsrUser().getNmNomeUser());
		    appartUserRichRowBean.setString("nm_userid",
			    appartUserRich.getUsrUser().getNmUserid());
		}

		if (tiStatoRichGestUser
		...
		if (appartUserRich.getOrgEnteSiam().getOrgAmbienteEnteConvenz() != null) {
		    appartUserRichRowBean.setBigDecimal("id_ambiente_ente_appart_user",
			    BigDecimal.valueOf(appartUserRich.getOrgEnteSiam()
		...
		appartUserRichRowBean.setString("nm_ente_appart_user",
			appartUserRich.getOrgEnteSiam().getNmEnteSiam());
		String nmAmbienteEnteConvenz = "";
		if (appartUserRich.getOrgEnteSiam().getOrgAmbienteEnteConvenz() != null) {
		    nmAmbienteEnteConvenz = appartUserRich.getOrgEnteSiam()
		...
		appartUserRichRowBean.setString("ente_appart_user",
			appartUserRich.getOrgEnteSiam().getNmEnteSiam());
    ...
    public UsrAppartUserRichTableBean getUsrAppartUserRichTableBean(BigDecimal idRichGestUser,
	    String tiAzioneRich, String flAzioneRichEvasa) {
	UsrAppartUserRichTableBean appartUserRichTableBean = new UsrAppartUserRichTableBean();
	List<UsrAppartUserRich> appartUserRichList = amministrazioneUtentiHelper
	...
	try {
		...
		appartUserRichTableBean = (UsrAppartUserRichTableBean) Transform
			.entities2TableBean(appartUserRichList);
	    }
	} catch (Exception e) {
	    log.error(e.getMessage(), e);
	}
	...
	UsrAppartUserRich appartUserRich = amministrazioneUtentiHelper
		.findById(UsrAppartUserRich.class, idAppartUserRich);
	try {
	    if (appartUserRich != null) {
		appartUserRichRowBean = (UsrAppartUserRichRowBean) Transform
			.entity2RowBean(appartUserRich);
		...
		if (appartUserRich.getOrgEnteSiam().getOrgAmbienteEnteConvenz() != null) {
		    appartUserRichRowBean.setBigDecimal("id_ambiente_ente_user",
			    BigDecimal.valueOf(appartUserRich.getOrgEnteSiam()
				    .getOrgAmbienteEnteConvenz().getIdAmbienteEnteConvenz()));
		}
	    }
     ...
     */
    @TransactionAttribute(TransactionAttributeType.REQUIRES_NEW)
    public void deleteUsrRichGestUser(LogParam param, BigDecimal idRichGestUser)
	    throws ParerUserError {
	...
	try {
	    UsrRichGestUser richGestUser = amministrazioneUtentiHelper
		    .findById(UsrRichGestUser.class, idRichGestUser);
	    ...
	    log.error("Errore durante la cancellazione della richiesta "
     ...
     * @param richGestUserRowBean row bean {@link UsrRichGestUserRowBean}
     * @param blob                contenuto richiesta
     *
     ...
     */
    @TransactionAttribute(TransactionAttributeType.REQUIRES_NEW)
    public Long saveRichiesta(LogParam param, UsrRichGestUserRowBean richGestUserRowBean,
     ...
     */
    @TransactionAttribute(TransactionAttributeType.REQUIRES_NEW)
    public void saveRichiesta(LogParam param, BigDecimal idRichGestUser,
	    UsrRichGestUserRowBean richGestUserRowBean, byte[] blob) throws ParerUserError {
	...
	// Controllo che non esista già  uno stesso identificativo UD, se presente, diverso da
	// quello che sto
	// modificando
	if (richGestUserRowBean.getCdRegistroRichGestUser() != null && !amministrazioneUtentiHelper
		.getUsrRichGestUserByOrganizzazioneAndUd(
			richGestUserRowBean.getBigDecimal("id_organiz_iam"),
			richGestUserRowBean.getCdRegistroRichGestUser(),
			richGestUserRowBean.getAaRichGestUser(),
			richGestUserRowBean.getCdKeyRichGestUser(), idRichGestUserExcluded)
		.isEmpty()) {
	    throw new ParerUserError(
		    "Il numero protocollo della richiesta è già associato ad un'altra richiesta");
	}

	// Controllo che non esista già  uno stesso identificativo richiesta, se esiste, per uno
	// stesso ente
	// convenzionato
	if (richGestUserRowBean.getCdRichGestUser() != null) {
	    if (!amministrazioneUtentiHelper
		    .getUsrRichGestUserByRichEnte(richGestUserRowBean.getCdRichGestUser(),
			    richGestUserRowBean.getIdEnteRich(), idRichGestUserExcluded)
		    .isEmpty()) {
		throw new ParerUserError(
			"L'identificativo della richiesta è già associato ad un'altra richiesta");
	    }
	}
	...
	if (richGestUserRowBean.getTiUserRich()
		.equals(ConstUsrRichGestUser.TiUserRich.UTENTE_CODIF.name())) {
	    UsrUser user = amministrazioneUtentiHelper.findById(UsrUser.class,
		    richGestUserRowBean.getIdUserIamRich());
	    // Controllo che l'utente riferito appartenga allo stesso ente convenzionato a cui
	    ...
	    if (richGestUserRowBean.getIdEnteRich() != null && user.getOrgEnteSiam() == null
		    || (user.getOrgEnteSiam() != null && (user.getOrgEnteSiam()
			    .getIdEnteSiam() != richGestUserRowBean.getIdEnteRich().longValue()
			    || !user.getOrgEnteSiam().getTiEnteConvenz().name()
    ...
    private void setDatiRichiesta(UsrRichGestUser richGestUser,
	    UsrRichGestUserRowBean richGestUserRowBean, byte[] blob) {
	...
	if (richGestUserRowBean.getIdEnteRich() != null) {
	    enteSiam = amministrazioneUtentiHelper.findById(OrgEnteSiam.class,
		    richGestUserRowBean.getIdEnteRich());
	}
     ...
     * @param param                 parametri per il logging
     * @param idRichGestUser        id richiesta
     * @param tiStatoRichGestUser   tipo stato richiesta
     * @param appartUserRichRowBean row bean {@link UsrAppartUserRichRowBean}
     *
     * @return id della nuova richiesta
     *
     * @throws ParerUserError errore generico
	...
	try {
		...
		updateTiStatoRichGestUser(idRichGestUser,
	    ...
	    log.error("Errore imprevisto durante il salvataggio dell'azione : "
		    + ExceptionUtils.getRootCauseMessage(ex), ex);
     ...
     * @param param                 parametri per il logging
     * @param idRichGestUser        id richiesta
     * @param idAppartUserRich      id appartenenza richiesta
     * @param appartUserRichRowBean row bean {@link UsrAppartUserRichRowBean}
     *
     * @throws ParerUserError errore generico
     */
     ...
     * @throws ParerUserError errore generico
     */
    private void controlliAzioneUtente(BigDecimal idRichGestUser,
	    UsrAppartUserRichRowBean appartUserRichRowBean, boolean isUpdate)
	    throws ParerUserError {
	...
	// MAC#30423 - Amministrazione utenti / Gestione richieste: revisione ammissibilità di più
	// azioni
	// da evadere per lo stesso utente
	//
	// è possibile inserire una azione di "Modifica abilitazioni" qualora esista già per lo
	// stesso utente una azione
	// tra:
	// - Riattivazione utente
	// - Reset password
	//
	// è possibile inserire una azione di "Riattivazione utente" qualora esista già per lo
	// stesso utente una azione
	// tra:
	...
	// - Reset password
	//
	// è possibile inserire una azione di "Reset password" qualora esista già per lo stesso
	// utente una azione tra:
	// - Modifica abilitazioni
	// - Modifica ente
	// - Riattivazione utente
	switch (azioneEnum) {
	...
	if (appartUserRichRowBean.getIdUserIam() != null) {
	    ...
	    UsrUser user = amministrazioneUtentiHelper.findById(UsrUser.class,
		    appartUserRichRowBean.getIdUserIam());
	    ...
	    // throw new ParerUserError("Utente non esistente o non appartenente al dominio per lo
	    // username
	    ...
	    // || user.getTipoUser().equals(ApplEnum.TipoUser.NON_DI_SISTEMA.name())) {
	    // throw new ParerUserError(
	    // "L'azione 'Richiesta di reset password' Ã¨ possibile solo se l'utente ha stato
	    // corrente pari ad ATTIVO e
		...
		if ((!statoUserCorrente
			.equals(ConstUsrStatoUser.TiStatotUser.ATTIVO.getDescrizione())
			&& !statoUserCorrente
				.equals(ConstUsrStatoUser.TiStatotUser.DISATTIVO.getDescrizione()))
			|| user.getTipoUser().equals(ApplEnum.TipoUser.NON_DI_SISTEMA.name())) {
		    throw new ParerUserError(
		...
		if (!statoUserCorrente
			.equals(ConstUsrStatoUser.TiStatotUser.ATTIVO.getDescrizione())
			&& !statoUserCorrente.equals(
				ConstUsrStatoUser.TiStatotUser.DISATTIVO.getDescrizione())) {
		    throw new ParerUserError(
		...
		if (existsUtenteByUseridAndTipo(appartUserRichRowBean.getNmUserid())) {
		    throw new ParerUserError(
			    "Lo username dell'utente da creare è già presente nel sistema");
		}
	    }
	}
	...
	if (!amministrazioneUtentiHelper
		.getOtherUsrAppartUserRichByUser(idRichGestUser,
			appartUserRichRowBean.getNmUserid(), appartUserRichRowBean.getIdUserIam())
		.isEmpty()) {
	    throw new ParerUserError(
	...
	appartUserRich.setUsrRichGestUser(
		amministrazioneUtentiHelper.findById(UsrRichGestUser.class, idRichGestUser));
	appartUserRich.setOrgEnteSiam(amministrazioneUtentiHelper.findById(OrgEnteSiam.class,
		appartUserRichRowBean.getIdEnteUser()));
	...
	if (appartUserRichRowBean.getIdUserIam() != null) {
	    appartUserRich.setUsrUser(amministrazioneUtentiHelper.findById(UsrUser.class,
		    appartUserRichRowBean.getIdUserIam()));
	    appartUserRich.setTiAppartUserRich(
		    ConstUsrAppartUserRich.TiAppartUserRich.UTENTE_CODIFICATO.name());
	} else {
     ...
     * @param tiAzioneRich      tipo azione
     * @param flAzioneRichEvasa flag 1/0 (true/false)
     *
     * @throws ParerUserError errore generico
     */
    @TransactionAttribute(TransactionAttributeType.REQUIRES_NEW)
    public void deleteUsrAppartUserRich(LogParam param, BigDecimal idAppartUserRich,
	    BigDecimal idUserIam, String tiAzioneRich, String flAzioneRichEvasa)
	    throws ParerUserError {
	try {
	    UsrAppartUserRich appartUserRich = amministrazioneUtentiHelper
		    .findById(UsrAppartUserRich.class, idAppartUserRich);
	    ...
	    sacerLogEjb.log(param.getTransactionLogContext(),
		    paramHelper.getParamApplicApplicationName(), param.getNomeUtente(),
		    param.getNomeAzione(), SacerLogConstants.TIPO_OGGETTO_RICHIESTA,
		    BigDecimal.valueOf(appartUserRich.getUsrRichGestUser().getIdRichGestUser()),
		    param.getNomePagina());
	    amministrazioneUtentiHelper.removeEntity(appartUserRich, true);
	    UsrRichGestUser richGestUser = amministrazioneUtentiHelper
		    .findById(UsrRichGestUser.class, idRichGestUser);
	    if (richGestUser.getUsrAppartUserRiches() != null
		    && richGestUser.getUsrAppartUserRiches().isEmpty()) {
		richGestUser.setTiStatoRichGestUser(
			ConstUsrRichGestUser.TiStatoRichGestUser.DA_COMPLETARE.name());
	    }
	} catch (Exception e) {
	    log.error("Errore durante la cancellazione dell'azione "
		    + ExceptionUtils.getRootCauseMessage(e), e);
	...
	// MEV #18372: Ricavo idOrganizApplic dai nuovi parametri NM_ENTE_UNITA_DOC_ACCORDO e
	// NM_STRUT_UNITA_DOC_ACCORDO
	...
	UsrVAbilOrganiz abilOrganiz = amministrazioneUtentiHelper
		.getUsrVAbilOrganiz(idUserIamCorrente, idOrganizApplic);
	try {
	    if (abilOrganiz != null) {
		UsrVAbilOrganizRowBean abilOrganizRB = (UsrVAbilOrganizRowBean) Transform
			.entity2RowBean(abilOrganiz);
		abilOrganizTB.add(abilOrganizRB);
	    } else {
		AplApplic applic = amministrazioneUtentiHelper.getAplApplicByName(APP_SACER);
		List<UsrVAbilOrganiz> abilOrganizList = amministrazioneUtentiHelper
			.getUsrVAbilOrganizList(idUserIamCorrente,
				BigDecimal.valueOf(applic.getIdApplic()), "STRUTTURA", null, false);
		if (!abilOrganizList.isEmpty()) {
		    abilOrganizTB = (UsrVAbilOrganizTableBean) Transform
    ...
    @TransactionAttribute(TransactionAttributeType.REQUIRES_NEW)
    public KeycloakMessageSent resetPassword(LogParam param, BigDecimal idUserIam)
	    throws ParerUserError {
	UsrAppartUserRich richiestaDaEvadere = amministrazioneUtentiHelper.getRichiestaDaEseguire(
		idUserIam,
    ...
    @TransactionAttribute(TransactionAttributeType.REQUIRES_NEW)
    public void savePassword(LogParam param, BigDecimal idUserIam, String psw)
	    throws ParerUserError {
	UsrAppartUserRich richiestaDaEvadere = amministrazioneUtentiHelper.getRichiestaDaEvadere(
		idUserIam,
		ConstUsrAppartUserRich.TiAzioneRich.RICHIESTA_RESET_PWD.getDescrizione());
	...
	ctx.getBusinessObject(AmministrazioneUtentiEjb.class).cessaUtente(param, idUserIam,
		richiestaDaEvadere);
    ...
    public void cessaUtente(LogParam param, BigDecimal idUserIam,
	...
	if (apl != null) {
	    esistonoVersamentiPing = restTemplateEjb
		    .chiamaExistsVersamentiPerUtente(idUserIam.longValueExact());
	}

	/*
	 * Se è configurato PING nel sistema e se non esistono versamenti il flag diventa Ok si può
	 * procedere altrimenti e ci sono versamenti il flag è KO non si può procedere.
	 */
	...
	UsrVChkUserCancSacer cancSacer = amministrazioneUtentiHelper
		.findViewById(UsrVChkUserCancSacer.class, idUserIam);
	boolean cancOkSacer = (cancSacer == null || cancSacer.getFlCancOk().equals("1"));
	UsrVChkUserCancIam cancIam = amministrazioneUtentiHelper
		.findViewById(UsrVChkUserCancIam.class, idUserIam);
	...
	if (user.getAplSistemaVersante() != null) {
	    authEjb.deleteUserManagingUltimoAccordoEntiConvenzionati(param, idUserIam, cancOkPing,
		    cancOkSacer, cancOkIam, richiestaDaEvadere);
	} else {
	    ctx.getBusinessObject(AmministrazioneUtentiEjb.class).deleteOrCessaUtente(param,
		    idUserIam, cancOkPing, cancOkSacer, cancOkIam, richiestaDaEvadere);
	}
    }

    @TransactionAttribute(TransactionAttributeType.REQUIRED)
    public void deleteOrCessaUtente(LogParam param, BigDecimal idUserIam, boolean cancOkPing,
	    boolean cancOkSacer, boolean cancOkIam, UsrAppartUserRich richiestaDaEvadere) {
	UsrUser user = amministrazioneUtentiHelper.findById(UsrUser.class, idUserIam);
	...
	if (!cancOkPing || !cancOkSacer || !cancOkIam || !user.getOrgEnteUserRifs().isEmpty()) {
	    ...
	    statoUser.setUsrRichGestUser(
		    (richiestaDaEvadere != null ? richiestaDaEvadere.getUsrRichGestUser() : null));
	    amministrazioneUtentiHelper.insertEntity(statoUser, true);
	    user.setIdStatoUserCor(BigDecimal.valueOf(statoUser.getIdStatoUser()));
	    user.setFlAttivo("0");
		...
		esistonoVersamentiPing = restTemplateEjb
			.chiamaExistsVersamentiPerUtente(idUserIam.longValue());
	    }
	    if (user.getTipoUser().equals(ApplEnum.TipoUser.PERSONA_FISICA.name())
	    ...
	    if (user.getTipoUser().equals(ApplEnum.TipoUser.AUTOMA.name())
		    && !amministrazioneUtentiHelper
			    .checkExistsVersamentiSacer(idUserIam.longValue())) {
		user.setAplSistemaVersante(null);
	    }
     ...
     */
    @Deprecated
    public boolean checkEsistenzaUd(BigDecimal idOrganizIam, String registro, BigDecimal anno,
	    String numero) {
	UsrOrganizIam organizIam = amministrazioneUtentiHelper.findById(UsrOrganizIam.class,
		idOrganizIam);
	return amministrazioneUtentiHelper.checkEsistenzaUd(organizIam.getIdOrganizApplic(),
		registro, anno, numero);
    }

    public UsrRichGestUserRowBean getUsrRichGestUserRowBean(BigDecimal IdOrganizIamRich,
	    String cdRegistroRich, BigDecimal aaRich, String cdKeyRich) {
	...
	List<UsrRichGestUser> usrRichGestUserByOrganizzazioneAndUd = amministrazioneUtentiHelper
		.getUsrRichGestUserByOrganizzazioneAndUd(IdOrganizIamRich, cdRegistroRich, aaRich,
			cdKeyRich, null);
	try {
		...
		richGestUserRowBean.setString("nm_ente_siam",
		...
		richGestUserRowBean.setString("nm_ente_convenz",
			richGestUser.getOrgEnteSiam().getNmEnteSiam());
		richGestUserRowBean.setString("nmAmbienteEnteCongiunti",
			richGestUser.getOrgEnteSiam().getOrgAmbienteEnteConvenz()
    ...
    public UsrRichGestUserRowBean getUsrRichGestUserRowBean(BigDecimal idEnteConvenzRich,
	    String cdRich) {
	UsrRichGestUserRowBean richGestUserRowBean = null;
	...
	try {
		...
		richGestUserRowBean
			.setBigDecimal("id_organiz_iam",
				richGestUser.getUsrOrganizIam() != null
					? BigDecimal.valueOf(
						richGestUser.getUsrOrganizIam().getIdOrganizIam())
					: null);
		richGestUserRowBean.setBigDecimal("id_ente_siam",
			BigDecimal.valueOf(richGestUser.getOrgEnteSiam().getIdEnteSiam()));
		richGestUserRowBean.setString("nm_ente_siam",
    ...
    public boolean existsAzioneUtente(BigDecimal idRichGestUser, BigDecimal idAppartUserRich,
	    List<String> tiAzioneRich, BigDecimal idUserIam, String flAzioneRichEvasa) {
	return amministrazioneUtentiHelper.existsAzioneUtente(idRichGestUser, idAppartUserRich,
		tiAzioneRich, idUserIam, null, null, null, flAzioneRichEvasa);
    }

    public boolean existsAzioneUtente(BigDecimal idRichGestUser, BigDecimal idAppartUserRich,
	    String tiAzioneRich, BigDecimal idUserIam, String flAzioneRichEvasa) {
	List<String> tiAzioneRichList = new ArrayList<>();
	tiAzioneRichList.add(flAzioneRichEvasa);
	return amministrazioneUtentiHelper.existsAzioneUtente(idRichGestUser, idAppartUserRich,
		tiAzioneRichList, idUserIam, null, null, null, flAzioneRichEvasa);
    }

    public boolean existsAzioneUtente(BigDecimal idRichGestUser, List<String> tiAzioneRich,
	    String nmCognomeUser, String nmNomeUser, String nmUserid, String flAzioneRichEvasa) {
	return amministrazioneUtentiHelper.existsAzioneUtente(idRichGestUser, null, tiAzioneRich,
		null, nmCognomeUser, nmNomeUser, nmUserid, flAzioneRichEvasa);
    }

    public BigDecimal getAzioneUtente(BigDecimal idRichGestUser, List<String> tiAzioneRich,
	    String nmCognomeUser, String nmNomeUser, String nmUserid, String flAzioneRichEvasa) {
	UsrAppartUserRich appartUserRich = amministrazioneUtentiHelper.getAzioneUtente(
		idRichGestUser, tiAzioneRich, null, nmCognomeUser, nmNomeUser, nmUserid,
		flAzioneRichEvasa);
	if (appartUserRich != null) {
    ...
    public BigDecimal getAzioneUtente(BigDecimal idRichGestUser, List<String> tiAzioneRich,
	    BigDecimal idUserIam) {
	UsrAppartUserRich appartUserRich = amministrazioneUtentiHelper
		.getAzioneUtente(idRichGestUser, tiAzioneRich, idUserIam);
	if (appartUserRich != null) {
	    return BigDecimal.valueOf(appartUserRich.getIdAppartUserRich());
	}
	return null;
    ...
    public boolean existsAzioneUtente(BigDecimal idRichGestUser, String tiAzioneRich,
	    String nmCognomeUser, String nmNomeUser, String nmUserid, String flAzioneRichEvasa) {
	List<String> tiAzioneRichList = new ArrayList<>();
	tiAzioneRichList.add(tiAzioneRich);
	return amministrazioneUtentiHelper.existsAzioneUtente(idRichGestUser, null,
		tiAzioneRichList, null, nmCognomeUser, nmNomeUser, nmUserid, flAzioneRichEvasa);
    }
    ...
    public boolean isLastRichiestaForUser(BigDecimal idUserIam, BigDecimal idRichGestUser,
	    List<String> tiAzione) {
	List<Long> usrRicGestUserByUser = amministrazioneUtentiHelper
		.getUsrRicGestUserByUser(idUserIam, tiAzione);
	...
	try {
		...
		rowBean = (UsrVVisLastRichGestUserRowBean) Transform
			.entity2RowBean(usrVVisLastRichGestUser);
	    }
	} catch (ClassNotFoundException | IllegalStateException | IllegalAccessException
		| IllegalArgumentException | InstantiationException | NoSuchMethodException
		| InvocationTargetException ex) {
	    throw new ParerUserError(
		    "Errore inatteso nel caricamento dell'ultima richiesta per l'utente");
	}
	return rowBean;
    }
	...
	String errore = null;
	AplSistemaVersante sistemaVersante = amministrazioneUtentiHelper
	...
	} else if (tipoUser.equals(ApplEnum.TipoUser.AUTOMA.name()) && !sistemiVersantiHelper
		.checkSistemaVersantePerAutoma(idSistemaVersante, idEnteAppart)) {
    ...
    public UsrVLisEntiSiamPerAzioTableBean getUsrVLisEntiSiamPerAzioTableBean(
	    BigDecimal idRichGestUser, long idUserIamCor) {
	...
	try {
		...
		entiSiamPerAzioRowBean = (UsrVLisEntiSiamPerAzioRowBean) Transform
			.entity2RowBean(enteSiamPerAzio);
		String ambiente = enteSiamPerAzio.getNmAmbienteEnteConvenz() != null
			? enteSiamPerAzio.getNmAmbienteEnteConvenz() + " / "
			: "";
		entiSiamPerAzioRowBean.setString("nome_ambiente_ente",
			ambiente + enteSiamPerAzio.getNmEnteSiam());
	...
	try {
		...
		entiSiamAppEnteRowBean = (UsrVLisEntiSiamAppEnteRowBean) Transform
			.entity2RowBean(enteSiamAppEnte);
		String ambiente = enteSiamAppEnte.getNmAmbienteEnteConvenz() != null
			? enteSiamAppEnte.getNmAmbienteEnteConvenz() + " / "
			: "";
		entiSiamAppEnteRowBean.setString("nome_ambiente_ente",
			ambiente + enteSiamAppEnte.getNmEnteSiam());
	...
	List<UsrVLisAbilOrganiz> abilOrganizList = amministrazioneUtentiHelper
		.getUsrVLisAbilOrganizList(idUserIam);
	try {
	    if (abilOrganizList != null && !abilOrganizList.isEmpty()) {
		abilOrganizTB = (UsrVLisAbilOrganizTableBean) Transform
			.entities2TableBean(abilOrganizList);
	    }
	} catch (Exception e) {
	    log.error(e.getMessage(), e);
	...
	List<UsrVLisAbilEnte> abilEnteList = amministrazioneUtentiHelper
		.getUsrVLisAbilEnteList(idUserIam);
	try {
	...
	OrgEnteSiam enteSiam = amministrazioneUtentiHelper.findById(OrgEnteSiam.class, idEnteSiam);
	return enteSiam.getTiEnteConvenz() != null && enteSiam.getTiEnteConvenz()
		.equals(ConstOrgEnteSiam.TiEnteConvenz.AMMINISTRATORE);
    }

    public boolean isEnteOrganoVigilanza(BigDecimal idEnteSiam) {
	OrgEnteSiam enteSiam = amministrazioneUtentiHelper.findById(OrgEnteSiam.class, idEnteSiam);
	return enteSiam.getTiEnteNonConvenz() != null && enteSiam.getTiEnteNonConvenz()
    ...
    public boolean existsRichiestaUtenteAzione(BigDecimal idRichGestUser, BigDecimal idUserIam,
	    String azione) {
	return amministrazioneUtentiHelper.existsRichiestaUtenteAzione(idRichGestUser, idUserIam,
		azione);
    }
    ...
    public boolean existsRichiestaUtenteDiversaAzione(BigDecimal idRichGestUser,
	    BigDecimal idUserIam, String azione) {
	return amministrazioneUtentiHelper.existsRichiestaUtenteDiversaAzione(idRichGestUser,
		idUserIam, azione);
    }

    public boolean isReferenteOk(BigDecimal idEnteSiam, BigDecimal idUserIam) {
	OrgVChkRefEnte record = amministrazioneUtentiHelper.getOrgVChkRefEnte(idEnteSiam,
		idUserIam);
	return record.getFlRefOk().equals("1");
    }

    public UsrAppartUserRichRowBean getRichiestaUtenteAzioneRowBean(BigDecimal idRichGestUser,
	    BigDecimal idUserIam, String azione) {
	UsrAppartUserRichRowBean rb = new UsrAppartUserRichRowBean();
	UsrAppartUserRich azioneRichiesta = amministrazioneUtentiHelper
		.getRichiestaUtenteAzione(idRichGestUser, idUserIam, azione);
	try {
		...
		if (azioneRichiesta.getOrgEnteSiam().getOrgAmbienteEnteConvenz() != null) {
		    rb.setBigDecimal("id_ambiente_ente_user",
			    BigDecimal.valueOf(azioneRichiesta.getOrgEnteSiam()
				    .getOrgAmbienteEnteConvenz().getIdAmbienteEnteConvenz()));
		}
	    }
	} catch (Exception e) {
     ...
     *
     * @param nmApplic        puo' valere SACER o SACER_PREINGEST
     * @param idUsoUserApplic l'identificativo che mette in relazione l'utente con l'applicazione
     *
	...
	UsrUsoUserApplic usoUserApplic = amministrazioneUtentiHelper
		.findById(UsrUsoUserApplic.class, idUsoUserApplic);
	if (usoUserApplic != null) {
		...
		if (amministrazioneUtentiHelper.getAplApplicByName(
			ConstAplApplic.NmApplic.SACER_PREINGEST.name()) != null) {
		    risposta = restTemplateEjb.chiamaExistsVersamentiPerUtente(
			    usoUserApplic.getUsrUser().getIdUserIam());
		}
		return risposta;
	    }
	}
	...
	String url = paramHelper.getValoreParamApplic(
		ConstIamParamApplic.NmParamApplic.URL_KEYCLOAK.name(), null, null,
		Constants.TipoIamVGetValAppart.APPLIC);
	String clientId = paramHelper.getValoreParamApplic(
		ConstIamParamApplic.NmParamApplic.KEYCLOAK_CLIENT_ID.name(), null, null,
		Constants.TipoIamVGetValAppart.APPLIC);

...
public class UserHelper extends GenericHelper {
	...
	Query q = getEntityManager()
		.createQuery("SELECT u FROM UsrUser u WHERE (u.nmUserid = :username)");
	...
	if (user != null) {
		...
		if (!PwdUtil.encodePassword(password).equals(user.getCdPsw())) {
		    ...
		    .encodePBKDF2Password(PwdUtil.decodeUFT8Base64String(user.getCdSalt()),
			    password)
    ...
    public void updateUserPwd(String username, String oldpassword, String password, int numOldPsw,
	    byte[] salt, Date scadenzaPwd) throws ParerUserError {
	...
	if (!user.getFlAttivo().equals("1")) {
		...
		if (idpLogger.scriviLog(logDto) == IdpLogger.EsitiLog.UTENTE_DISATTIVATO) {
		    ...
		    // Non devo dare possibilità all'utente di risalire a quale sia la causa
		    // dell'errore (user o
		    // password)
		...
		.encodePBKDF2Password(PwdUtil.decodeUFT8Base64String(user.getCdSalt()), oldpassword)
		.equals(user.getCdPsw())) {
	    logDto.setTipoEvento(LogDto.TipiEvento.BAD_PASS);
	    logDto.setDsEvento(ConstLogEventoLoginUser.DS_EVENTO_BAD_PSW_EDIT_PSW);
	    if (idpLogger.scriviLog(logDto) == IdpLogger.EsitiLog.UTENTE_DISATTIVATO) {
	...
	Query q = getEntityManager()
		.createQuery("SELECT u FROM UsrUser u WHERE u.idUserIam = :iduser");
	...
	Query q = getEntityManager()
		.createQuery("SELECT u FROM UsrUser u WHERE u.nmUserid = :nmUserid");
     ...
     * @param newExpirationDate la prossima data di scadenza della pwd
     */
    @Deprecated
	...
	String queryStr = "SELECT * FROM APL_ENTRY_MENU t0 WHERE EXISTS "
		...
		+ " WHERE ruo.id_ruolo in (" + ruoParam + ")"
		+ " AND usoruo.id_ruolo = ruo.id_ruolo AND "
    ...
    /**
     * @deprecated metodo non utilizzato, da recuperare quando la MenuUtil farò a meno della CONNECT
     *             BY PRIOR
     *
     * @param ruoli ruoli
     *
     ...
     * @param idOrgStrut id organizzazione
     *
     * @return lista di {@link PrfRuolo}
     *
     * @throws AuthorizationException eccezione
     */
    @Deprecated
    public List<PrfRuolo> getUserRoles(long idUtente, long idOrgStrut)
	...
	if (resultList.isEmpty()) {
		...
		throw new AuthorizationException(
			"Per la struttura scelta non sono definiti i ruoli dell'utente");
	    }
	}
	return resultList;
    }
     ...
     * @deprecated
     *
     * @param userRoles ruoli utente
     * @param idPagina  id pagina
     *
     ...
     * @deprecated
     *
     * @param usrUsoUserApplicList lista di uso user applic
     *
     * @return lista di {@link UsrUsoUserApplic}
     */
    @Deprecated
    public List<UsrUsoUserApplic> getUsrUsoUserApplicFiltrate(
	    List<UsrUsoUserApplic> usrUsoUserApplicList) {
    ...
    /**
     * Dati in input un insieme di applicazioni (tramite id), restituisce l'insieme "filtrato" delle
     * applicazioni non consentite (SacerIam e Dpi) non avendo valorizzato il campo dsUrlReplicaUser
     *
     * @param idApplicSet lista distinta di id applicazione
     *
     * @return lista elemnti di tipo {@link AplApplic}
     */
    public List<AplApplic> getAplApplicFiltrate(Set<BigDecimal> idApplicSet) {
	String queryStr = "SELECT aplApplic FROM AplApplic aplApplic "
		+ "WHERE aplApplic.idApplic IN (:idApplicSet) "
    ...
    /**
     * Dati in input un insieme di applicazioni (tramite id), restituisce l'insieme "filtrato"
     * dell'applicazione SacerIam
     *
     * @param idApplicSet lista distinta di id applicazione
     *
	...
	String queryStr = "SELECT DISTINCT apl FROM AplTipoOrganiz aplTipoOrganiz JOIN aplTipoOrganiz.aplApplic apl "
		+ "WHERE aplTipoOrganiz.aplApplic.idApplic IN (:idApplicSet) "
		+ "ORDER BY apl.nmApplic ";
    ...
    /**
     * Data in ingresso un'applicazione (il nome, che è univoco) restituisce un booleano che indica
     * se l'applicazione è consentita (ovvero non è SacerIam o Dpi, controllando sul campo
     * dsUrlReplicaUser)
     *
	...
	String queryStr = "SELECT aplApplic FROM AplApplic aplApplic "
		+ "WHERE aplApplic.nmApplic = :nmApplic ";
    ...
    public AplVTreeEntryMenu getAplEntryMenu(String menuName, String menuDesc, String applName)
	    throws Exception {
    ...
    /**
     * Metodo che restituisce, dati i parametri nome applicazione e nome menuEntry, la menuEntry
     * corrispondente
     *
     * @param menuName nome menu
     * @param applName nome applicazione
     *
     * @return AplEntryMenu la menuEntry corrispondente
     *
     * @throws Exception nel caso siano stati trovati più record corrispondenti
     */
     ...
     *
     * @param aplPaginaWeb entity {@link AplPaginaWeb}
     *
     ...
     * @throws Exception nel caso siano stati trovati più record corrispondenti
     */
    public AplPaginaWeb getAplPaginaWeb(String nomePagina, String nomeApplicazione)
	    throws Exception {
     ...
     *
     * @param page         entity {@link AplPaginaWeb}
     * @param descPagina   descrizione pagina
     * @param tiHelpOnline tipo help
     * @param entryMenu    entity {@link AplEntryMenu}
     */
    public void updateAplPaginaWeb(AplPaginaWeb page, String descPagina, String tiHelpOnline,
     ...
     * @param entryMenu        entity {@link AplEntryMenu}
     */
    public void insertAplPaginaWeb(String nomeApplicazione, String nomePagina, String descPagina,
	    String tiHelpOnline, AplEntryMenu entryMenu) {
    ...
    /**
     * Metodo che dato nome applicazione e una lista di pagine, elimina le pagine non comprese nella
     * lista per quell'applicazione
     *
     * @param nomeAppl nome applicazione
     * @param dati     lista dati
     */
	...
	StringBuilder deleteQry = new StringBuilder("DELETE FROM AplPaginaWeb a WHERE a.id IN (")
		.append(selectQry).append(")");
    ...
    public AplAzionePagina getAplAzionePagina(String nomeAzione, String nomePagina,
     ...
     *
     * @param action           entity {@link AplAzionePagina}
     * @param nomeApplicazione nome applicazione
     * @param nomePagina       nome pagina
     * @param descAzione       descrizione azione
     *
     * @throws Exception errore generico
     */
    public void updateAplAzionePagina(AplAzionePagina action, String nomeApplicazione,
	    String nomePagina, String descAzione) throws Exception {
     ...
     * @return pk
     */
    public Long insertAplAzionePagina(String nomeApplicazione, String nomePagina, String nomeAzione,
    ...
    /**
     * Metodo che dato nome applicazione e una lista di azioni, elimina le azioni non comprese nella
     * lista per quell'applicazione
     *
     * @param nomeAppl    nome applicazione
     * @param nomePag     nome pagina
     * @param pageActions lista azioni
     */
	...
	StringBuilder deleteQry = new StringBuilder("DELETE FROM AplAzionePagina a WHERE a.id IN (")
		.append(selectQry).append(")");
	Query q = getEntityManager().createQuery(deleteQry.toString());
    ...
    /**
     * Metodo che dato in input il nome applicazione, una pagina e una lista di azioni (prese dal
     * file CSV), seleziona le azioni su DB non comprese nella lista con tipo evento configurato
     *
     * @param nomeAppl    nome applicazione
     * @param nomePag     nome pagina
     ...
     */
    public List<AplAzionePagina> getActionsNotInCsv(String nomeAppl, String nomePag,
	...
	String queryStr = "SELECT azionePagina FROM AplAzionePagina azionePagina "
		...
		+ "AND azionePagina.nmAzionePagina NOT IN (:actions) "
		+ "AND azionePagina.aplTipoEvento IS NOT NULL ";
    ...
    /**
     * Metodo che dato in input il nome applicazione e una lista di pagine (prese dal file CSV),
     * seleziona le pagine su DB non comprese nella lista per le quali sia definita almeno una
     * azione su cui e’ definito il tipo evento di configurazione del logging
     *
     * @param nomeAppl nome applicazione
     * @param dati     lista dati
     *
	...
	String queryStr = "SELECT paginaWeb FROM AplPaginaWeb paginaWeb "
		+ "WHERE paginaWeb.aplApplic.nmApplic = :nomeappl "
		+ "AND paginaWeb.nmPaginaWeb NOT IN (:pages) "
		+ "AND EXISTS (SELECT azionePagina FROM AplAzionePagina azionePagina "
		+ "WHERE azionePagina.aplPaginaWeb = paginaWeb "
		+ "AND azionePagina.aplTipoEvento IS NOT NULL) ";
    ...
    /**
     * Metodo che dato in input il nome applicazione e una lista di menù tra quelli che potrebbero
     * essere tolti, seleziona i menù su DB della lista per i quali è definito L'HELP ON LINE
     *
     * @param nomeAppl nome applicazione
     * @param dati     lista dati potenzialmente cancellabili
     *
	...
	String queryStr = "SELECT entryMenu FROM AplEntryMenu entryMenu "
		+ "WHERE entryMenu.aplApplic.nmApplic = :nomeappl "
		+ "AND entryMenu.nmEntryMenu NOT IN (:menus) "
    ...
    /**
     * Metodo che dato in input il nome applicazione e una lista di pagine potenzialmente
     * cancellabili, seleziona le pagine per le quali è definito L'HELP ON LINE
     *
     * @param nomeAppl nome applicazione
     * @param dati     lista dati
     *
     ...
     * lista per quell'applicazione
     *
     * @param idActions lista id azioni
     */
    public void deleteActions(List<Long> idActions) {
	String queryStr = "DELETE FROM AplAzionePagina action "
		+ "WHERE action.idAzionePagina NOT IN (:idActions) ";
    ...
    public AplServizioWeb getAplServizioWeb(String nomeServizio, String nomeApplicazione)
	    throws Exception {
     ...
     *
     * @param service      entity AplServizioWeb
     * @param descServizio descrizione
     */
     ...
     *
     * @param nomeApplicazione nome applicazione
     * @param nomeServizio     nome servizio
     * @param descServizio     descrizione servizio
     */
    public void insertAplServizioWeb(String nomeApplicazione, String nomeServizio,
	    String descServizio) {
    ...
    /**
     * Metodo che dato nome applicazione e una lista di servizi web, elimina i servizi non compresi
     * nella lista per quell'applicazione
     *
     * @param nomeAppl nome applicazione
     * @param dati     lista dati
     *
	...
	StringBuilder deleteQry = new StringBuilder("DELETE FROM AplServizioWeb a WHERE a.id IN (")
		.append(selectQry).append(")");
	int numEliminati = 0;
	Query q = getEntityManager().createQuery(deleteQry.toString());
    ...
    /**
     * Metodo che restituisce una lista di menuEntry di ultimo livello (livello 3) dato il nome
     * applicazione
     *
     * @param applName nome applicazione
     *
	...
	String queryStr = "SELECT menu FROM AplEntryMenu menu "
		+ "WHERE menu.aplApplic.nmApplic = :nomeappl "
	...
	String queryStr = "SELECT menu FROM AplEntryMenu menu "
		+ "WHERE menu.aplApplic.idApplic = :idApplic "
		+ "AND menu.niLivelloEntryMenu = :livelloMenu ";
    ...
    /**
     * Metodo che restituisce una lista di pagine dato il nome applicazionee il tipo di help
     * associato alla pagina che può essere HELP_PAGINA oppure HELP_RICERCA_DIPS
     *
     * @deprecated
     *
     * @param applName     nome applicazione
    ...
    /**
     * Metodo che restituisce una lista di pagine dato il nome applicazione e il tipo di help
     * associato alla pagina che può essere HELP_PAGINA oppure HELP_RICERCA_DIPS
     *
     * @param idApplic     id applicazione
     * @param tiHelpOnLine tipo help
     ...
     * @param numOrdineEntryMenu  ordine
     * @param nomeEntryMenuPadre  entry padre
     * @param linkEntryMenu       link menu
     *
     * @throws Exception errore generico
     */
    public void insertAplEntryMenu(String nomeApplicazione, String nomeEntryMenu,
	    String descEntryMenu, String numLivelloEntryMenu, String numOrdineEntryMenu,
	    String nomeEntryMenuPadre, String linkEntryMenu) throws Exception {
     ...
     *
     * @param menu                AplEntryMenu da aggiornare
     * @param nomeApplicazione    nome applicazione
     * @param descEntryMenu       descrizione entry menu
     * @param numLivelloEntryMenu numero livello
     * @param numOrdineEntryMenu  ordine
    ...
    /**
     * Metodo che dato nome applicazione e una lista di menuEntries, elimina le entrate a menu non
     * comprese nella lista per quell'applicazione
     *
     * @param applName    nome applicazione
     * @param menuEntries lista di menuEntries
     */
	...
	StringBuilder deleteQry = new StringBuilder("DELETE FROM AplEntryMenu a WHERE a.id IN (")
		.append(selectQry).append(")");
	Query q = getEntityManager().createQuery(deleteQry.toString());
     ...
     *
     * @param ruolo     il ruolo dell'autorizzazione
     * @param dichAutor la dichiarazione di autorizzazione
     */
    ...
    /**
     * Determina le dichiarazioni di autorizzazioni per l'applicazione corrente di tipo ENTRY_MENU e
     * con scopo UNA_ABILITAZIONE, per le quali l'entrata a menù specificata non sia foglia; ogni
     * dichiarazione determinata viene aggiornata assegnando lo scopo pari a ALL_ABILITAZIONI_CHILD
     * infine elimina le dichiarazioni ridondanti
     *
    ...
    /**
     * Ritorna la lista di dichiarazioni di autorizzazione dati ruolo, dichiarazione e scopo
     * dichiarazione
     *
     * @param ruolo            entity {@link PrfUsoRuoloApplic}
     * @param tiDichAutor      autore
     * @param tiScopoDichAutor scopo
     *
     * @return Lista di dichiarazioni
     */
    public List<PrfDichAutor> getDichAutor(PrfUsoRuoloApplic ruolo, String tiDichAutor,
	    String tiScopoDichAutor) {
     ...
     *
     * @param ruolo               entity {@link PrfUsoRuoloApplic}
     * @param listAplEntryMenu    lista degli entry menu
     * @param listAplPaginaWeb    lista delle pagine
     * @param listAplAzionePagina lista delle azioni
     * @param counter             contatore delle autorizzazioni inserite
     * @param counterPages        contatore delle pagine create
     * @param listAplServizioWeb  lista servizi pagine web di tipo {@link AplPaginaWeb}
     *
     * @return il contatore counter aggiornato
     */
    public int insertAuthAllAbilitazioni(PrfUsoRuoloApplic ruolo,
	    List<AplEntryMenu> listAplEntryMenu, List<AplPaginaWeb> listAplPaginaWeb,
	    List<AplAzionePagina> listAplAzionePagina, List<AplServizioWeb> listAplServizioWeb,
     ...
     *
     * @param ruolo         entity {@link PrfUsoRuoloApplic}
     * @param dichiarazioni le dichiarazioni da autorizzare
     * @param dichAutor     il tipo di dichiarazione
     * @param counter       contatore delle autorizzazioni inserite
     * @param counterPages  contatore delle pagine inserite
     *
     * @return il contatore counter aggiornato alle nuove autorizzazioni
     */
    public int insertAuthAllAbilitazioniChild(PrfUsoRuoloApplic ruolo,
	    List<PrfDichAutor> dichiarazioni, String dichAutor, int counter,
	    MutableInt counterPages) {
	if (dichAutor.equals(ConstPrfDichAutor.TiDichAutor.ENTRY_MENU.name())) {
		...
		String queryStr = "SELECT * FROM APL_ENTRY_MENU menu "
		...
		queryStr = "SELECT * FROM APL_ENTRY_MENU entry" + " WHERE entry.id_entry_menu in"
			+ " (SELECT CONNECT_BY_ROOT nodo.id_entry_menu"
			...
			+ " AND NOT EXISTS (" + " SELECT * FROM PRF_AUTOR aut"
			+ " WHERE aut.id_uso_ruolo_applic = " + ruolo.getIdUsoRuoloApplic()
			+ " AND aut.ti_dich_autor = '"
			+ ConstPrfDichAutor.TiDichAutor.ENTRY_MENU.name() + "'"
    ...
    /**
     * Inserisce le autorizzazioni con scopo UNA_ABILITAZIONE su EntryMenu,Pagine, Azioni e Servizi
     * Web
     *
     * @param ruolo         entity {@link PrfUsoRuoloApplic}
     * @param dichiarazioni le dichiarazioni da autorizzare
     ...
     */
    public int insertAuthUnaAbilitazione(PrfUsoRuoloApplic ruolo, List<PrfDichAutor> dichiarazioni,
	...
	for (PrfDichAutor dichautor : dichiarazioni) {
	    ...
	    if (dichAutor.equals(ConstPrfDichAutor.TiDichAutor.ENTRY_MENU.name())) {
		// Inserisco le autorizzazioni per gli AVI
		String queryStr = "SELECT * FROM APL_ENTRY_MENU entry"
			+ " WHERE entry.id_entry_menu in"
			+ " (SELECT CONNECT_BY_ROOT nodo.id_entry_menu"
			+ " FROM APL_ENTRY_MENU nodo" + " WHERE nodo.id_entry_menu = "
			+ dichautor.getAplEntryMenuFoglia().getIdEntryMenu()
			+ " CONNECT BY PRIOR nodo.id_entry_menu = nodo.id_entry_menu_padre)"
			+ " AND NOT EXISTS (" + " SELECT * FROM PRF_AUTOR aut"
			+ " WHERE aut.id_uso_ruolo_applic = " + ruolo.getIdUsoRuoloApplic()
			+ " AND aut.ti_dich_autor = '"
		...
		for (AplEntryMenu menu : entries) {
		    ...
		    && !isAuthorizedAplPaginaWeb(ruolo, ConstPrfDichAutor.TiDichAutor.PAGINA.name(),
			    dichautor.getAplAzionePagina().getAplPaginaWeb())) {
		autor = new PrfAutor();
		autor.setPrfUsoRuoloApplic(ruolo);
		autor.setTiDichAutor(ConstPrfDichAutor.TiDichAutor.PAGINA.name());
		counterPages.increment();
     ...
     *
     * @param ruolo     entity {@link PrfUsoRuoloApplic}
     * @param dichAutor tipo di dichiarazione
     * @param pagina    la pagina web all'interno della dichiarazione
     *
     * @return true se esiste la dichiarazione
     */
    public boolean isAuthorizedAplPaginaWeb(PrfUsoRuoloApplic ruolo, String dichAutor,
	    AplPaginaWeb pagina) {
    ...
    /**
     * Metodo che verifica la presenza di autorizzazioni dati i ruoli e il nome del servizio web di
     * cui verificare l'autorizzazione
     *
     * @deprecated
     *
     * @param ruoli       lista elementi di tipo {@link PrfRuolo}
	...
	String queryStr = "SELECT servizioWeb FROM PrfRuolo ruoli"
		+ " JOIN ruoli.prfUsoRuoloApplics ruoloApp" + " JOIN ruoloApp.prfAutors autors"
		+ " JOIN autors.aplServizioWeb servizioWeb"
		+ " WHERE ruoli.idRuolo IN (:ruoli) AND autors.tiDichAutor = '"
		+ ConstPrfDichAutor.TiDichAutor.SERVIZIO_WEB.name()
		+ "' AND servizioWeb.nmServizioWeb = :servizioWeb";
    ...
    /**
     * Richiamato per eliminare un record da PrfRuoloCategoria quando elimino una categoria
     * associata ad un determinato ruolo
     *
     * @param idRuolo      id ruolo
     * @param tiCategRuolo tipo categoria ruolo
     */
    ...
    /**
     * Richiamato per eliminare un record da PrfUsoRuoloApplic quando elimino una applicazione
     * associata ad un determinato ruolo
     *
     * @param idRuolo  id ruolo
     * @param idApplic id applicativo
     */
	...
	String queryStr = "SELECT usoRuolo FROM PrfUsoRuoloApplic usoRuolo "
		+ "WHERE usoRuolo.prfRuolo.idRuolo = :idRuolo "
		+ "AND usoRuolo.aplApplic.idApplic = :idApplic";
	...
	if (idDichAutor != null) {
	    PrfDichAutor dich = getEntityManager().find(PrfDichAutor.class,
		    idDichAutor.longValue());
	    if (dich != null) {
	...
	if (idDichAbilOrganiz != null) {
	    UsrDichAbilOrganiz dich = getEntityManager().find(UsrDichAbilOrganiz.class,
		    idDichAbilOrganiz.longValue());
	    if (dich != null) {
		getEntityManager().remove(dich);
		getEntityManager().flush();
	    }
	...
	if (idDichAbilDati != null) {
	    UsrDichAbilDati dich = getEntityManager().find(UsrDichAbilDati.class,
		    idDichAbilDati.longValue());
	    if (dich != null) {
		getEntityManager().remove(dich);
		getEntityManager().flush();
	    }
     ...
     * @deprecated
     *
     * @param usrUsoUserApplic usr uso user applic
     */
    @Deprecated
	...
	String queryStr = "SELECT DISTINCT abil_dati.idApplic, abil_dati.dlCompositoOrganiz, abil_dati.nmApplic "
		+ "FROM UsrVAbilDati abil_dati " + "WHERE abil_dati.id.idUserIam = :idUserIam "
		+ "AND NOT EXISTS " + " (SELECT abil_org.id " + " FROM UsrVAbilOrganiz abil_org "
     ...
     */
    @Deprecated
    public UsrUsoUserApplic getUsrUsoUserApplicByUserIamApplic(BigDecimal idUserIam,
	    BigDecimal idApplic) {
     ...
     * @deprecated
     *
     * @param idUsoUsrApplic id uso user applic
     *
     * @return lista di {@link UsrUsoUserApplic}
     */
    @Deprecated
	...
	Query q = getEntityManager()
		.createQuery("SELECT u FROM UsrUsoUserApplic u WHERE u.aplApplic.nmApplic = :appl");
	q.setParameter("appl", Constants.SACERIAM);
	return q.getResultList();
    }

    public void checkAplApplicURL(String nmApplic, boolean hasBeenModified)
     ...
     *
     * @param user   l'utente da replicare
     * @param applic l'applicazione inserita/modificata/cancellata
     * @param oper   il tipo di operazione
     */
    public void registraLogUserDaReplic(UsrUser user, AplApplic applic,
	    ApplEnum.TiOperReplic oper) {
	LogUserDaReplic ur = new LogUserDaReplic();
	ur.setAplApplic(applic);
    ...
    public void registraLogUserDaReplic(BigDecimal idUserIam, BigDecimal idApplic,
	    ApplEnum.TiOperReplic oper) {
    ...
    public void registraLogUserDaReplic(BigDecimal idRuolo, AplApplic applic)
	    throws EJBTransactionRolledbackException {
    ...
    /**
     * Elimina un record da UsrIndIpUser quando elimino un indirizzo IP associato ad un determinato
     * utente
     *
     * @param idIndIpUser id indirizzo ip user
     */
    public void deleteUsrIndIpUser(BigDecimal idIndIpUser) {
	if (idIndIpUser != null) {
	    UsrIndIpUser indIpUser = getEntityManager().find(UsrIndIpUser.class,
		    idIndIpUser.longValue());
	    if (indIpUser != null) {
    ...
    /**
     * Elimina un record da UsrUsoUserApplic quando elimino una applicazione associata ad un
     * determinato utente
     *
     * @param idUsoUserApplic id applicativo
     */
    public void deleteUsrUsoUserApplic(BigDecimal idUsoUserApplic) {
	if (idUsoUserApplic != null) {
	    UsrUsoUserApplic usoUser = getEntityManager().find(UsrUsoUserApplic.class,
		    idUsoUserApplic.longValue());
	    if (usoUser != null) {
     ...
     * @deprecated
     *
     * @param idUsoRuoloUserDefaul id ruolo di default
     *
	...
	return getEntityManager().find(UsrUsoRuoloUserDefault.class,
		idUsoRuoloUserDefaul.longValue());
    }

    public List<UsrVCheckRuoloDefault> getUsrVCheckRuoloDefaultList(long idUserCorrente,
	    long idRuolo, long idApplic, String dichAutor) {
	String queryStr = "SELECT check FROM UsrVCheckRuoloDefault check "
		+ "WHERE check.id.idUserCorrente = :idUserCorrente "
		+ "AND check.id.idRuoloAggiunto = :idRuolo "
    ...
    /**
     * Ritorna la lista delle organizzazioni abilitate da una dichiarazione di abilitazione alle
     * organizzazioni appena aggiunta, che però non sono abilitate per l'utente corrente
     * (amministratore)
     *
     * @param idUserIamCorrente         id user IAM
     * @param idDichAbilOrganizAggiunta id dichiarazione
     *
     * @return la lista delle organizzazioni non abilitate per l'amministratore rispetto alla
     *         dichiarazione
     */
    ...
    public UsrUsoRuoloDich getUsrUsoRuoloDich(UsrDichAbilOrganiz dich, PrfRuolo prfRuolo,
	    UsrOrganizIam organizIamRuolo, String tiScopoRuolo) {
	Query q = getEntityManager().createQuery("SELECT r FROM UsrUsoRuoloDich r "
		+ "WHERE r.usrDichAbilOrganiz = :dich " + "AND r.prfRuolo = :ruolo "
	...
	UsrUsoRuoloDich usoRuoloDich = getEntityManager().find(UsrUsoRuoloDich.class,
		idUsoRuoloDich.longValue());
	getEntityManager().remove(usoRuoloDich);
	getEntityManager().flush();
    }

    /**
     * Metodo che ritorna la lista di un determinato tipo di dichiarazione in base al ruolo, per
     * tutte le applicazioni esclusa SACER_IAM
     *
     * @param idRuolo     l'id del ruolo
     * @param tiDichAutor il tipo di dichiarazione
     *
	...
	String queryStr = "SELECT dichAutor " + "FROM PrfVLisDichAutor dichAutor "
		+ "WHERE dichAutor.idRuolo = :idRuolo "
		+ "AND dichAutor.tiDichAutor = :tiDichAutor "
    ...
    public List<UsrVCheckRuoloDich> getUsrVCheckRuoloDichList(long idUserCorrente,
	    BigDecimal idRuolo, BigDecimal idApplic, BigDecimal idOrganizIamRuolo,
	    String dichAutor) {

	String queryStr = "SELECT check FROM UsrVCheckRuoloDich check "
		+ "WHERE check.id.idUserCorrente = :idUserCorrente "
		+ "AND check.id.idRuoloAggiunto = :idRuolo " + "AND check.idApplicDich = :idApplic "
		+ "AND check.idOrganizIamRuolo = :idOrganizIamRuolo ";
    ...
    /**
     * Per ogni applicazione usata dall'utente passato in ingresso, registra un record nel log degli
     * utenti da replicare con l'operazione passata in input
     *
     * @param username username
	...
	String queryStr = "SELECT COUNT(u) " + "FROM UsrVAllAutor u "
		+ "WHERE u.idUserIam = :idUserIam " + "AND u.tiUsoRuo = 'DEF' "
		+ "AND u.tiDichAutor = 'SERVIZIO_WEB' ";
    ...
    public List<UsrVCreaAbilDati> getUsrVCreaAbilDati(String nmUserid, String nmApplic,
    ...
    /**
     * Aggiunge le dichiarazioni di abilitazione ai tipi dato (USR_DICH_ABIL_DATI) automatiche, sia
     * inserendole come record nella tabella, sia inserendole nel tablebean da visualizzare
     * nell'online
     *
     * @param creaAbilDato     il record con i dati con cui creare la dichiarazione di abilitazione
     * @param user             entity {@link UsrUser}
     * @param usrUsoUserApplic entity {@link UsrUsoUserApplic}
     * @param tipiDato         il tablebean cui aggiungere la dichiarazione per mostrarlo poi
     *                         nell'online
     *
     * @return pk
     */
    public long aggiungiDichAbilTipoDato(UsrVCreaAbilDati creaAbilDato, UsrUser user,
	    UsrUsoUserApplic usrUsoUserApplic, UsrDichAbilDatiTableBean tipiDato) {
	...
	dichRB.setTiScopoDichAbilDati(
		creaAbilDato.getUsrVCreaAbilDatiId().getTiScopoDichAbilDati());
	//
	UsrOrganizIam organiz = null;
	if (creaAbilDato.getUsrVCreaAbilDatiId().getIdOrganizIam().compareTo(BigDecimal.ZERO) > 0) {
	    organiz = this
		    .getUsrOrganizIamById(creaAbilDato.getUsrVCreaAbilDatiId().getIdOrganizIam());
	}
	...
	if (creaAbilDato.getUsrVCreaAbilDatiId().getIdClasseTipoDato() != null) {
	    classeTipoDato = (this.getAplClasseTipoDatoById(
		    creaAbilDato.getUsrVCreaAbilDatiId().getIdClasseTipoDato()));
	}
	...
	if (dichRB.getIdSuptEstEnteConvenz() != null) {
	    dich.setOrgSuptEsternoEnteConvenz(getEntityManager()
		    .find(OrgSuptEsternoEnteConvenz.class, dichRB.getIdSuptEstEnteConvenz()));
	}
	if (dichRB.getIdVigilEnteProdut() != null) {
	    dich.setOrgVigilEnteProdut(getEntityManager().find(OrgVigilEnteProdut.class,
		    dichRB.getIdVigilEnteProdut()));
	}
	if (dichRB.getIdAppartCollegEnti() != null) {
	    dich.setOrgAppartCollegEnti(getEntityManager().find(OrgAppartCollegEnti.class,
		    dichRB.getIdAppartCollegEnti()));
	}
	...
	dichRB.setString("nm_classe_tipo_dato",
		(classeTipoDato != null ? classeTipoDato.getNmClasseTipoDato() : ""));
	if (organiz != null) {
	    UsrVTreeOrganizIam tree = this
		    .getUsrVTreeOrganizIam(new BigDecimal(organiz.getIdOrganizIam()));
    ...
    /**
     * Aggiunge le abilitazioni alle organizzazioni (USR_ABIL_ORGANIZ) ricavandole dalla relativa
     * vista
     *
     * @param idUserIam id user IAM
     * @param idApplic  di applicazione
     */
	...
	if (idOrganizIam != null) {
	    Query query = getEntityManager().createQuery("SELECT user FROM UsrVTreeOrganizIam user "
		    + "WHERE user.idOrganizIam = :idOrganizIam");
	    query.setParameter("idOrganizIam", idOrganizIam);
	    List<UsrVTreeOrganizIam> utenti = query.getResultList();
	    if (utenti.isEmpty()) {
		throw new NoResultException(
			"Nessun UsrVTreeOrganizIam trovato con idOrganizIam " + idOrganizIam);
	    }
	    return utenti.get(0);
	} else {
	...
	Query query = getEntityManager().createQuery(
		"SELECT user.tipoUser FROM UsrUser user " + "WHERE user.idUserIam = :idUserAdmin");
	...
	Query query = getEntityManager().createQuery(
		"SELECT userEnteByAbilOrg " + "FROM UsrVLisEnteByAbilOrg userEnteByAbilOrg "
     ...
     */
    @Deprecated
    public List<BigDecimal> getUtentiAbilitatiAmbienteEnteConvenz(
	    BigDecimal idAmbienteEnteConvenz) {
	Query query = getEntityManager()
		.createQuery("SELECT DISTINCT abilAmbEnteConvenz.id.idUserIam "
			+ "FROM UsrVAbilAmbEnteConvenz abilAmbEnteConvenz "
	...
	Query query = getEntityManager().createQuery("SELECT DISTINCT abilEnteConvenz.id.idUserIam "
		+ "FROM UsrVAbilEnteConvenz abilEnteConvenz "
	...
	Query query = getEntityManager()
		.createQuery("SELECT DISTINCT abilAmbEnteXente.id.idUserIam "
			+ "FROM UsrVAbilAmbEnteXente abilAmbEnteXente "
	...
	for (UsrOldPsw oldPsw : oldPswList) {
	    String saltedPassword = PwdUtil.encodePBKDF2Password(
		    PwdUtil.decodeUFT8Base64String(oldPsw.getCdSalt()), newPassword);
     ...
     * @deprecated
     *
     * @param idUserIamCor     id user
     * @param idUserIamGestito id user gestito
     *
     * @return lista di {@link UsrVAbilOrgVigilToAdd}
     */
    @Deprecated
    public List<UsrVAbilOrgVigilToAdd> getUsrVAbilOrgVigilToAddList(long idUserIamCor,
	    long idUserIamGestito) {
     ...
     * @param idUserIamGestito id user IAM gestito
     */
    ...
    public List<UsrVAbilOrgVigilToAdd> getAbilOrganizToVigil(long idUserIamCor,
	    long idUserIamGestito) {
	Query q = getEntityManager().createQuery(
     ...
     * @param idUserIamCor     id user IAM
     * @param idUserIamGestito id user IAM gestito
     * @param idEnteFornitEst  id ente
     * @param idEnteProdut     id produttore
     */
    public void aggiungiAbilEntiSuptFornit(long idUserIamCor, long idUserIamGestito,
	    long idEnteFornitEst, long idEnteProdut) {
	...
	Query q = getEntityManager().createQuery("SELECT u FROM UsrVAbilEnteSupportoToAdd u "
		+ "WHERE u.id.idUserIamCor = :idUserIamCor "
		+ "AND u.id.idUserIamGestito = :idUserIamGestito "
		+ "AND u.idEnteFornitEst = :idEnteFornitEst "
		+ "AND u.id.idEnteConvenz = :idEnteProdut ");
	q.setParameter("idUserIamCor", bigDecimalFrom(idUserIamCor));
	q.setParameter("idUserIamGestito", bigDecimalFrom(idUserIamGestito));
	...
	// Scorro la lista e se il record non è già presente nella tabella delle abilitazioni, lo
	// inserisco
	lista.forEach(u -> {
		...
		abilEnteSiam.setOrgSuptEsternoEnteConvenz(this
			.findById(OrgSuptEsternoEnteConvenz.class, u.getIdSuptEstEnteConvenz()));
	...
	Query q = getEntityManager().createQuery("SELECT u FROM UsrAbilEnteSiam u "
		+ "WHERE u.usrUser.idUserIam = :idUserIamGestito "
		+ "AND u.orgEnteSiam.idEnteSiam = :idEnteProdut");
	q.setParameter("idUserIamGestito", idUserIamGestito);
     ...
     *
     * @param idUserIamCor          id user IAM
     * @param idCollegEntiConvenz   id del collegamento dal quale ricavare le abilitazioni da
     *                              assegnare agli utenti
     * @param idEnteConvenzExcluded id dell'ente inserito nel collegamento che non deve essere
     *                              considerato
     */
    public void aggiungiAbilEnteColleg(long idUserIamCor, long idCollegEntiConvenz,
	    long idEnteConvenzExcluded) {
	Query q = getEntityManager().createNativeQuery(
		"INSERT INTO USR_ABIL_ENTE_SIAM (id_abil_ente_siam, id_user_iam, id_ente_siam, ds_causale_abil, id_appart_colleg_enti, fl_abil_automatica) "
			+ "SELECT to_number('" + randInt()
			...
			+ "AND appart.id_Colleg_Enti_Convenz = ? "
			+ "AND utente.fl_Abil_Enti_Colleg_Autom = '1' "
     ...
     *
     * @param idUserIam         id user IAM dell'utente interessato alla rimozione dell'abilitazione
     * @param idVigilEnteProdut id della vigilanza (ORG_VIGIL_ENTE_PRODUT)
     */
     ...
     *
     * @param idUsoUserApplic id user su applicazione
     ...
     *
     * @param idUsoUserApplic id user IAM
     * @param idTipoDatoIam   id ente
     *
     * @return true se abilitato
     */
    public boolean isAbilEntiConvDati(long idUsoUserApplic, long idTipoDatoIam) {
	String queryStr = "SELECT abilDati FROM UsrAbilDati abilDati "
		+ "JOIN abilDati.usrUsoUserApplic usoUserApplic "
     ...
     */
    @Deprecated
    public UsrDichAbilEnteConvenz retrieveDichAbilEnteConv(long idUserIam,
	    long idAmbienteEnteConvenz, long idEnteSiam) {
	...
	Query query = getEntityManager()
		.createQuery("SELECT abilOrganiz FROM UsrAbilOrganiz abilOrganiz "
	...
	Query query = getEntityManager().createQuery(
		"SELECT DISTINCT abilEnte.id.idUserIamGestito FROM UsrVLisAbilEnte abilEnte "
	...
	Query query = getEntityManager()
		.createQuery("SELECT dich.usrUser.idUserIam, dich.idDichAbilEnteConvenz "
			+ "FROM UsrDichAbilEnteConvenz dich "
			+ "WHERE dich.tiScopoDichAbilEnte = 'UN_AMBIENTE' "
    ...
    public void deleteAbilitazioniUtenteUnAmbiente(long idEnteSiam, long idUserIam,
	    long idDichAbilEnteConvenz) {
	...
	Query q = getEntityManager().createQuery("SELECT user.nmUserid "
		+ "FROM UsrVAbilEnteCollegToDel abilEnteCollegToDel, UsrUser user "
	...
	Query q = getEntityManager().createQuery("SELECT DISTINCT user.nmUserid "
		+ "FROM UsrVAbilEnteCollegToDel abilEnteCollegToDel, UsrUser user "

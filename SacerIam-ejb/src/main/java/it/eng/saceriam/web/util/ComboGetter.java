...
public class ComboGetter {
    ...
    private static BaseRow createKeyValueBaseRow(String key, String value, String key2,
	    String value2) {
	...
	DecodeMap mappaTipo = new DecodeMap();
	String key = "ti_stato_replic";
	for (ApplEnum.TiStatoReplicForSched stato : sortEnum(
		ApplEnum.TiStatoReplicForSched.values())) {
	    bt.add(createKeyValueBaseRow(key, stato.name()));
	}
	mappaTipo.populatedMap(bt, key, key);
	return mappaTipo;
	...
	if (isEnteAmministratore) {
	    for (ApplEnum.TiScopoDichAbilOrganiz abil : sortEnum(
		    ApplEnum.TiScopoDichAbilOrganiz.values())) {
	...
	String key = "dich";
	if (isEnteAmministratore) {
	    for (ApplEnum.TiScopoDichAbilDati abil : sortEnum(
		    ApplEnum.TiScopoDichAbilDati.values())) {
		bt.add(createKeyValueBaseRow(key, abil.name()));
	    }
	} else {
	    for (ApplEnum.TiScopoDichAbilDati abil : sortEnum(
		    ApplEnum.TiScopoDichAbilDati.getComboScopiNonAllOrg())) {
		bt.add(createKeyValueBaseRow(key, abil.name()));
	    }
	}
	mappaDich.populatedMap(bt, key, key);
	...
	for (ConstUsrStatoUser.TiStatotUser tiStatoUser : sortEnum(
	    ...
	    bt.add(createKeyValueBaseRow(key, tiStatoUser.name(), key2,
		    tiStatoUser.getDescrizione()));
	}
	...
	DecodeMap mappaTipo = new DecodeMap();
	String key = "ti_classe_tipo_servizio";
	for (ConstOrgTipoServizio.TipoFatturazione tipo : sortEnum(
		ConstOrgTipoServizio.TipoFatturazione.values())) {
	...
	DecodeMap mappaTipo = new DecodeMap();
	String key = "tipo_tariffa";
	for (ConstOrgTariffa.TipoTariffa tipo : sortEnum(
		ConstOrgTariffa.TipoTariffa.getTipoTariffaSoloValoreFisso())) {
	    bt.add(createKeyValueBaseRow(key, tipo.name()));
	}
	mappaTipo.populatedMap(bt, key, key);
	return mappaTipo;
	...
	for (ConstOrgCollegEntiConvenz.TiColleg tipo : sortEnum(
		ConstOrgCollegEntiConvenz.TiColleg.values())) {
	    bt.add(createKeyValueBaseRow(key, tipo.name()));
	}
	mappaTipo.populatedMap(bt, key, key);
	return mappaTipo;
	...
	for (ConstOrgAccordoEnte.TiScopoAccordo scopo : sortEnum(
		ConstOrgAccordoEnte.TiScopoAccordo.values())) {
	...
	for (ConstPrfRuolo.TiCategRuolo categRuolo : sortEnum(
		ConstPrfRuolo.TiCategRuolo.values())) {
	...
	for (ConstUsrRichGestUser.TiRichGestUser tipo : sortEnum(
		ConstUsrRichGestUser.TiRichGestUser.values())) {
	...
	if (!scopoDichAbilOrganiz.equals(ActionEnums.ScopoDichAbilOrganiz.UNA_ORG.name())) {
	    bt.add(createKeyValueBaseRow(key,
	...
	for (ConstLogEventoLoginUser.TipoEvento tipoEvento : sortEnum(
		ConstLogEventoLoginUser.TipoEvento.values())) {
	...
	DecodeMap mappaTipo = new DecodeMap();
	String key = "ti_categ_ruolo";
	for (ApplEnum.TipoUser categ : sortEnum(

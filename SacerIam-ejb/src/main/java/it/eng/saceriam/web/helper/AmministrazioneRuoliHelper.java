...
public class AmministrazioneRuoliHelper extends GenericHelper {
    ...
    /**
     * Ritorno i ruoli di un utente in base all'applicazione La vista gestisce il "raggruppamento"
     * dei ruoli che gestiscono più applicazioni
     *
     * @param nmApplics       lista distinta nome applicazioni
     * @param tiStatoAllinea1 tipo stato
     * @param tiStatoAllinea2 tipo stato
     * @param nmRuolo         nome ruolo
     * @param tiRuolo         tipo ruolo
     * @param tiCategRuolo    categoria
     *
     * @return lista elementi di tipo {@link PrfVLisRuolo}
     */
    public List<PrfVLisRuolo> getPrfVLisRuoloList(Set<String> nmApplics, String tiStatoAllinea1,
            String tiStatoAllinea2, String nmRuolo, String tiRuolo, String tiCategRuolo) {
    ...
    /**
     * Metodo che ritorna la lista di dichiarazioni in base al ruolo e al tipo dichiarazione dati
     * come parametri
     *
     * @param idUsoRuoloSet la collezione di idUsoRuoloApplic
     * @param idApplic      id applicazione
     * @param tiDichAutor   il tipo di dichiarazione
     *
     * @return la lista di dichiarazioni
     */
    public List<PrfVLisDichAutor> getPrfVLisDichAutorList(Set idUsoRuoloSet, String tiDichAutor,
            BigDecimal idApplic) {
    ...
    public List<PrfVLisDichAutor> getPrfVLisDichAutorList(BigDecimal idRuolo, String tiDichAutor,
            BigDecimal idApplic) {
        String queryStr = "SELECT usoruoli " + "FROM PrfVLisDichAutor usoruoli "
                + "WHERE usoruoli.idRuolo = :idruolo " + "AND usoruoli.tiDichAutor = :dichautor ";
     ...
     *
     * @param usoRuoloApplicRowBean uso ruolo
     * @param tiDichAutor           tipo dich autore
     *
     * @return voci di menu delle abilitazioni
     */
    @Deprecated
    public List<Object[]> getAbilitazioniEntryMenu4Applic(
            PrfUsoRuoloApplicRowBean usoRuoloApplicRowBean, String tiDichAutor) {
        return getAbilitazioniEntryMenu4Applic(usoRuoloApplicRowBean);
    }

    /**
     * @param usoRuoloApplicRowBean uso Ruolo applicazione
     *
     * @return voci di menu delle abilitazioni
     */
    public List<Object[]> getAbilitazioniEntryMenu4Applic(
            PrfUsoRuoloApplicRowBean usoRuoloApplicRowBean) {
    ...
    /**
     * @param idUsoRuoloApplic is uso ruolo
     * @param nmApplic         nome applicazione
     * @param idRuolo          id ruolo
     *
     * @return voci di menu delle abilitazioni
     */
    public List<Object[]> getAbilitazioniEntryMenu4Applic(BigDecimal idUsoRuoloApplic,
            String nmApplic, BigDecimal idRuolo) {
        ...
        String nativeQuery = "SELECT dich_padre.id_dich_autor as id_dich_autor_padre, dich_foglia.id_dich_autor, "
                ...
                + "and dich_padre.id_uso_ruolo_applic = :idUsoRuoloApplic ) "
                + "left join prf_dich_autor dich_foglia "
                + "on (dich_foglia.id_entry_menu_foglia = tree.id_entry_menu "
                + "and dich_foglia.id_uso_ruolo_applic = :idUsoRuoloApplic ) "
                + "where id_entry_menu in ( " + "select id_entry_menu "
                + "from PRF_V_LIS_DICH_AUTOR " + "where id_ruolo = :idRuolo "
     ...
     * @param idApplic        id applicazione
     * @param ricercaFoglie   ricerca
     * @param nodiGiaPresenti lista nodi
     *
     * @return il tableBean della lista di entry menu
     */
     ...
     *
     * @param idAzionePagina id azione per pagina
     *
     ...
     *
     * @param idApplicazione  id applicazione
     * @param nodiGiaPresenti nodi presenti
     *
    ...
    public List<AplPaginaWeb> getPagesList(BigDecimal idApplicazione, Set<String> nodiGiaPresenti) {
        StringBuilder queryStr = new StringBuilder("SELECT pag " + "FROM AplPaginaWeb pag "
                + "WHERE pag.aplApplic.idApplic = :idappl ");
        if (!nodiGiaPresenti.isEmpty()) {
     ...
     *
     * @param idApplicazione    id applicazione
     * @param azioniGiaPresenti lista azioni presenti
     *
     * @return il tableBean della lista di pagine
     */
    public List<AplPaginaWeb> getPagesAzioniList(BigDecimal idApplicazione,
            Set<BigDecimal> azioniGiaPresenti) {
        StringBuilder queryStr = new StringBuilder("SELECT DISTINCT pag "
                + "FROM AplAzionePagina azio " + "JOIN azio.aplPaginaWeb pag "
                + "WHERE pag.aplApplic.idApplic = :idappl ");
     ...
     * @param idApplicazione    id applicazione
     * @param idPaginaWeb       id pagina
     * @param azioniGiaInserite lista azioni inserite
     *
     * @return il tableBean della lista di azioni
     */
    public List<AplAzionePagina> getActionsPagesList(BigDecimal idApplicazione,
            BigDecimal idPaginaWeb, Set<BigDecimal> azioniGiaInserite) {
        String queryStr = "SELECT azio " + "FROM AplAzionePagina azio "
                + "JOIN azio.aplPaginaWeb pag " + "WHERE pag.aplApplic.idApplic = :idappl "
                + "AND pag.idPaginaWeb = :idpagina ";
    ...
    public List<AplServizioWeb> getServicesList(BigDecimal idApplicazione,
            Set<String> nodiGiaPresenti) {
        StringBuilder queryStr = new StringBuilder("SELECT serv " + "FROM AplServizioWeb serv "
                + "WHERE serv.aplApplic.idApplic = :idappl ");
        if (!nodiGiaPresenti.isEmpty()) {
    ...
    public List<UsrUser> getUtentiUsoRuoloUserDefaultList(BigDecimal idRuolo,
            List<String> tiStatoUser) {
        StringBuilder queryStr = new StringBuilder(
                "SELECT uuua.usrUser FROM UsrUsoUserApplic uuua, UsrStatoUser statoUser "
                        + "JOIN uuua.usrUsoRuoloUserDefaults uurud "
                        + "WHERE uurud.prfRuolo.idRuolo = :idRuolo "
    ...
    public List<UsrUser> getUtentiUsoRuoloOrganizList(BigDecimal idRuolo,
            List<String> tiStatoUser) {
        StringBuilder queryStr = new StringBuilder(
    ...
    public Long countMenusList(BigDecimal idApplic) {
        Query query = getEntityManager().createQuery(
                "SELECT COUNT(apl) FROM AplEntryMenu apl WHERE apl.aplApplic.idApplic = :idApplic");
    ...
    public Long countPagesList(BigDecimal idApplic) {
        Query query = getEntityManager().createQuery(
                "SELECT COUNT(pag) FROM AplPaginaWeb pag WHERE pag.aplApplic.idApplic = :idApplic ");
        query.setParameter("idApplic", longFrom(idApplic));
        return (Long) query.getSingleResult();
    }
    ...
    public PrfUsoRuoloApplic getPrfUsoRuoloApplic(long idRuolo, long idAppl) {
        String queryStr = "SELECT usoruoli FROM PrfUsoRuoloApplic usoruoli "
                + "JOIN usoruoli.prfRuolo ruoli " + "WHERE usoruoli.aplApplic.idApplic = :idappl "
                + "AND ruoli.idRuolo = :idrole";
        Query q = getEntityManager().createQuery(queryStr);
    ...
    public List<PrfVLisUserDaReplic> retrievePrfVLisUserDaReplic(Long idRuolo) {
        Query q = getEntityManager().createQuery(
                "SELECT prf FROM PrfVLisUserDaReplic prf WHERE prf.idRuolo = :idRuolo");
        q.setParameter("idRuolo", bigDecimalFrom(idRuolo));
        return q.getResultList();
    }

    /**
     * Esegue una bulk delete sulle dichiarazioni di abilitazione per il ruolo idRuolo sulla base
     * della vista PrfVLisDichAllineaTodel
     *
     * @param idRuolo id ruolo
     *
    ...
    public List<PrfVLisDichAllineaToadd> retrievePrfVLisDichAllineaToadd(Long idRuolo) {
        Query q = getEntityManager().createQuery(
                "SELECT prf FROM PrfVLisDichAllineaToadd prf WHERE prf.idRuolo = :idRuolo");
        q.setParameter("idRuolo", bigDecimalFrom(idRuolo));
        return q.getResultList();
    }
    ...
    public int deletePrfAllineaRuolo(Long idRuolo, String nmApplic) {
        Query q = getEntityManager().createQuery(
                "DELETE FROM PrfAllineaRuolo allineaRuolo WHERE allineaRuolo.prfRuolo.idRuolo = :idRuolo"
                        + (StringUtils.isNotBlank(nmApplic)
                                ? " AND allineaRuolo.nmApplic = :nmApplic"
                                : ""));
        q.setParameter("idRuolo", idRuolo);
    ...
    public Long countPrfAllineaRuolo(Long idRuolo) {
        return getEntityManager().createQuery(
                "SELECT COUNT(prf) FROM PrfAllineaRuolo prf WHERE prf.prfRuolo.idRuolo = :idRuolo",
                Long.class).setParameter("idRuolo", idRuolo).getSingleResult();
    }
    ...
    @Deprecated
    public List<PrfRuoloCategoria> getPrfRuoloCategoriaList(BigDecimal idRuolo,
            List<String> tiCategRuolo) {
        String queryStr = "SELECT ruoloCategoria FROM PrfRuoloCategoria ruoloCategoria "
                + "WHERE ruoloCategoria.prfRuolo.idRuolo = :idRuolo ";

        if (tiCategRuolo != null && !tiCategRuolo.isEmpty()) {
            queryStr = queryStr
                    + "AND (SELECT ruoloCategoria2 FROM PrfRuoloCategoria ruoloCategoria2 "
    ...
    /**
     * Restituisce la voce di menu principale AplEntryMenu associata a una specifica applicazione e
     * ruolo. La voce principale è identificata come quella senza entryMenuPadre (radice del menu).
     *
     * @param idApplic l'ID dell'applicazione di riferimento
     * @param idRuolo  l'ID del ruolo per cui recuperare la voce di menu principale
     * @return lista contenente la voce di menu principale abilitata per il ruolo e l'applicazione
     *         specificati
     */
    public List<AplEntryMenu> findVoceMenuPrincipalePerAppERuolo(BigDecimal idApplic,
            BigDecimal idRuolo) {
        String queryStr = "SELECT DISTINCT e FROM AplEntryMenu e " + "JOIN e.aplApplic app "
                + "JOIN e.prfAutors aut " + "JOIN aut.prfUsoRuoloApplic uso "
                + "JOIN uso.prfRuolo ruo " + "WHERE app.idApplic = :idApplic "
                + "AND   ruo.idRuolo = :idRuolo " + "AND   e.entryMenuPadre IS NULL";
        Query query = getEntityManager().createQuery(queryStr);
        query.setParameter("idApplic", idApplic.longValueExact());
        query.setParameter("idRuolo", idRuolo.longValueExact());
        return query.getResultList();
    }

    /**
     * Restituisce la lista delle azioni AplAzionePagina associate a una specifica applicazione e
     * ruolo, escludendo quelle già presenti per un secondo ruolo.
     *
     * @param idApplic l'ID dell'applicazione di riferimento
     * @param idRuolo  l'ID del ruolo per cui recuperare le azioni
     * @param idRuolo2 l'ID del secondo ruolo da escludere dal risultato
     * @return lista di azioni abilitate solo per il primo ruolo e non per il secondo
     */
    public List<AplAzionePagina> findAzioniPerAppERuolo(BigDecimal idApplic, BigDecimal idRuolo,
            BigDecimal idRuolo2) {
        String queryStr = "SELECT DISTINCT e FROM AplAzionePagina e "
                + "JOIN e.aplPaginaWeb.aplApplic app " + "JOIN e.prfAutors aut "
                + "JOIN aut.prfUsoRuoloApplic uso " + "JOIN uso.prfRuolo ruo "
                + "WHERE app.idApplic = :idApplic " + "AND ruo.idRuolo = :idRuolo "
                + "AND e.aplPaginaWeb IS NOT NULL "
                + "AND NOT EXISTS (SELECT 1 FROM AplAzionePagina e2 " + " JOIN e2.prfAutors aut2 "
                + "JOIN aut2.prfUsoRuoloApplic uso2 " + "JOIN uso2.prfRuolo ruo22 "
                + "WHERE e2 = e " + "AND ruo22.idRuolo = :idRuolo2)";
        Query query = getEntityManager().createQuery(queryStr);
        query.setParameter("idApplic", idApplic.longValueExact());
        query.setParameter("idRuolo", idRuolo.longValueExact());
        query.setParameter("idRuolo2", idRuolo2.longValueExact());
        return query.getResultList();
    }

    /**
     * Restituisce la lista dei servizi web AplServizioWeb associati a una specifica applicazione e
     * ruolo, escludendo quelli già presenti per un secondo ruolo.
     *
     * @param idApplic l'ID dell'applicazione di riferimento
     * @param idRuolo  l'ID del ruolo per cui recuperare i servizi
     * @param idRuolo2 l'ID del secondo ruolo da escludere dal risultato
     * @return lista di servizi web abilitati solo per il primo ruolo e non per il secondo
     */
    public List<AplServizioWeb> findServiziPerAppERuolo(BigDecimal idApplic, BigDecimal idRuolo,
            BigDecimal idRuolo2) {
        String queryStr = "SELECT DISTINCT e FROM AplServizioWeb e " + "JOIN e.aplApplic app "
                + "JOIN e.prfAutors aut " + "JOIN aut.prfUsoRuoloApplic uso "
                + "JOIN uso.prfRuolo ruo " + "WHERE app.idApplic = :idApplic "
                + "AND ruo.idRuolo = :idRuolo " + "AND NOT EXISTS (SELECT 1 FROM AplServizioWeb e2 "
                + " JOIN e2.prfAutors aut2 " + "JOIN aut2.prfUsoRuoloApplic uso2 "
                + "JOIN uso2.prfRuolo ruo22 " + "WHERE e2 = e " + "AND ruo22.idRuolo = :idRuolo2)";
        Query query = getEntityManager().createQuery(queryStr);
        query.setParameter("idApplic", idApplic.longValueExact());
        query.setParameter("idRuolo", idRuolo.longValueExact());
        query.setParameter("idRuolo2", idRuolo2.longValueExact());
    ...
    /**
     * Restituisce tutte le voci di menu AplEntryMenu associate a una specifica applicazione e
     * ruolo.
     *
     * @param idApplic l'ID dell'applicazione di riferimento
     * @param idRuolo  l'ID del ruolo per cui recuperare la voce di menu principale
     ...
     *         specificati
     */
    public List<AplEntryMenu> findTutteVociMenuPerAppERuolo(BigDecimal idApplic,
            BigDecimal idRuolo) {
        String queryStr = "SELECT DISTINCT e FROM AplEntryMenu e " + "JOIN e.aplApplic app "
                + "JOIN e.prfAutors aut " + "JOIN aut.prfUsoRuoloApplic uso "
                + "JOIN uso.prfRuolo ruo " + "WHERE app.idApplic = :idApplic "
                + "AND ruo.idRuolo = :idRuolo " + "AND e.entryMenuPadre IS NOT NULL ";
        Query query = getEntityManager().createQuery(queryStr);
        query.setParameter("idApplic", idApplic.longValueExact());
        query.setParameter("idRuolo", idRuolo.longValueExact());
        return query.getResultList();
     ...
     * @return lista di azioni abilitate solo per il primo ruolo e non per il secondo
     */
    public List<AplPaginaWeb> findTutteLePaginePerAppERuolo(BigDecimal idApplic,
            BigDecimal idRuolo) {
        String queryStr = "SELECT DISTINCT e FROM AplPaginaWeb e " + "JOIN e.aplApplic app "
                + "JOIN e.prfAutors aut " + "JOIN aut.prfUsoRuoloApplic uso "
                + "JOIN uso.prfRuolo ruo " + "WHERE app.idApplic = :idApplic "
                + "AND ruo.idRuolo = :idRuolo " + "AND e.aplEntryMenu IS NOT NULL ";
        Query query = getEntityManager().createQuery(queryStr);
        query.setParameter("idApplic", idApplic.longValueExact());
        query.setParameter("idRuolo", idRuolo.longValueExact());
        return query.getResultList();

...
public class SistemiVersantiHelper extends GenericHelper {
    ...
    /**
     * Restituisce la lista dei sistemi versanti presenti nella vista AplVRicSistemaVersante in base ai parametri di
     * ricerca impostati e all'utente
     *
     * @param filtri
    ...
    /**
     * Restituisce la lista dei sistemi versanti presenti nella vista AplVRicSistemaVersante in base ai parametri di
     * ricerca impostati e all'utente. Usa un oggetto wrapper di tutti i filtri.
     *
     *
     * @param filtri
     *            filtri
     ...
     */
    public List<AplVRicSistemaVersante> getAplVRicSistemaVersanteList(FiltriSistemiVersantiPlain filtri) {
        ...
        if (denominazione != null) {
            queryStr.append(whereWord).append("UPPER(sistemiVersanti.nmSistemaVersante) LIKE :denominazione ");
            whereWord = " AND ";
        }
        String descrizione = filtri.getDescrizione();
        if (descrizione != null) {
            queryStr.append(whereWord).append("UPPER(sistemiVersanti.dsSistemaVersante) LIKE :descrizione ");
            whereWord = " AND ";
        }
        String produttore = filtri.getProduttore();
        if (produttore != null) {
            queryStr.append(whereWord).append("UPPER(sistemiVersanti.nmProduttore) LIKE :produttore ");
            whereWord = " AND ";
        }
        ...
        if (idOrganizApplic != null) {
            queryStr.append(whereWord).append("sistemiVersanti.idOrganizApplic = :idOrganizApplic ");
            whereWord = " AND ";
        ...
        if (!idArchivista.isEmpty()) {
            if (noArchivista.equals("1")) {
                ...
                queryStr.append(whereWord).append("sistemiVersanti.idUserIamArk IN (:idArchivista)");
            }
            whereWord = " AND ";
        } else {
            // Flag checkato
        ...
        queryStr.append(whereWord)
                .append("sistemiVersanti.idUserIamCor = :idUserIam ORDER BY sistemiVersanti.nmSistemaVersante ");
        ...
        List<AplVRicSistemaVersante> listaSistemiVersanti = (List<AplVRicSistemaVersante>) query.getResultList();

        return listaSistemiVersanti;
    }

    /**
     * Restituisce il sistema versante presente nella vista AplVRicSistemaVersante in base ai 3 campi passati in
     * ingresso che formano la chiave
     *
    ...
    /**
     * Restituisce il sistema versante presente nella vista AplVRicSistemaVersante in base ai 2 campi passati in
     * ingresso che formano la chiave
     *
     * @param idSistemaVersante
     *            id sistema versante
    ...
    public long insertAplSistemaVersante(String denominazione, String descrizione, String versione,
            BigDecimal idEnteSiam, String dsEmail, String flPec, String flIntegrazione, String flAssociaPersonaFisica,
            Date dtIniVal, Date dtFineVal, String dsNote) {
        ...
        sistemaVersante.setOrgEnteSiam(getEntityManager().find(OrgEnteSiam.class, idEnteSiam.longValue()));
    ...
    public boolean canDeleteSistemaVersante(long idSistemaVersante) {
        Query query = em.createQuery("SELECT sistemaVersante FROM AplSistemaVersante sistemaVersante "
    ...
    /**
     * Elimina un sistema versante da DB. Se ci sono dei riferimenti nei servizi erogati, cancello il legame con essi
     * (ma non ovviamente il servizio erogato). La chiave esterna sui servizi erogati verrà impostata a null Altrimenti,
     * cancello direttamente il sistema versante.
     *
     * @param idSistemaVersante
     *            id sistema
     */
    ...
    public AplSistemaVersante getAplSistemaVersanteByName(String denominazione) {
        ...
        if (denominazione != null) {
            ...
            List<AplSistemaVersante> ListaSistemiVersanti = (List<AplSistemaVersante>) query.getResultList();
            if (!ListaSistemiVersanti.isEmpty()) {
                sistemaVersante = ListaSistemiVersanti.get(0);
            }
        }
    ...
    public List<AplSistemaVersante> getAplSistemaVersanteValidiList(String tipoUser, BigDecimal idEnteUtente) {
        String queryStr = "";
    ...
    public boolean checkSistemaVersantePerAutoma(BigDecimal idSistemaVersante, BigDecimal idEnteUtente) {
    ...
    public List<AplSistemaVersanteUserRef> getAplSistemaVersanteUserRefList(BigDecimal idSistemaVersante) {
    ...
    public boolean isReferenteDittaProduttriceInSistemaVersante(BigDecimal idSistemaVersante, BigDecimal idUserIam) {
        Query query = getEntityManager()
                .createQuery("SELECT sistemaVersUserRef FROM AplSistemaVersanteUserRef sistemaVersUserRef "
    ...
    public List<AplVLisOrganizUsoSisVers> getAplVLisOrganizUsoSisVers(BigDecimal idSistemaVersante) {
        List<AplVLisOrganizUsoSisVers> list = new ArrayList<>();
        if (idSistemaVersante != null) {
            Query query = getEntityManager()
                    .createQuery("SELECT organizUsoSisVers FROM AplVLisOrganizUsoSisVers organizUsoSisVers "
    ...
    public boolean existsUtentiAssociatiSistemaVersante(BigDecimal idSistemaVersante, String tipoUser) {
        Query query = getEntityManager().createQuery("SELECT 1 FROM UsrUser user "
                + "JOIN user.aplSistemaVersante sistemaVersante " + "WHERE user.tipoUser = :tipoUser "
